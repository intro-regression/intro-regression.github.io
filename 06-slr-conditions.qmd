...
The normality condition is more important, however, when computing the interval for the predicted value for an individual observation. Recall from @sec-slr-theory-predict, that the standard error for these intervals takes into account variability about the regression line (variability in $\mu_{Y|X}$) in addition to variability in the distribution of $Y$ for a the given value of $X$. We assume the shape of this distribution is normal, thus the calculations of $SE_{\hat{y}}$ may be misleading if there are large departures from normality.
