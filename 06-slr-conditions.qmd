...
The **leverage** of the $i^{th}$ observation is a measure of how much influence an observation has on the predicted values, and thus the estimated coefficients (@hoaglin1978hat). It is computed by computing the distance between $x_i$ and the average value of the predictor across all $n$ observations, $\bar{x} = \frac{1}{n}\sum_{j=1}^n x_j$, relative to this distance across all observations.
