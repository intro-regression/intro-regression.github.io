...
<!--# add definitions of aroma and flavor from salt, acid, fat, heat in the "how salt affects flavor section-->
...
- `country_of_origin`: Country in which the coffee beans are grown
...
#| fig-cap: Univariate and bivariate exploratory data analysis for flavor and aroma grades
#| fig-subcap: 
#|  - "Distribution of the flavor grade"
#|  - "Distribution of the aroma grade"
#|  - "Relationship between the flavor and aroma grades"
#| layout: [[1,1],[1]]
...
#| tbl-cap: "Summary statistics for aroma and flavor grades"
coffee_ratings |>
  skim(aroma, flavor) |>
...
#| tbl-cap: "Linear regression model using the aroma grade to understand variability in the flavor grade. Table includes 95% confidence intervals for the coefficients"
#| echo: false
...
#| fig-cap: "Residuals versus predicted values from the model in @tbl-coffee-model"
#| echo: false
...
#| fig-cap: Example of violation in the linearity condition, in which the residuals are not randomly scattered around 0. From the plot, we can generally guess with some certainty whether the residual will be positive or negative for a given predicted value or range of predicted values. 
#| echo: false
...
# make plot of residuals vs predicted
coffee_model_aug |>
  filter(country_of_origin %in% top_countries) |>
  ggplot(aes(x = country_of_origin, y = .resid)) + 
...
@fig-indep-subgroup shows the distribution of residuals for the five countries with the most observations in the data. The distribution of residuals for each country is approximately centered around zero. This indicates that the model is not systematically over- or underpredicting for a particular country (at least out of these five).[^06-slr-conditions-2] Thus, based on this analysis of subgroups and the description of the coffee data, we conclude that the [**independence condition is satisfied**]{.underline}. We do not have evidence of a serial or cluster effect in the data description.

[^06-slr-conditions-2]: Here, we have analyzed the distribution of residuals for five countries to illustrate the independence condition. In practice, we may want to analyze the distribution of residuals for more countries in the data.
...
#| fig-cap: "Distribution of the residuals for the model in @tbl-coffee-model with normal density curve"
#| echo: false
...
#| fig-cap: Plot of the residuals versus predicted values for the model in @tbl-coffee-model. The vertical spread is approximately equal for the majority of predicted values, as shown by the residuals generally falling between the two dotted lines when moving along the $x$-axis.
#| echo: false
ggplot(data = coffee_model_aug, aes(x = .fitted, y = .resid)) +
  geom_point() +
...
From @fig-coffee-equal-var, the vertical spread of the residuals is approximately equal, as the vast majority of the residuals are between the two dotted lines as we move along the $x$-axis. There appears to be less variability for very low and very high predicted value, but we note there are also few observations at these points. This is often the case at the extreme ends of the distribution of predicted values. Thus, we went evaluate the equal variance condition, we want to focus on the variability in the majority of the data. Given the approximately equal vertical spread of the residuals in the majority of the data, **the equal variance condition is satisfied.**
...
- **Independence**: Use the description of the data and data collection process to assess if the observations can reasonably be treated as independent. If data are collected over time, examine the plot of residuals versus time to assess potential serial effect. If unaccounted for subgroups are represented in the data, examine the plot of residuals versus predicted for each subgroup.
...
If the independence condition is not met, more advanced statistical methods, called *multilevel models* or *hierarchical models*, would be required to properly deal with the correlated residuals. We introduce these models in @sec-random-intercepts.
...
By the Central Limit Theorem, we know the distribution of the estimate slope (the statistic of interest) is normal when the sample size is sufficiently large (see @sec-slr-inf-clt for more details), regardless of the distribution of the original data. Therefore, when the sample size is "large enough", we can be confident that the distribution of the estimated slope $\hat{\beta}_1$ is normal, even if the residuals (and thus the response variable) do not follow a normal distribution. This is also true when computing the interval for the estimated mean for a given value of the predictor, introduced in @sec-slr-theory-predict.

The normality condition is more important, however, when computing the interval for the predicted value for an individual observation. Recall from @sec-slr-theory-predict, that the standard error for these intervals takes into account variability about the regression line (variability in $E(Y|X)$) in addition to variability in the distribution of $Y$ for a the given value of $X$. We assume the shape of this distribution is normal, thus the calculations of $SE_{\hat{y}}$ may be misleading if there are large departures from normality.

A common threshold for a "large enough" sample size is 30. This means if the sample has at least 30 observations (which is often the case for modern data sets!), we can use all the methods we've discussed to fit a simple linear regression model and draw conclusions from inference, even if the residuals are not normally distributed. Thus the inferential methods based on the Central Limit Theorem are considered robust to violations in the normality assumption, because we can feel confident about the conclusions from the inferential procedures even if the normality condition is not satisfied (given $n > 30$). One note of caution is that 30 should be used as a general guideline but not a strict threshold. Therefore, if the sample size is very close to 30, then the inferential methods based on the Central Limit Theorem are not robust to major departures from normality such as extreme skewness or outliers.
...
### Recap {#sec-slr-conditions-recap}
...
|                 |                       |                            |                                                                                     |
|                 |                       |                            | Important if $n < 30$ or computing prediction intervals for individual observations |
+-----------------+-----------------------+----------------------------+-------------------------------------------------------------------------------------+
| Equal variance  | Not needed            | Not needed                 | Important                                                                           |
+-----------------+-----------------------+----------------------------+-------------------------------------------------------------------------------------+

: Conditions necessary based on the analysis task
...
An observation is considered an **influential point** if the estimated regression coefficients noticeably differ when the point is included in the data used to fit the model versus when it is not. @fig-influential-point illustrates the potential impact an influential point can have on the estimated regression line. Additionally, influential points can affect $SE_{\hat{\beta}_1}$, the estimated standard error of the slope, which can result in unreliable inferential conclusions as described in the previous section.

```{r}
#| label: fig-influential-point
#| fig-cap: Simulated data showing the effect of an influential point represented by the red triangle. The slope of the line changes when the influential point is removed from the data set.
#| fig-subcap: 
...
The **leverage** of the $i^{th}$ observation is a measure of how much influence an observation has on the predicted values, and thus the estimated coefficients (@hoaglin1978hat). It is computed as the distance between $x_i$ and the average value of the predictor variable across all $n$ observations, $\bar{x} = \frac{1}{n}\sum_{j=1}^n x_j$, relative to this distance across all observations.

When doing simple linear regression, an observation's leverage, denoted $h_i$, is calculated using @eq-slr-leverage.

$$h_i = \frac{1}{n} + \frac{(x_i - \bar{x})^2}{\sum_{j=1}^n(x_j - \bar{x})^2}$$ {#eq-slr-leverage}
...
A commonly used threshold for identifying observations with *high leverage* is $\frac{2(p+1)}{n}$ ( $\frac{2 \times 2}{n}$ for simple linear regression). This means that an observation is considered to have high leverage if its value of leverage is greater than twice the average value of leverage for the observations in the data set. This threshold for high leverage was defined in @hoaglin1978hat when sample sizes were generally smaller compared to modern data. Therefore, this threshold can result in a overwhelming number of observations being identified as having high leverage when the sample size is large (the case in most modern data sets). Therefore, some data scientists suggest using a threshold $\frac{3(p+1)}{n}$ to identify high leverage observations when analyzing large data sets.

::: {.analysis_in_practice latex=""}
Leverage only depends on the values of the predictor variable(s). It does [**not**]{.underline} depend on values of the response.
:::

Let's take a look at the leverage for the observations in the coffee ratings data set. There are `r nrow(coffee_ratings)` observations in the data and one predictor variable, so the threshold for identifying observations with high leverage is $\frac{2 \times 2}{1338} =$ `r round(2 * 2/ nrow(coffee_ratings), 3)`.
...
@fig-coffee-leverage is a visualization of the leverage for each observation in the data set. The red, horizontal dotted line marks the threshold for identifying points as having high leverage. There are `r n_large_leverage` observations that have high leverage.

```{r}
#| label: fig-coffee-leverage
#| fig-cap: "Leverage for observations in the coffee data"
...
::: {.yourturn latex=""}
There are `r n_large_leverage` observations that have high values for the leverage. What does it mean for an observation to have "high leverage" in the context of the coffee data?[^06-slr-conditions-3]
:::

[^06-slr-conditions-3]: An observation with high leverage has an aroma value that is far away from the mean value of aroma in this data set. It can have a value that is much lower or much higher than the mean.

Let's take a look at the observations with the highest values of leverage to get a better understanding about which points may be high leverage.

```{r}
#| label: tbl-coffee-large-leverage
#| tbl-cap: Five observations with the highest leverage values
#| echo: false
...
Recall that the leverage only depends on the values of the predictor variable, so we only need to consider `aroma` to better understand why these points are high leverage. From the EDA in @sec-coffee-intro, the average aroma grade in the data set is `r round(mean(coffee_ratings$aroma), 3)` points and the standard deviation is `r round(sd(coffee_ratings$aroma), 3)`. The observations with the highest values of leverage have aroma grades that are either much higher (e.g., 8.75) or much lower (e.g., 5.08) than the average. Thus the high leverage points include coffees that both smell much better and perhaps much worse than the majority of the coffees in the data.

Even when we identify points with high leverage, we still want to understand how these points are (or are not) influencing the model results. Thus, knowing an observation has high leverage is not enough to warrant any specific action. We'll examine Cook's distance to determine if the observation is influential in the model in @sec-cooks-distance and discuss approaches to deal with such influential points in @sec-outliers-impact .
...
The residuals have the same units as the response variable, so what is considered a residual with large magnitude depends on the scale and range of the response variable. This means what is considered a "large" residual can be different for every analysis. We can address this by instead examining **standardized residuals**, defined as the residual divided by its standard error. @eq-std-resid shows the formula to calculate the standardized residual for the $i^{th}$ observation.

$$std.resid_i = \frac{e_i}{SE_{e_i}} = \frac{e_i}{\hat{\sigma}_{\epsilon} \sqrt{1 - h_i}}$$ {#eq-std-resid}

The residuals for every analysis are now on the same scale. Thus, we can use a common threshold to determine residuals that are considered to have large magnitude. An observation is an outlier in the response if its standardized residual has a magnitude greater than two, $|std.resid_i| > 2$ ($std.resid_i < - 2 \hspace{1mm} \text{ or }\hspace{1mm} std.resid_i > 2$). For large data sets, we recommend a more restrictive threshold of 3, so there is not an overwhelming number of observations considered outliers that may require further evaluation.
...
<!--# add callout connecting large standardized residuals to high leverage?-->

```{r}
#| label: fig-coffee-stdresid
#| fig-cap: Standardized residuals versus observation number for model in @tbl-coffee-model
...
These are the five observations with the largest magnitude residuals are shown in @tbl-coffee-large-resid.

<!--# think of a way to visualize observations with large magnitude residuals-->

```{r}
#| label: tbl-coffee-large-resid
#| tbl-cap: Five observations with largest magnitude residuals
#| echo: false
...
We see from @tbl-coffee-large-resid and @fig-coffee-large-resid that most of these observations have flavor ratings that are lower than would be expected from the model based on their aroma ratings. The exception is the observation with the largest magnitude standardized residual which has a higher actual flavor rating than the model predicts based on its very low aroma rating.

```{r}
#| echo: false
#| label: fig-coffee-large-resid
#| fig-cap: Scatterplot of flavor grade versus aroma grade. Points marked with a red triangle are the observations with large standardized residuals with magnitude greater than 3.
...
As with observations that have high leverage, we want to assess whether these points have out-sized influence on the model coefficients to help determine how to deal with these outliers (or if we need to do anything at all). To do so, we will look at the last diagnostic, Cook's distance.
...
**Cook's distance** [@cooksdistance] is a measure of an observation's overall impact on the estimated model coefficients. In @eq-cooks-d, we see that Cook's distance takes into account an observation's leverage, $h_i$, the standardized residuals, $std.resid_i$, and the number of predictors in the model, $p$ ($p=1$ for simple linear regression).

$$D_i = \frac{1}{p}(std.resid_i)^2\Big(\frac{h_i}{1-h_i}\Big)$$ {#eq-cooks-d}
...
#| fig-cap: Cook's distance for observations in the coffee data
...
#| tbl-cap: Estimated model coefficients for the model with and without the influential observation
#| echo: false
...
3.  **Model conditions and diagnostics**: Check the model conditions (LINE) to assess whether the model is a good fit for the data.
    ...
    - If there are influential points, check the leverage and standardized residuals to try to understand why these points are influential. Use this information, along with the analysis objective and subject matter expertise, to determine how to best proceed with dealing with these points.
...
#| tbl-cap: First ten rows of output produced by `augment()`
#| echo: false
