...
In @sec-ch-logistic, we introduced logistic regression models for binary response variables. Here we will show some of the mathematics underlying these models, making use of the matrix notation for regression introduced in @sec-appendix-linear.
...
$$\text{logit}(\pi) = \log\Big(\frac{\pi}{1-\pi}\Big) = \beta_0 + \beta_1X_1 + \beta_2X_2 + \dots + \beta_pX_p$$ {#eq-logistic-model-appendix}

where $\pi = Pr(Y = 1)$.

Similar to linear regression, we can write a matrix representation of @eq-logistic-model-appendix.

$$\text{logit}(\boldsymbol{\pi}) = \log\Big(\frac{\boldsymbol{\pi}}{1 - \boldsymbol{\pi}}\Big) = \mathbf{X}\boldsymbol{\beta}$$ {#eq-logistic-matrix}
...
Though not directly in @eq-logistic-model-appendix or @eq-logistic-matrix, the underlying data also includes $\mathbf{y}$, an $n\times 1$ vector of the binary response variables.
...
The response variable follows a Bernoulli distribution, such that $P(Y = y_i) = \pi_{i}^{y_i}(1 - \pi_i)^{1-\y_i}$. Let $\mathbf{x}_i^\mathsf{T}$ be the $i^{th}$ row of the design matrix $\mathbf{X}$. Then, using @eq-logistic-probability-matrix, we have

$$P(Y=y_i) = \Big(\frac{e^{\mathbf{x}_i^\mathsf{T}\boldsymbol{\beta}}}{1 + e^{\mathbf{x}_i^\mathsf{T}\boldsymbol{\beta}}}\Big)^{y_i}\Big(1 - \frac{e^{\mathbf{x}_i^\mathsf{T}\boldsymbol{\beta}}}{1 + e^{\mathbf{x}_i^\mathsf{T}\boldsymbol{\beta}}}\Big)^{1 - y_i}$$ {#eq-logistic-proby-matrix} that the likelihood function is a measure of how likely we observe the data given particular values of the model parameters $\hat{\boldsymbol{\beta}}$.
...
$$L(\boldsymbol{\beta}|\mathbf{X},\mathbf{y}) = \prod_{i=1}^n\Big(\frac{e^{\mathbf{x}_i^\mathsf{T}\boldsymbol{\beta}}}{1 + e^{\mathbf{x}_i^\mathsf{T}\boldsymbol{\beta}}}\Big)^{y_i}\Big(1 - \frac{e^{\mathbf{x}_i^\mathsf{T}\boldsymbol{\beta}}}{1 + e^{\mathbf{x}_i^\mathsf{T}\boldsymbol{\beta}}}\Big)^{1 - y_i}$$ {#eq-logistic-likelihood}
...
\end{aligned}$$ {#eq-logistic-log-likelihood}
...
$$\hat{\boldsymbol{\beta}} \sim N(\boldsymbol{\beta}, (\mathbf{X}^\mathsf{T}\mathbf{V}\mathbf{X})^{-1})$$ {#eq-dist-beta-logistic}
...
[^appendix-logistic-1]: Recall that the variance of the Bernoulli distribution depends on $\pi$, so each observation has a different variance. This is in contrast to linear regression, where all observations have the same variance $\sigma^2$.
