...
<!--# update this to include missing data -->
...
This function imputes the missing values and creates a new logical variable with the suffix `_imp` that is `TRUE` if the value was imputed and `FALSE` otherwise. The summary statistics for the variable `calories` with the missing values imputed are in table @tbl-missing-data-calories-imputed.
...
Because the ultimate analysis goal is modeling `avg_rating`, we can evaluate whether the observations in the imputed data have a similar relationship between the response and predictor variable as the observed data. For example, let's look at the relationship between the number of reviews and average ratings using `ggmice()`. The function will automatically identify the observed versus imputed observations.
...
[^13-special-topics-3]: The accuracy is the `r (563 + 2219 + 94)/nrow(voters)` $((563 + 2219 + 94)/5836)$. The misclassification rate is `r 1 - (563 + 2219 + 94) / nrow(voters)` $(1 - 0.493)$.
