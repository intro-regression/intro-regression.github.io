...
Our analysis objective is to use the various recipe characteristics to explain variability in the average rating. Here we will focus on handling missing data while using the analysis objective to inform our analysis decisions.
...
recipes_sub <- hotdeck(recipes_sub, variable = "calories",  ord_var = c("fat", "carbs", "protein"))
```
...
```{r}
#| echo: false 
recipes_sub_mice <- mice(recipes_sub, m = 5, printFlag = FALSE)
```
...
recipes_sub_mice_1 <- complete(recipes_sub_mice, 1)

skim(recipes_sub_mice_1)
```
...
#| fig-cap: "Relationship betwen `reviews` and `avg_rating` for observed and imputed data. Note that `avg_rating` has missing data, so imputed data could be mputed for either variable or both variables."
#| echo: true
...
[^13-special-topics-3]: The accuracy is the `r (563 + 2219 + 94)/nrow(voters)` $(563 + 2219 + 94)/5836)$. The misclassification rate is `r 1 - (563 + 2219 + 94) / nrow(voters)` $(1 - 0.407)$.
...
[^13-special-topics-8]: We are 95% confident that improvement in GPA from participating in Project ACE is between 0.003 and 0.39 points.
