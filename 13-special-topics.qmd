...
A more informed approach for imputing missing values is by using **nearest neighbor imputation**, in which the missing values are imputed using values from observations that look similar to the one with missing data. This can be done using other observations within the data set, called **hot deck imputation** \[(paper: <https://pmc.ncbi.nlm.nih.gov/articles/PMC3130338/>)\], or by using other observations in an external data set, called **cold deck imputation** \[CITATION\]. By using the more informed approach to filling in missing data, we reduce some of the limitations of imputation that occur with mean imputation. There is still the potential to underestimate the variability of the imputed variable, but there is generally little impact to the regression coefficients and inferential statistics \[Cite gelamn here?\].

<!--# check description of hotdeck imputation and add citations. https://www.jstatsoft.org/article/view/v074i07-->

These and any imputation methods can potentially result in biased data that does not accurately reflect the underlying population. A strategy introduced by [CITATION RUBIN 1987? Or 1997] to mitigate the effects of these biases is through **multiple imputation**, in which multiple data sets are created using different imputation methods, then the results are averaged across the imputed data sets. In @sec-missing-data-r, we show code for conducting multiple imputation in R We point the reader to the texts in @sec-missing-data-further-reading for more details about multiple imputation. <!--# do i need to cite something?-->
...
Use `ggmice()` from the **ggmice** package [CITATION] to visualize the distribution of the imputed values for each new data frame created by `mice()`. <!--# explain the code below-->
```{r}
library(ggmice)
...
Since the ultimate aim is modeling, we can evaluate whether the imputed data has a similar relationship between the response and predictor variable. For example, if we wish to use the data to explain variability in the average rating, we can look at the relationship.
...
We have just provided a brief introduction to handling missing data, but there is a wide variety of research and strategies for dealing with such data in an analysis. To learn more more about strategies for handling missingness and the analysis implications, see [CITATION] Little and Rubin 2002 and  Chapter 25 of [CITATION]. We also refer the reader to [CITE MICE Documentation] and CITE https://www.jstatsoft.org/article/view/v074i07 and [CITE GGMICE documentation] for more details on working with missing data in R. 
