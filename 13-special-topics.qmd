...
## recipes subset for missing data chapter 
recipes_sub <- read_csv("data/recipes-sub.csv")
...
This chapter introduces readers to a collection of models that are extensions of linear and logistic regression. The goal is to provide an introduction to the topic along with resources for those interested in learning more. The following topics are introduced:
...
In @sec-ch-eda, we presented exploratory data analysis, one of the first steps in the data science workflow (see @fig-data-science-workflow). One of the main purposes of exploratory data analysis is to understand the distributions of the variables in the data set, and as part of that identify special features in the data such as outliers or missing data before moving on to modeling. We introduced strategies for dealing with outliers that impact regression modeling in @sec-outliers-impact, and in this section we will discuss some strategies for dealing with missing data.

<!--# notes for reference: https://www2.stat.duke.edu/courses/Fall19/sta210.001/slides/lec-slides/23-model-val-missing.html#31-->

<!--# reference for MICE package: https://amices.org/mice/-->

<!--# Link to Gelman book - See Chapter 25-->

### Data: Recipes {#sec-missing-data-dataset}

We will return to the `recipes` data from @sec-ch-eda that contains details about the nutrition, preparation and cooking time, and the reviews and ratings for recipes on Allrecipes.com. As we conducted exploratory data analysis in @sec-ch-eda, we observed that some observations had missing values for one or more variables we were interested in analyzing. Here, we will look at the different types of missingness in the data and introduce some strategies for handling missing data.

The data set was adapted from the `cuisines` data frame in the **tastyR** package [@tastyR]. We will primarily focus on the nutrition and ratings information in this section and work with the data set `recipes_sub`. The variable definitions in `recipes_sub` are below.
...
::: {.objective latex=""}
The analysis objective is to use the various recipe characteristics to explain variability in the average rating. Here we will focus on handling missing data while using the analysis objective to inform our analysis decisions.
:::

```{r}
#| eval: false

## Make recipes_sub data frame
recipes_sub <- recipes |> 
  select(name, calories, fat, carbs, protein, avg_rating, total_ratings, reviews)

recipes_sub |> write_csv("data/recipes-sub.csv")
```

### Identifying missingness {#sec-missing-data-identify}

We begin by observing how missingness appears in the data set. @tbl-missing-example shows examples of five recipes from the `recipes_sub` data frame, each of which have different types of missingness. Let's take a look at this subset of recipes and what we observe about the missingness in each.

```{r}
#| label: tbl-missing-example
#| tbl-cap: "Examples of missingness in the `recipes` data"

options(knitr.kable.NA = 'NA')

recipes_use <- c("Danish Butter Cookies", 
                 "Brazilian Quentao",
...
recipes_sub |> 
  select(name, calories, fat, carbs, protein, avg_rating, total_ratings, reviews) |> 
  filter(name %in% recipes_use) |>
  kable()
```

- Chef John's Nashville Hot Chicken is an example of a complete case. A **complete case** is an observation that has values for all variables in the analysis. These observations have no missing values, so they are ready for analysis without applying the methods in this section.

- Rose Water Turkish Delight has a missing value for one variable, `fat`, but has data for all other variables.

- The Brazilian Quentao recipe has missing values for all the variables related to nutrition information (`calories`, `fat`, `carbs`, and `protein`). This is a recipe for mulled wine, a type of drink; therefore, the missingness is logical in this case. These nutrition variables are most relevant to food dishes, so it is not surprising that such information is not available for this drink recipe.

- The Danish Butter Cookies has missing values for the variables related to the user reviews (`avg_rating`, `total_ratings`, `reviews`). Upon looking at the webpage for this recipe, we observe that this recipe has no reviews. Because the data are scraped from the website, it is logical that these values appear as missing in the data set. One consideration for the analysis is how to deal with the missing data for `avg_rating`, because it is the response variable on interest based on the analysis objective in @sec-missing-data-dataset. One option is to narrow the scope of the analysis to recipes that have at least some minimum number of ratings or reviews.

- The Chicken Adobo Tacos recipe has missing values for all variables of interest. This observation is not currently usable for the analysis, because it does not have any observed values that can inform the data exploration and modeling. We may consider removing this observation from the analysis given the lack of useful information. We discuss how this type of analysis decision could impact the results in @sec-missing-data-types.

These recipes have illustrated the two ways missingness can occur in the data: item non-response and unit non-response. **Item non-response** occurs when an observation has missing values for one variable or a small subset of variables. This is was the case with the Rose Water Turkish Delight, Brazilian Quentao, and Danish Butter Cookie recipes. Depending on the reason for missingness, we can use one of various imputation strategies to fill in the missing values. **Unit non-response** occurs when an observation has so many missing values that is not useful for the analysis. This is the case with the Chicken Adobo Tacos recipe that is missing values for every variable except `name`.

### Types of missingness {#sec-missing-data-types}

In @sec-missing-data-identify, we observed how missingness can appear in the data and provided potential explanations for some of the patterns of missingness. Now instead of looking at individual observations, let's look at the patterns of missingness within a single variable. There are three patterns of missingness within a single variable: missing completely at random, missing at random, and missing not at random.

A variable has observations **missing completely at random (MCAR)** if there is no explainable pattern to the missingness. This means there is no explainable pattern to the missingness. A variable is **missing at random (MAR)** if the pattern of missingness depends on other variables, but not the variable itself. For example, in the `recipes` data, missingness for the `avg_rating` variable is due to a recipe having no ratings (`total_ratings = 0`), not due to something about the average rating for the recipe. The last type of missing is **missing not at random (MNAR).** A variable has observations MNAR if the pattern of missingness depends on the variable itself. For example, in the recipes data `total_ratings` is MNAR, because a missing value for this variable means a recipe has 0 ratings.

It is important to understand the pattern of missingness, because it helps us understand how our approach for handling the missing values may bias the analysis results. We'll briefly discuss these impacts as we introduce strategies for handling missing data in @sec-missing-data-strategies

### Strategies for handling missing data {#sec-missing-data-strategies}

Now that we understand how to identify missingness in the data and patterns of missingness, let's discuss some strategies for handling missing values. We introduce some strategies here and direct the reader to @sec-missing-data-further-reading for a more in-depth discussion about missing data.

#### Complete case analysis

Perhaps the easiest way to deal with missingness is to do a **complete case analysis**, in which observations with any missing values are removed from the analysis.[^13-special-topics-1] This is the approach used in many R functions. For example, when creating visualizations using **ggplot2** [@ggplot2], only observations that have observed values for all the variables in the visualization are included. There is a warning produced that indicates how many observations were excluded from the visualization. The regression functions `lm()` and `glm()` also exclude observations that have missing values for the response variable or any of the predictor variables. Unlike **ggplot2**, however, there is no warning produced. Therefore, it is imperative to conduct exploratory data analysis to identify and handle the missingness before any modeling.

[^13-special-topics-1]: This has been the primary approach used in the book, since our focus has been on introducing regression methods.

Complete case analysis provides a good example of the importance of understanding the pattern of missingness. If the missingness is MCAR, then we could reasonably assume removing observations with missingness does not systematically bias the data, because the missingness is random. If the missingness is MAR or MNAR, however, simply removing observations with missingness could potentially bias the data. For example, if we removed observations with missing values for `avg_rating` from an analysis of the `recipes` data, we would be systematically excluding recipes with no ratings. These could be newer recipes that haven't been on the site for long or recipes that are generally not popular. It could be fine to narrow the scope of the analysis to exclude new and unpopular recipes, but it is important that we are aware of this limitation and clearly communicate it as we interpret results from the analysis.

#### Imputation

In general, the alternative to complete case analysis is **imputation**, a systematic approach for filling in the missing values. By filling in the missing values, we are able to retain all the observations for the analysis and thus avoid some of the potential biases from complete case analysis. The most straightforward imputation is based on a logical rule<!--# does logical rule need to be cited?-->. If the missingness is logical, such as the missingness for `total_ratings`, we can impute the missing values using a logical rule. For example, recipes that had missing data for `total_ratings`, in fact had no ratings; thus, we can impute missing values of `total_ratings` with `0`.

If we are imputing missing values for a categorical predictor, we can fill in the missing values with `Missing` or a similar informative label. By doing this, we create a new level of the categorical predictor that identifies the missing observations. This is similar to the `No Response` or `Refused to respond` level on surveys to denote questions in which the participate did not provide a response. If we use the variable as a predictor in a regression model, an indicator variable is created for the new `Missing` level in addition to the other existing levels (with the exception of the baseline level). If the indicator for `Missing` is statistically significant, then we may conclude that the observations with missing data for that variable are statistically different from the baseline category. This could indicate the missingness is not completely random, but in fact represents a subset of the population. <!--# make sure this is correct.-->

There is a vast array of methods for imputing missing values for quantitative variables. One commonly used approach is **mean imputation**, in which the missing values are imputed using the mean value computed from the observed values of that variable. The primary advantages to this method are that it is easy to execute and easy to understand and explain to others. The primary disadvantage is that imputing all the missing values using the mean can result in a distorted <!--# need different word so it's not the same--> distribution of the variable, in which an out-sized proportion of the distribution is concentrated at the mean value. If this is the case, we may underestimate the variability in the variable and thus potentially compute inaccurate regression coefficients and inferential statistics. Therefore, we should interpret results with caution if a large proportion of observations were imputed using the mean.

A more informed approach for imputing missing values is by using **nearest neighbor imputation**, in which the missing values are imputed using values from observations that look similar to the one with missing data. This can be done using other observations within the data set, called **hot deck imputation**, or by using observations from an external data set, called **cold deck imputation**. By using the more informed approach to filling in missing data, we reduce some of the limitations of mean imputation. There is still the potential to underestimate the variability of the imputed variable, but there is generally little impact to the regression coefficients and inferential statistics [@gelman2007data].

These and any imputation methods can potentially result in biased data that does not accurately reflect the underlying population. A strategy to mitigate the effects of these biases is **multiple imputation** [@rubin1978multiple; @rubin1987multiple] in which multiple data sets are created using different imputation methods, then the results are averaged across the imputed data sets. In @sec-missing-data-r, we show code for conducting multiple imputation in R, and we point the reader to the texts in @sec-missing-data-further-reading for more detail about multiple imputation. <!--# do i need to cite something?-->

::: {.analysis_in_practice latex="1"}
If a variable has a high proportion of missing values, then the variable may not be useful for the analysis even after imputation. If this is the case, consider if there is another variable or combination of variables in the data set that provide similar information as the with one with substantial missingness.
:::

### Missing data in R {#sec-missing-data-r}

The first step to identifying missingness is identifying where it occurs in the data set. Presumably, we could visually inspect the data, as in @sec-missing-data-identify, but this approach is tedious and unreliable when the data set has a lot of observations and/or variables. There are multiple functions in R that produce variable summaries, including the proportion of missing observations.

#### Variable summaries

The first is `summary()`, a base R function. To get a summary of all columns of a data frame, we input the name of the data frame in the function, e.g., `summary(recipes_sub)`. We can also use `summary()` and `select()` in a pipeline to specify a subset of variables to include in the summary.
...
Another option for variable summaries is `skim()` from the **skimr** package [@skimr-2] introduced in @sec-eda-data-summaries. The code and output produced by `skim()` are in @tbl-missing-data-skim. The first part of the output is an overall summary of the data, and the second part is the individual variable summaries, organized by variable type. For the purposes of this section, the most important information is in the `n_missing` and `complete_rate` columns, specifying the number of observations with missing values and the percent of observations with values, respectively.
...
```{r}
#| label: tbl-missing-data-skim
#| tbl-cap: "Subset of output from `skim()`"
#| echo: false
...
We can "manually" impute missing values using a logical rule or mean imputation utilizing `if_else()` in the **dplyr** package [@dplyr]. Below is code to use mean imputation for the missing values of `reviews`.

<!--# maybe change this to a logical imputation to more conceptually align with previous text.-->

```{r}
#| echo: true 

# get mean of observed data
mean_reviews <- mean(recipes$reviews, na.rm = TRUE)

# impute missing values with mean
recipes_sub <- recipes_sub |>
  mutate(reviews_imputed = if_else(is.na(reviews), mean_reviews, reviews)
  )
```

The summary statistics for `reviews_imputed` with missing data handled using mean imputation are in @tbl-missing-data-skim-reviews-imputed.
...
recipes_sub |>
  skim(reviews_imputed)
```

```{r}
#| echo: false
#| label: tbl-missing-data-skim-reviews-imputed
#| tbl-cap: "Summary of `reviews` with imputed values"
recipes_sub |>
  skim(reviews_imputed) |>
  select(-numeric.hist)
```

Hot deck imputation is done using `hotdeck()` from the **VIM** package [@VIM]. Observations are arranged based on one to three specified ordering variables, and the missing values are imputed based on the values from "nearest neighbor" observations. Below is the code to impute the missing values for `calories` using hot deck imputation, where the nearest neighbors are defined based on the other nutrition variables: `fat`, `carbs`, and `protein`.

```{r}
#| echo: true


recipes_sub <- hotdeck(recipes_sub, variable = "calories", 
                       ord_var = c("fat", "carbs", "protein"))
```

This function imputes the missing values and creates a new logical variable with the suffix `_imp` that is `TRUE` if the value was imputed and `FALSE` otherwise. The summary statistics for the variable `calories` with the missing values imputed are in @tbl-missing-data-calories-imputed.

```{r}
#| label: tbl-missing-data-calories-imputed
#| tbl-cap: "Summary of `calories` with imputed values"
recipes_sub|>
  skim(calories) |>
  select(-numeric.hist)
```

Multiple imputation is conducted using `mice()` in the **mice** package [@mice]. In the code below, we input the data frame that contains missing values and the number of complete data sets we want to generate to compute the final results. Note that the default value is `m = 5`. The argument `printFlag = FALSE` suppress output about each iteration. We save the results as a new data frame, `recipes_sub_mice`.

```{r}
#| echo: true
recipes_sub_mice <- mice(recipes, m = 5, printFlag = FALSE)
```

The `complete()` function is used to view the completed data frames. In the code below, we view the first complete data frame and provide a summary of the variables in the completed data frame.

```{r}
#| echo: true
#| eval: false

recipes_mice_complete_1 <- complete(recipes_sub_mice, 1)

skim(recipes_mice_complete_1)
```
...
It is often helpful to visualize the imputed data in addition to the examining the summary statistics. The function `ggmice()` from the **ggmice** package [@ggmice] to visualize the distribution of the imputed values for each new data frame created by `mice()`. It follows **ggplot2** syntax, so we can use the regular **ggplot2** customization options. In the code below, we view the distribution of `reviews` for the observed data and each imputed data set. The imputed data sets are defined by the variable `.imp`.

```{r}
#| label: fig-ggmice-reviews
#| fig-cap: "Distribution of `reviews` for observed (blue) and imputed (red) data"
#| echo: true

library(ggmice)

ggmice(recipes_sub_mice, aes(x = .imp, y = reviews)) +
  geom_boxplot() +
  labs(x = "Imputation number",
       y = "Number of reviews")
```

Because the ultimate analysis goal is modeling `avg_rating`, we can evaluate whether the observations in the imputed data have a similar relationship between the response and predictor variables as the observed data. For example, let's look at the relationship between the number of reviews and average ratings using `ggmice()`. The function will automatically identify the observed versus imputed observations.

```{r}
#| label: fig-ggmice-reviews-v-rating
#| fig-cap: "Relationship betwen `reviews` and `avg_rating` for observed and imputed data. Note that `avg_rating` has missing data, so values could be imputed for either variable or both variables."
#| echo: true

ggmice(recipes_sub_mice, aes(x = reviews, y = avg_rating)) +
  geom_point() + 
  labs(x = "Number of reviews",
       y = "Average rating")
```

The plot in @fig-ggmice-reviews-v-rating puts the values from all imputed data frames on the sample plot, so a given imputed observation is represented multiple times on the plot. If we wish to look at the relationship between two variables separately for the observed data and each imputed data set, we can facet by the imputation number (`.imp`) as in `fig-ggmice-reviews-v-rating-facet`.

```{r}
#| label: fig-ggmice-reviews-v-rating-facet
#| fig-cap:  "Relationship betwen `reviews` and `avg_rating` faceted by imputation number"
#| echo: true
...
### Further reading {#sec-missing-data-further-reading}

This section provided a short introduction to working with missing data, but there is a large body of research and strategies for dealing with such data in an analysis. For a more detailed introduction to working with missing data and the analysis implications of missingness, see *Statistical Analysis with Missing Data* [@little2019statistical] and Chapter 25 of *Data Analysis Using Regression and Multilevel/Hierarchical Models* [@gelman2007data].
...
- `voter_category`: A respondent's past voting behavior, categorized as
...
\end{aligned}$$ {#eq-multinom-general}

From @eq-multinom-general, we see a few features of the multinomial logistic model. We start by choosing a baseline level for the response variable. Each equation in the fitted model quantifies the log odds of being at another level of the response variable versus the baseline[^13-special-topics-2]. We saw this implicitly in logistic regression, where the response variable, $\log(\frac{\pi}{1-\pi})$, is the "log odds of $Y = 1$ versus $Y = 0$."

[^13-special-topics-2]: Similar to how categorical predictors are interpreted as one level versus the baseline (see [@sec-mlr-interpret-categorical]).

The next feature of multinomial logistic regression is the multiple equations that comprise the model. More specifically, if the response variable has $K$ levels, then there will be $K - 1$ equations in the model. We will ultimately use the model to compute the predicted probabilities an observation takes for each level of the response variable, so we need all $K-1$ equations in order to compute the predicted probabilities for every level of the response variable.
...
The interpretation for the coefficients of the multinomial logistic model is very similar to the interpretation for the logistic model in @sec-logistic-interpret-coef. Like logistic regression, the model coefficients represent the change in the log odds, but in practice we want to write interpretations in terms of the odds. The primary difference in the interpretation is the explicit statement of the baseline level of the response variable.
...
> *For each additional year older, the odds of an individual voting sporadically versus rarely/never are expected to multiply by `r round(exp(0.041), 3)`* $(e^{0.041})$*, holding political affiliation constant.*

Next, we interpret the effect of political affiliation on the voting frequency. As with other models, we write the interpretation in reference to the baseline level of the categorical predictor. The interpretation of the coefficients for `party_id` in terms of the log odds is the following:
...
> *The odds an individual who has no political affiliation votes sporadically versus rarely/never are `r round(exp(-0.677), 3)`* $(e^{-0.872})$ *times the odds of an individual whose party affiliation is Democrat, holding age constant.*

> *The odds an individual whose political affiliation is Republican votes sporadically versus rarely/never are `r round(exp(-0.106), 3)`* $(e^{-0.106})$ *times the odds of an individual whose party affiliation is Democrat, holding age constant.*
...
$$\hat{\pi}_B = \frac{e^{\beta_{0B} + \beta_{1B}X_1 + \beta_{2B}X_2 + \dots + \beta_{pB}X_p}}{1 + \sum_{k \in (B,C)}e^{\beta_{0k} + \beta_{1k}X_1 + \beta_{2k}X_2 + \dots + \beta_{pk}X_p}}$$ {#eq-multinom-pred-prob}

<!--# do i want to do the subscript i?-->

The numerator of @eq-multinom-pred-prob is the odds of an observation being in category $B$ versus the baseline $A$. The denominator is the sum of the odds for every possible level. In this example, it is the sum of the odds an observation is in category $A$ versus $A$ (that's the "1" in the denominator), the odds an observation is in category $B$ versus $A$, and the odds an observation is in category $C$ versus $A$. A similar formula is used to compute $\hat{p}_c$, the predicted probability of being in category $C$. The difference is the numerator, which would be the odds of being in category $C$ versus $A$.

The predicted probability an observation is in the baseline category $A$ is computed as

$$\hat{\pi}_A = 1 - (\hat{\pi}_B + \hat{\pi}_C)$$ {#eq-multinom-pred-baseline}

@eq-multinom-pred-baseline utilizes the fact that the sum of the probabilities across all possible categories equals 1. This is also why we need all $K - 1$ equations as part of the multinomial logistic when there are $K$ categories of the response variable.
...
- the misclassification rate[^13-special-topics-3]

There are `r nrow(voters)` observations in the data.
:::

[^13-special-topics-3]: The accuracy is `r (563 + 2219 + 94)/nrow(voters)` $((563 + 2219 + 94)/5836)$. The misclassification rate is `r 1 - (563 + 2219 + 94) / nrow(voters)` $(1 - 0.493)$.
...
The syntax for the confusion matrix, ROC curves, and AUC are similar to that for logistic regression in @sec-logistic-prediction-R.

The confusion matrix is produced using `conf_mat()` from the **yardstick** package [@yardstick]. We use `autoplot()` to format the confusion matrix such that the cells are shaded by the number of observations.
...
Lastly, the AUC is computed using `roc_auc()` from the **yardstick** R package. The weighted average AUC is computed using the argument `estimator = "macro_weighted"`. The unweighted average AUC is computed by default.
...
Multinomial logistic regression is part of a larger class of models called *generalized linear models*. Linear regression and logistic regression are also examples of generalized linear models. These models have been widely studied; in fact, many statistics programs have a full course dedicated to generalized linear models. For readers interested in a more in-depth introduction to multinomial logistic regression and generalized linear models, see *Data Analysis Using Regression and Multilevel/Hierarchical Models* [@gelman2007data] and *Beyond Multiple Linear Regression* [@roback2021beyond]. We refer readers interested in an introduction to the underlying theory alongside application to *An Introduction to Generalized Linear Models* [@dobson2018introduction].

## Multilevel models {#sec-random-intercepts}

One of the key assumptions we've made throughout this book is that the observations in the sample data are independent of each other. Often in practice, however, there is an underlying grouping structure in the data, and the observations are not completely independent of one another. For example, the data may contain multiple measurements from each individual (called **repeated measures**) as in @fig-repeated-measures.
...
- `taxon` : Code made as a combination of the lemur's genus and species. Note that the genus is a broader categorization that includes lemurs from multiple species. <!--# is this note helpful or confusing?--> This analysis focuses on the following taxa:
...
$$\begin{aligned}
y_{ij} = \beta_0 &+ \beta_1x_{i1} + \dots \beta_qx_{iq} \\
&+ \beta_{q+1}x_{ij(q+1)} + \dots + \beta_px_{ijp} \\ 
&+ u_i + \epsilon_{ij} \\
&u_i \sim N(0, \sigma^2_u) \hspace{5mm} \epsilon_{ij} \sim (0,\sigma^2)
\end{aligned}$$

where $\beta_0$ is the global intercept, $\beta_1, \ldots, \beta_q$ are the coefficients associated fixed effects at the second level of the data, $\beta_{q+1}, \dots, \beta_p$ are the coefficients associated with the fixed effects at the first level of the data, $u_i$ is the random effect on the intercept, and $\epsilon_{ij}$ is the error term. Let's look at the model for the lemurs analysis and break down these components further.
...
The coefficients $\beta_0, \beta_1, \ldots, \beta_5$ and the variance components $\sigma_u$ and $\sigma$ are estimated using maximum likelihood estimation. The output for the lemurs random intercepts model is in @tbl-lemurs-model-random-int.
...
broom.mixed::tidy(lemurs_fit) |>
  kable(digits = 3, format.args = list(nsmall = 3))
```
...
- Explain what $\hat{\sigma}_u = 311.504$ means in the context of the data.[^13-special-topics-5]
:::

[^13-special-topics-5]: `age:` As a lemur gets one month older, its weight is expected to increase by 147.719 grams, on average, after adjusting for taxon, sex, and litter size.
...
The results can be displayed in a tidy format using `tidy()` in the **broom.mixed** R package [@broom.mixed]. The results below are neatly displayed using `kable()`.
...
> - They can be more easily represented visually, making it easier for non-experts to understand them;
...
$$SSR = \sum_{i = 1}^n(y_i - \hat{y}_i)^2$$ {#eq-tree-ssr}
...
::: {.yourturn latex=""}
What is the predicted expenditure for a public institution in the West region with enrollment of 20,000 students.[^13-special-topics-6]
:::

[^13-special-topics-6]: The predicted expenditure is 2.1 million. Note that the fact that it was a public institution was not used based on its path in the decision tree.
...
$$\text{Gini index} = 1 - \sum_{k = 1}^K p_{mk}^2$$ {#eq-gini-index}

where $p_{mk}$ is the proportion of observations in the $m^{th}$ node that belong to class $k$. Similar to SSR the model selects the variable and cutoff that minimizes the Gini index at each step. Low values of the Gini index indicate a large proportion of observations in a given node are from the same class.
...
<!--# include something here where the propensity scores are included in the final outcome model? This gets into ideas of doubly robust estimation.-->

<!--# paper for reference on doubly robust: https://pubmed.ncbi.nlm.nih.gov/16401269/-->
...
$$\text{GPA} = \beta_0 + \beta_1 ~\text{Treatment} + \epsilon \hspace{8mm} \epsilon \sim N(0, \sigma^2)$$ {#eq-treatment-model} For the Project ACE analysis, the response variable GPA is quantitative, so we use a linear regression model of the form in @eq-treatment-model. The fitted model is in @tbl-model-treatment-effect.
...
::: {.yourturn latex=""}
Interpret the 95% confidence interval for the effect of participating in Project ACE in the context of the data.[^13-special-topics-8]
:::

[^13-special-topics-8]: We are 95% confident that improvement in GPA from participating in Project ACE is between 0.017 and 0.401 points.
