...
- 
  ...
  - Fill in missing data using neighboring observations that are similar to the observation with missing data.

    - Hot deck imputation - use observation in the same data set

    - Cold deck imputation - use observation from a different data set (for example, if you are analyzing data from 2025, find a "neighbor" observation from a data set collected in previous year)

    - Can still have under estimated standard deviation for the variable with missingness, but generally the regression coefficients and subsequent inference is fairly accurate

      - Potential to expand effects of any measurement error in the observations used for imputation

  - As one can imagine, biases can result from imputation methods, particularly if the missingness is missing at random or missing not at random

- Logical rule - perhaps there is some logic that could be applied to filling in the missing data.

  - Find exmaple from the recipes data.

- Multiple imputation

  - create multiple data sets using differet imputation methods, then average the analysis results.

- Call out box about variables that have a lot of missing values.

### Imputation in R

- Add something about skimr tog et a quick view (or call back if this is introduced in chapter 3)
...
mean_ratings <- mean(recipes$avg_rating, na.rm = TRUE)

# impute missing values with mean

recipes <- recipes |>
  mutate(reviews_imputed = if_else(is.na(reviews), mean_reviews, reviews), 
         avg_rating_imputed = if_else(is.na(avg_rating), mean_ratings, avg_rating))

# look at distribution of original vs. imputed

ggplot(data = recipes) + 
  geom_density(aes(x = reviews), color = "red") +
  geom_density(aes(x =reviews_imputed), color = "blue")
```
