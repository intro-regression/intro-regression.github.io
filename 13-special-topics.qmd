...
- Introduce data set (maybe go back to recipes data set from Chapter 30).

- How does missingness appear in the data set.

  - Showing example - some columns of data.
...
- Chef John's Nashville Hot Chicken - complete case

- Brazilian Quentao - missing nutrition information. Upon further reserach, find that this is a type of mulled wine, so it is logical that these values would be missing.

- Chicken Adobo Tacos - Missing a lot of data so, perhaps this observation is not useable

- Rose Water Turkish Delight - missing 1 of the nutrition variables, so a type of imputation could be helpful.

- Danish Butter Cookies - missing data about reviews and ratings which is the primary response of interest.

- Generally NAs

- Sometimes in survey data, missignness can be non-response. Use data from Chapter 11 as an example of this.
