...
In @sec-ch-eda, we presented exploratory data analysis, one of the first steps in the data science workflow <!--# reference the chapter 1 diagram?--> One of the main purposes of exploratory data analysis is to understand the distributions of the variables in the data set, and as part of that identify special features in the data such as outliers or missing data before moving on to modeling. We introduced strategies for dealing with outliers in \[ insert chapter references\], and in this section we will discuss some strategies for dealing with missing data.
...
In @sec-ch-eda, we introduced a data set containing details about the nutrition, preparation and cooking time, and the reviews and ratings for recipes on the Allrecipes.com data set. As we conducted exploratory data analysis, we observed that some observations had missing values for one or more variables we were interested in analyzing. Here, we will look at the different types of missingness in the data and introduce some basic strategies for handling missing data.

The data was adapted from the `cuisines` data frame in the **tastyR** R package [@tastyR]. We will primarily focus on the nutrition and ratings information in this section. The variable definitions are below.
...
We begin by observing how missingness appears in the data set. @tbl-missing-example shows examples of five recipes from the `recipes` data frame, each of which have different types and levels of missingness. For the sake of illustration, we will only consider whether there is missingness for the subset of variables analyzed in this section, not the entire `recipes` data set.
...
Let's take a look at this subset of recipes and what we observe about the missingness in each.

Chef John's Nashville Hot Chicken is an example of acomplete case. A **complete case** is an observation that has values for all variables in the analysis. These types of observations have no missing values, so they are ready for analysis without applying the methods in this section.

Rose Water Turkish Delight only has a missing value for the variable `fat` but has data for all other variables. We impute the missing value by filling in the missing data using observed values of `fat` from other recipes. We discuss imputation strategies in @sec-missing-data-strategies.

The Brazilian Quentao recipe has missing values for all the variables related to nutrition information (`calories`, `fat`, `carbs`, and `protein`). This is a recipe for mulled wine, a type of drink; therefore, the missingness is logical in this case. These nutrition variables are most relevant to food dishes, so it is not surprising that such information is not available for this drink recipe. We can consider if there are logical values to impute for this missing data or exclude drink recipes from the analysis. We discuss these options further in @sec-missing-data-strategies.

The Danish Butter Cookies is missing data related to the user reviews (`avg_rating`, `total_ratings`, `reviews`). Upon looking at the webpage for this recipe, we observe that this recipe has no reviews. Thus, we could logically impute 0 for the `total_ratings` and `reviews`. It is less clear, however, how to impute the missing data for `avg_rating`. Depending on the analysis goals, we might narrow the scope of the analysis to recipes that have at least some minimum number of ratings or reviews.

Lastly, the Chicken Adobo Tacos recipe has missing values for all variables of interest. This observation is not currently usable for the analysis, because it does not have any observed values that can inform the data exploration and modeling. We could go to the recipe's page on Allrecipes.com and manually impute any values we see on the page now that weren't available when the data were originally collected. Depending the number of observations in the data that have such volume of missingness, this type of manual imputation could be unfeasible in practice.
...
<!--# could this be combined with the previous section-->

In @sec-missing-data-identify, we observed how missingness can appear in the data and provided some preliminary strategies for dealing with the missingness depending on what we knew about the observation. Now instead of looking at individual observations, let's look at the patterns of missingness within an individual variable. There are three patterns of missingness for a given variable: missing completely at random, missing at random, and missing not at random.

A variable is **missing completely at random (MCAR)** if the pattern of missingness is completely random. This means there is no explainable pattern to the missingness. A variable is **missing at random (MAR)** if the pattern of missingness depends on other variables, but not the variable itself. For example, in the `recipes` data, missingness for the `avg_rating` variable is due to a recipe having no ratings (`total_ratings = 0`), not due to something about a given average rating for the recipe. The last type of missing is **missing not at random (MNAR).** A variable is MNAR if the pattern of missingness depends on the variable itself. For example, in the recipes data `total_ratings` is MNAR, because a missing value for this variable means a recipe has 0 ratings.

It is important to understand the pattern of missingness, because it helps us understand the potential impact of the strategy we use to deal with the missingness in terms of potential bias to the analysis. We'll briefly discuss these impacts as we introduce strategies for handling missing data in @sec-missing-data-strategies

### Strategies for handling missing data {#sec-missing-data-strategies}

Now that we understand how to identify missingness in the data and patterns of missingness, let's discuss some strategies for handling missing values. We introduce some basic strategies here and direct the reader to the texts in @sec-missing-data-further-reading for a more in-depth discussion about missing data.

#### Complete case analysis

Perhaps the easiest way to deal with missingness is to do a **complete case analysis** in which we remove observations that have any missing values for the variables of interest.[^13-special-topics-1] This is the approach used in many R functions. For example, when creating visualizations using **ggplot2** [@ggplot2], the visualization only includes observations that have observed values for all the variables in the visualization. There is a warning produced that indicates how many observations were excluded from the visualization. The regression functions `lm()` and `glm()` also excluded observations that have missing values for the response variable or any of the predictor variables. Unlike **ggplot2**, however, there is no warning produced. Therefore, it is imperative to conduct exploratory data analysis to identify and handle the missingness before any modeling.
...
Complete case analysis provides a good example of the importance of understanding the pattern of missingness. If the missingness is MCAR, then we could reasonably assume removing observations with missingness does not systematically bias the data since the missingness is random. If the missingness is MAR or MNAR, however, simply removing observations with missingness could potentially bias the data. For example, if we removed observations with missing values for `avg_rating` from an analysis of the `recipes` data, we would be systematically excluding recipes with no ratings. These could be newer recipes that haven't been on the site for long or recipes that are generally not popular. It could be fine to narrow the scope of the analysis to exclude new and unpopular recipes, but it is important that we are aware of this limitation and clearly communicate it as we interpret results from the analysis.

#### Imputation

In general, the alternative to complete case analysis is **imputation**, a systematic approach for filling in the missing values. By filling in the missing values, we are able to retain all the observations for the analysis and thus avoid some of the potential biases from complete case analysis. In @sec-missing-data-identify, we introduced one imputation method for the missing values of `total_ratings`. We proposed imputing the missing values using a logical rule <!--# does this term need to be cited--> that recipes that had missing data for total ratings, in fact had no ratings; thus, we can impute missing values of `total_ratings` with `0`. Here we will introduce some additional imputation strategies and direct the reader to the texts in @sec-missing-data-further-reading for more advanced imputation methods.

If we are imputing missing values for a cateogrical predictor, we can fill in the missing values with `Missing` or some other label that indicates the values are missing. By doing this, we create a new level of the categorical predictor that identifies the missing observations. This is similar to the `No Response` or `Refused to respond` level on surveys to denote questions in which the paritcipate did not provide a response. If we use the variable as a predictor in a regression model, an indicator variable is created for the new `Missing` level, in addition to the other existing levels (with the exception of the baseline level). If the indicator for `Missing` is statistically significant, then we may conclude that the observations with missing data for that observation are statistically different from the baseline category. This could indicate the missingness is not completely random, and in fact, represents a subset of the population. <!--# make sure this is correct.-->

There is a vast array of methods for imputing missing values for quantitative variables. One commonly used approach is **mean imputation**, in which the missing values are imputed using the mean value computed from the observed values of that variable. The primary advantages to this method are that it is easy to execute and easy to understand and explain to others. The primary disadvantage is that imputing all the missing values using the mean can result in a distorted <!--# need different word so it's not the same--> distribution of the variable, in which an out-sized proportion of the distribution is concentrated at the mean value. If this is the case, we can underestimate the variability in the variable and thus potentially compute inaccurate regression coefficients and inferential statistics if the imputed variable is used as a predictor or response variable in a regression model. Therefore, we should interpret results with caution if a large proportion of observations were imputed using the mean.

A more informed approach for imputing missing values is by using **nearest neighbor imputation**, in which the missing values are imputed using values from observations that look similar to the one with missing data. This can be done using other observations within the data set, called **hot deck imputation**, or by using other observations in an external data set, called **cold deck imputation**. By using the more informed approach to filling in missing data, we reduce some of the limitations of imputation that occur with mean imputation. There is still the potential to underestimate the variability of the imputed variable, but there is generally little impact to the regression coefficients and inferential statistics [@gelman2007data].

These and any imputation methods can potentially result in biased data that does not accurately reflect the underlying population. A strategy to mitigate the effects of these biases is **multiple imputation** [@rubin1978multiple; @rubin1987multiple] in which multiple data sets are created using different imputation methods, then the results are averaged across the imputed data sets. In @sec-missing-data-r, we show code for conducting multiple imputation in R We point the reader to the texts in @sec-missing-data-further-reading for more details about multiple imputation. <!--# do i need to cite something?-->
...
The first is `summary()`, a base R function. To get a summary of all columns of a data frame, we input the name of the data frame in the function, e.g., `summary(recipes)`. We can also specify which columns to include in the summary, as shown below.
...
Another option for variable summaries is `skim()` from the **skimr** R package [@skimr-2]. Similar to `summary()`, we can put the name of the data frame in the `skim()` function to produce a summary for every column. Otherwise, we can the variables to include in the data summary.

```{r}

recipes |>
  skim(name, calories, fat, carbs, protein,
       avg_rating, total_ratings, reviews)
```

We can "manually" impute missing values using a logical rule or mean imputation utilizing `if_else()` from the **dplyr R package** [@dplyr]. Below is code to use mean imputation for the missing values of `reviews`.
...
The summary statistics for `reviews_imputed` with missing data handled using mean imputation are below.
...
Hot deck imputation can be done using `hotdeck()` from the **VIM** R package[@VIM]Observations are arranged best of one to three specified ordering variables, the missing values are imputed based on the values from "nearest neighbor" observations. Below is the code to impute the missing values for `calories` using hot deck imputation, where the nearest neighbors are defined based on the other nutrition variables: `fat`, `carbs`, and `protein`.

```{r}
recipes <- hotdeck(recipes, variable = "calories",  ord_var = c("fat", "carbs", "protein"))
```

This function imputes the missing values and creates a new logical variable with the suffix `_imp` that is `TRUE` if the value was imputed and `FALSE` otherwise. The summary statistics for the variable `calories` with the missing values imputed are below.
...
We can conduct multiple mutation using the `mice()` in the **mice** package[@mice]. In the code below, we input the data frame that contains missing values and the number of complete data sets we want to generate to compute the final results. Note that the default value is `m = 5`. We save the results as a new data frame.

```{r}
recipes_subset <- recipes |> 
  select(name, calories, fat, carbs, protein, avg_rating, total_ratings, reviews)

recipes_mice <- mice(recipes_subset, m = 5)
```

We use `complete()` to view the completed data frames. In the code below, we view the first complete data frame.

```{r}
recipes_mice_1 <- complete(recipes_mice, 1)
...
Use `ggmice()` from the **ggmice** package [@ggmice] to visualize the distribution of the imputed values for each new data frame created by `mice()`. In the code below, we view the distribution of `reviews` for the observed data and each imputed data set. The imputed data sets are defined by the variable `.imp`.
...
Since our ultimately analysis goal is modeling, we can evaluate whether the imputed data has a similar relationship between the response and predictor variable. For example, if we wish to use the data to explain variability in the average rating, we can look at the relationship.

First we look at the relationship between the number of reviews and average ratings using `ggmice()`. The function will automatically identify the observed versus imputed observations.

```{r}
#| label: fig-ggmice-reviews-v-rating
#| fig-cap: "Relationship betwen `reviews` and `avg_rating` for observed and imputed data"
...
The plot in @fig-ggmice-reviews-v-rating puts the values from all imputed data frames on the sample plot, so a given imputed observation is represented multiple times on the plot. If we wish to look at the relationship between two variables separately for the observed data and each imputed data set, we can facet by the imputation number (`.imp`).
...
We have just provided a brief introduction to handling missing data, but there is a wide variety of research and strategies for dealing with such data in an analysis. To learn more more about strategies for handling missingness and the analysis implications, see @little2019statistical and chapter 25 of @gelman2007data.
