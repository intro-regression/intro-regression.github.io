...
&+ u_i + \epsilon_{ij} \\
&u_i \sim N(0, \sigma^2_u) \hspace{5mm} \epsilon_{ij} \sim (0,\sigma^2_{\epsilon})
\end{aligned}$$
...
$$\text{GPA} = \beta_0 + \beta_1 ~\text{Treatment} + \epsilon \hspace{8mm} \epsilon \sim N(0, \sigma^2_{\epsilon})$$ {#eq-treatment-model} For the Project ACE analysis, the response variable GPA is quantitative, so we use a linear regression model of the form in @eq-treatment-model. The fitted model is in @tbl-model-treatment-effect.
