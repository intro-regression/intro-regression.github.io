...
- 
  ...
  - Create missing indicator for categorical variable

    - enables you to analyze whether the observations with missing values have a different relationship with the response variable than other variables in the model.

  - Mean / median imputation for a quantitative variable

    - Impute missing values with the mean (or median) of that variable among the remaining observations

    - This method is nice, because it is very easy to execute and understand

    - Disadvantages

      - Can distort the distribution of the variable <!--# need different phrasing, this phrasing is used in gelman and hill-->, since there are an artificially high proportion of observations at the mean, thus underestimating the standard deviation of that variable.

      - can distort relationship between the predictor and response and result in inaccurate regression coefficient estimates and inference results

  - Nearest neighbor imputation

    - Fill in missing data using neighboring observations that are similar to the observation with missing data.

      - Hot deck imputation - use observation in the same data set (paper: <https://pmc.ncbi.nlm.nih.gov/articles/PMC3130338/>)
