...
Let's take a look at each example and what we observe about the missingness.
...
Lastly, the Chicken Adobo recipe has missing values for all variables of interest. This observation is not currently usable for the analysis, because it does not have any observed values that can inform the data exploration and modeling. We could go to the recipe's page on Allrecipes.com and manually impute any values we see on the page now that weren't available when the data were originally collected. Depending the number of observations in the data that have such volume of missingness, this type of manual imputation could be unfeasible in practice.

### Types of missigness

- Missingness may occur in data in multiple ways for a given observation

  - Item non-response in an observation has missing values for a subset of the variables

  - Unit non-response in which an observation has no available data, or not enough to be useful.

    - Can use the penguins data set for an example of this.
