...
See the [@palmerpenguins] for the full codebook.
...
A commonly used analogy for installing versus loading packages is that installing packages is like installing a light bulb in a room, and loading packages is like flipping a light switch.

We only need to install packages once (like light bulbs), but we load packages each time we start a new instance of R (like flipping the light switch each time we enter the room). Therefore, `install.packages()` is run in the console and `library()` is included at the top of the R script or Quarto document (see @sec-quarto).
:::
...
2.  *Each observation forms a row.*
    - Yes, the observations in this data set are individual penguins. The information for one penguin is across a single row. There are no instances of multiple penguins represented on a single row or a single penguin represented across multiple rows.
3.  *Each type of observational unit forms a table.*
...
The data sets in the tidyverse are stored as tibbles. **Tibbles** are like data frames in R with some differences in the underlying features. As @wickham2017r puts it, a tibble is a data frame that modifies "some older behaviours to make life a little easier". One nice feature of tibbles is that they show summary information about the data set overall and for each variable. For example, in @tbl-penguins we see there are `r nrow(penguins)` rows (observations) and `r ncol(penguins)` columns (variables). We also see the type for each column, e.g. `species` is stored as a `<fct>` (factor). We talk more about data types in @sec-initial-check.

### **The pipe**

One benefit of the **tidyverse** syntax is its readability. Though it can be more verbose than other coding styles, its naming convention and structure make it easy <!--# better word here--> to read and understand. A key part of the readability is due to the pipe. The **pipe**, coded as `|>`, is used to pass data from one function to the next. If we think of reading code aloud in English, we say "and then" when we see a pipe [@cetinkaya2021educator].
...
```{r}
# override the theme and set back to default for this chapter
...
#| fig-cap: Base layer for a ggplot
ggplot(data = penguins)
```

The next part of the code is used to set the **aes**thetics that indicate how the variables are mapped onto features of the plot. They are defined in in the `aes()` function as part of the code `mapping = aes()`. Often, we do not see the argument name `mapping =` in the code, but it is the second argument of `ggplot()`.
...
#| fig-cap: Variables on each axis defined
...
#| fig-cap: "Plot type (`geom`) specified"
...
#| fig-cap: "Example geoms for a single quantitative variable"
#| fig-subcap: 
#|   - "Histogram"
#|   - "Density plot"
#| layout-ncol: 2
...
```{r}
#| label: fig-boxplot
#| fig-cap: Side-by-side boxplot to visualize relationship between a quantitative and categorical variable

ggplot(data = penguins, aes(x = species, y = bill_length_mm)) + 
  geom_boxplot()
```

These are a few of the many geoms available in **ggplot2**. A full list is available at [https://ggplot2.tidyverse.org](#0).
...
#| fig-cap: Scatterplot with a title and axis labels
...
There are a variety of aesthetics, visual features, that can be customized in **ggplot2**. Some commonly used aesthetics include `color`, `shape`, `size`, `alpha` (transparency), `fill` (shade in area) and `linetype`. The same aesthetics can be applied across the entire plot, or aesthetics can be applied based on values of a variable.

We often apply the same aesthetic across an entire plot to make it more visually appealing or easier to read. This type of aesthetic change does not add any additional information to the visualization. For example, the code below changes the color of all points on the plot by adding the `color =` argument in `geom_point()`. The colors can be defined based on a list of over 600 colors[^02-review-r-8] or HTML hex color codes.

[^02-review-r-8]: Type `colors()` in R to see the full list of built-in colors.

```{r}
#| label: fig-ggplot-color-points
#| fig-cap: Scatterplot with colored points
...
The second way to apply aesthetics is to map them to specific variables. This makes the visualization engaging and adds additional information about the data. This is useful in data analysis when we want to make comparisons across subgroups of data.
...
#| fig-cap: "Scatterplot with color by `species`"
...
The argument `values = pnw_palette("Bay", 3)` indicates that the colors are mapped to the *Bay* color palette using their colors (one for each species).\

```{r}
#| label: fig-color-palette
#| fig-cap: "Scatterplot using Bay color palette from  **PNWColors** package" 
...
We may also wish to create a color palette. For example, here we apply the palette `my_color_palette` that is defined using HTML hex codes.[^02-review-r-11]

[^02-review-r-11]: This color palette is inspired by the examples in @palmerpenguins.

```{r}
#| label: fig-custom-color-palette
#| fig-cap: Scatterplot using user-defined color palette

my_color_palette <- c("#F87666", "#58B09C", "#7272AB")
...
![RStudio](images/02-quarto-render-rstudio.jpg){#fig-quarto-render-1}

![Positron](images/02-quarto-render-positron.jpg){#fig-quarto-render-2 width="2310"}

Rendering Quarto document in RStudio and Positron
:::
