...
In @sec-ch-logistic, we introduced data from the 2024 General Social Survey in which adults in the United States were asked their opinons on a variety of issues, including their comfort with driverless cars. In the previous chapter, we fit a logistic regression model and used the model to describe the characteristics associated with the odds an adult is comfortable with driverless cars. We continue with the analysis in this chapter, with a focus on using the model for prediction. We will also evaluate the model performance and show an example workflow for comparing two logistic regression models.
...
& = 1.303
\end{aligned}$$ {#eq-pred-log-odds}

where $\hat{\pi}$ is the predicted probability of being comfortable with driverless cars. Note that the difference between the predicted log odds computed in @eq-pred-log-odds and the predicted log odds in @tbl-aidrive-comfort-predict is due to rounding of the coefficients in @eq-pred-log-odds.
...
Using the predicted log odds from the first row of @tbl-aidrive-comfort-predict, the predicted odds for this individual are

$$\widehat{\text{odds}} = e^{\log \big( \frac{\hat{\pi}}{1-\hat{\pi}}\big)} = e^{1.306} = 3.6 $$

Lastly, we use the odds from @tbl-aidrive-comfort-predict to compute the predicted probability[^12-logistic-prediction-2]:

[^12-logistic-prediction-2]: These values may differ slightly from the values in the the table, because we are computing predictions using rounded coefficients.

$$\hat{\pi} = \frac{\widehat{\text{odds}}}{1 + \widehat{\text{odds}}} = \frac{3.692}{1 + 3.692} = 0.787$$

::: {.math_rules latex=""}
The relationship between the predicted log odds and the predicted probability is more concisely represented by the **sigmoid function** (@eq-sigmoid). The sigmoid function, denoted $\sigma(z)$, is the inverse of the logit function, i.e., the log odds.

$$\sigma(z) = \frac{e^z}{1 + e^z} = \frac{1}{1 + e^{-z}}$$

It produces the "S" curve often seen for the predicted probabilities from a logistic regression model. @fig-sigmoid is an illustration of the sigmoid function.

```{r}
#| label: fig-sigmoid
#| fig-cap: Illustration of the sigmoid function based on simulated data
...
ggplot(data, aes(x = x, y = y)) +
  ...
  labs(x = expression(z), y = expression(sigma(z)))
```
:::

::: {.yourturn latex=""}
Show how to compute the predicted odds and probability for individual #2 in @tbl-aidrive-comfort-predict.[^12-logistic-prediction-3]
:::

[^12-logistic-prediction-3]: **Odds:** $e^{\log(\frac{\hat{\pi}}{1-\hat{\pi}})} = e^{0.953} = 2.593$
...
When using the logistic regression model for classification, we define a threshold, such that an observation is classified as $\hat{Y} = 1$ if the predicted probability is greater than the threshold. Otherwise, the observation is classified as $\hat{Y} = 0$. If we're unsure what threshold to set, we can start with a threshold of 0.5, the default threshold typically used in statistical software. This means if the model predicts an observation is more likely than not to have response $Y = 1$, even if just by a small amount, then the observation is classified as having response $\hat{Y} = 1$.
...
@tbl-aidrive-comfort-pred-class shows the observed value of the response (`aidrive_comfort`), predicted probability, and predicted class for ten respondents. For many of these respondents, the observed and predicted classes are equal. There are some, such as Observation 6, in which the predicted classes differ from the observed. In this instance, the respondent had a combination of `age`, `income`, and `tech_easy`, that are associated with a higher probability of being comfortable with driverless cars; however, the individual responded they are not comfortable with driverless cars in the General Social Survey.
...
$$\text{accuracy} = \frac{349 + 601}{349 + 228 + 343 + 601} = 0.625 $$ {#eq-logistic-accuracy}
...
$$\text{misclassification} = \frac{228 + 343}{349 + 228 + 343 + 601} = 0.375$$ {#eq-logistic-misclassification}
...
$$\text{accuracy} = \frac{\text{True negative} + \text{True positive}}{\text{True negative} + \text{False negative} + \text{False positive} + \text{True positive}}$$ {#eq-accuracy-general}

::: {.yourturn latex=""}
Write the general equation for the misclassification computed in @eq-logistic-misclassification using the terms in @tbl-confusion-matrix-detailed.[^12-logistic-prediction-4]
:::

[^12-logistic-prediction-4]: $$\text{misclassification} = \frac{\text{False negative} + \text{False positive}}{\text{True negative} + \text{False negative} + \text{False positive} + \text{True positive}}$$
...
$$\text{Sensitivity} = \frac{\text{True positive}}{\text{False negative} + \text{True positive}}$$ {#eq-sensitivity}

The **false negative rate** is the proportion of those with observed $y_i = 1$ that were incorrectly classified as $\hat{y}_i = 0$.

$$\text{False negative rate} = \frac{\text{False negative}}{\text{False negative} + \text{True positive}}$$ {#eq-false-negative}
...
$$\text{Specificity} = \frac{\text{True negative}}{\text{True negative} + \text{False positive}}$$ {#eq-specificity}
...
$$\text{False positive rate} = \frac{\text{False positive}}{\text{True negative} + \text{False positive}}$$ {#eq-false-positive}
...
$$\text{Precision} = \frac{\text{True positive}}{\text{False positive} + \text{True positive}}$$ {#eq-precision}
...
- Precision[^12-logistic-prediction-5]
:::

[^12-logistic-prediction-5]: $$\begin{aligned}\text{Sensitivity} &= 601/ (228 + 601) = 0.725 \\ \text{Specificity} &= 349 / (349 + 343) = 0.504 \\\text{Precision} &= 601 / (343 + 601) = 0.637\end{aligned}$$

We have shown how we can derive a lot of useful information from the confusion matrix. As we use the confusion matrix to evaluate how well observations are classified; however, we need to keep in mind that the confusion matrix is determined based on the model predictions [**and**]{.underline} the threshold for classification. For example, in @tbl-confusion-matrix-0.3, we see a confusion matrix for the same model in @tbl-aidrive-comfort-fit using the threshold of 0.3.
...
aidrive_comfort_aug |>
 ...
 mutate(highlight = if_else(specificity == 0.4754335, "1", "0")) |>
  ...
  annotate(
    "segment", color = "steelblue", x = 0, y = 0, xend = 1, yend = 1, linewidth = 2, alpha = 0.6
  ) +
  annotate(
...
::: {.yourturn latex=""}
Compute the sensitivity and specificity from @tbl-aidrive-roc-confmat. You can confirm your result using the ROC curve in @fig-aidrive-comfort-roc-curve .[^12-logistic-prediction-6]
:::

[^12-logistic-prediction-6]: $\text{Sensitivity } = \frac{622}{207 + 622} = 0.75$ and $\text{Specificity } = \frac{329}{329 + 363} = 0.475$
...
In terms of the ROC curve in @fig-aidrive-comfort-roc-curve, this point is at a false positive rate of `r round((1 - roc_optimize$specificity), 3)` and true positive rate of `r round(roc_optimize$sensitivity, 3)`, corresponding to a threshold of `r round(roc_optimize$.threshold, 3)`.
...
::: {.yourturn latex=""}
What is a "true positive" in this scenario? What is a "false positive"?[^12-logistic-prediction-7]
:::

[^12-logistic-prediction-7]: A true positive is correctly classifying a patient with the illness as a "1", having the illness. A "false positive" is incorrectly classifying a patient that doesn't have the illness as a "1", having the illness.

When determining the probability threshold for classification, we need to think carefully about the implications of the analysis decision. More specifically, one thing we need to consider is the severity of the treatment the patient will undergo. If the treatment is minimal, then perhaps we might be willing to set a threshold that results in more false positives. If the treatment is very invasive, however, we want to minimize false positives. Otherwise, many patients will undergo an invasive treatment that they do not need. Similarly, we want to consider the implications of lower sensitivity and not diagnosing individuals who actually have the illness.

::: {.yourturn latex=""}
- If the main objective is high sensitivity, do we set a probability threshold closer to 0 or to 1?

- If the main objective is high specificity, do we set a probability threshold closer to 0 or to 1?[^12-logistic-prediction-8]
:::

[^12-logistic-prediction-8]: If the main objective is high sensitivity, we set a low threshold close to 0. If the main objective is high specificity, we set a high threshold close to 1.
...
Similar to linear regression, we can use the Akaike's Information Criterion (AIC) [@akaike1974] and the Bayesian Schwarz's Criterion (BIC) [@schwarz1978estimating] as relative measures for comparing logistic regression models. The equations for AIC and BIC in logistic regression are the same as those for linear regression (@sec-aic-bic).
...
The ROC curve data can be plotted using the `autoplot()` function. The resulting graph is a `ggplot` object, so additional **ggplot2** layers such as `labs` and `annotate` can be applied to the ROC curve.
...
We repeat the process of cross validation for Model 2 that includes the predictors `age`, `tech_easy`, `income_fct`, `polviews_fct`, and `tech_harm`. The performance metrics from cross validation are stored in the object `aidrive_cv_2_metrics`.
...
aidrive_test <- aidrive_test |>
  mutate(pred_prob = predict(aidrive_comfort_final, newdata = aidrive_test,
                             type = "response"))
...
As a final step, we refit the model using all observations in the data. At this point, we are ready to use the model for interpretation, drawing inferential conclusions, and putting into production for prediction and classification.
...
In @sec-ch-special-topics, we conclude with some advanced modeling techniques and special topics for analysis in practice.
