...
::: {.yourturn latex=""}
Show how to compute the predicted log odds, odds, and probability for individual #2 in @tbl-aidrive-comfort-predict.[^12-logistic-prediction-3]
:::

[^12-logistic-prediction-3]: **Log odds:** $\log \Big(\frac{\hat{\pi}}{1-\hat{\pi}}\Big) =  0.032 - 0.016  \times 19 + 0.075   \times 0 +  0.576 \times 0
    - 0.436  \times 0 +  0.264 \times 0 +  0.526  \times 0
...
::: {.yourturn latex=""}
What is a "true positive" in this scenario? What is a "false positive"?[^12-logistic-prediction-6]
:::

[^12-logistic-prediction-6]: A true positive is correctly classifying a patient with the illness as a "1", having the illness. A "false positive" is incorrectly classifying a patient that doesn't have the illness as a "1", having the illness.
...
- If the main objective is high specificity, do we set a probability threshold closer to 0 or to 1?[^12-logistic-prediction-7]
:::

[^12-logistic-prediction-7]: If the main objective is high sensitivity, we set a low threshold close to 0. If the main objective is high specificity, we set a high threshold close to 1.
