...
#| eval: false
#| echo: false
#| label: make-lemurs-sample-lt24mo-dataset
...
lemurs-sample-lt24mo |> write_csv("data/lemurs-sample-lt24mo.csv")
```
...
Three lemur taxa represented in the data. Images from @duke2025meetlemurs.
:::

<!--# check that I can actually use these images from Duke Lemur Website: https://lemur.duke.edu/discover/meet-the-lemurs/ -->

The data are in `lemurs-sample-lt24mo.csv`. This analysis in this chapter focuses on the following variables:
...
#| fig-cap: "Univariate distribution of the weight of young lemurs"
...
#| tbl-cap: "Summary statistics for the weight of young lemurs"
#| echo: false
...
#| fig-cap: "Univariate distributions fo the predictor variables"
#| fig-subcap: 
#|  - "Age (in months)"
#|  - "Taxon"
#|  - "Sex" 
...
#| tbl-cap: "Summary statistics for the quantitative predictor variables"
...
@fig-mlr-bivariate-4 shows the relationship between `litter_size` and `weight`. Random noise (a very small value drawn from a uniform distribution) was added to make the relationship easier to visualization. Because `litter_size` is discrete, the observations would fall into straight vertical lines, making it more challenging to understand the relationship between the two variables. Based on this scatterplot, the relationship between `litter_size` and `weight` appears to be positive and relatively weak.

Now that we have a better understanding of the variables in this analysis, we will use a multiple linear regression model to understand how taxon, litter size, age, and sex explain variability in the weight of lemurs. Before looking at the model specific to this data, we will discuss the details of the multiple linear regression model, in general, and to determine if a multiple linear regression model is appropriate for the data.
...
$$Y = \text{Model} + \text{Error}$$ {#eq-general-model-recap}
...
\end{aligned}$$ {#eq-general-model-mlr}
...
$$Y = \beta_0 + \beta_1X_1 + \beta_2X_2 + \dots + \beta_pX_p + \epsilon, \hspace{8mm} \epsilon \sim N(0, \sigma^2)$$ {#eq-mlr-stat-model-1}

where $\epsilon \sim N(0, \sigma^2)$ indicates the error terms are normally distributed with a mean of 0 and variance $\sigma^2$. @eq-mlr-stat-model-1 is the population-level statistical model for multiple linear regression.

Recall from @sec-slr-specify, the function $f(X) = E(Y|X)$ is the expected value (mean) of $Y$ given a value of the predictor $X$. Extending this to multiple linear regression, $f(\mathbf{X}) = f(X_1, X_2, \ldots, X_p)$ outputs the expected value of $Y$ for a combination of the predictors. Therefore,

$$E(Y|\mathbf{X}) = E(Y|X_1, X_2, \ldots, X_p) = \beta_0 + \beta_1X_1 + \beta_2X_2 + \dots + \beta_pX_p$$ {#eq-mlr-exp-y}
...
$$\hat{y}_i = \hat{E}(y_i|x_{i1},x_{i2}, \ldots, x_{ip}) = \hat{\beta}_0 + \hat{\beta}_1x_{i1} + \hat{\beta}_2x_{i2} + \dots + \hat{\beta}_px_{ip}$$ {#eq-mlr-estimated-model}
...
$$\sum_{i=1}^{n}e_i^2 = \sum_{i=1}^{n}(y_i - \hat{y}_i)^2 = \sum_{i=1}^n(y_i - [\beta_0 + \beta_1x_{i1} + \dots+ \beta_px_{ip}])^2$$ {#eq-mlr-min-sse}
...
$$\mathbf{Y} = \mathbf{X}\boldsymbol{\beta} + \boldsymbol{\epsilon}$$ {#eq-mlr-stat-model-matrix}
...
$$\boldsymbol{\hat{\beta}} = (\mathbf{X}^\mathsf{T}\mathbf{X})^{-1}\mathbf{X}^\mathsf{T} \mathbf{Y}$$ {#eq-mlr-est-beta-matrix}
...
#| tbl-cap: Multiple linear regression model age, sex, taxon, and litter size to explain variability in the weight of young lemurs
#| echo: false
...
The interpretation for coefficients of quantitative predictors is similar to the interpretation in simple linear regression in @sec-slr-interpret-coef. Given a quantitative predictor $x_j$, its model coefficient $\beta_j$ is the slope describing the relationship between $x_j$ and the response variable, *after adjusting for the other predictors in the model*. For example, the coefficient for `age` in @tbl-lemurs-model is `r round(age_coef, 3)`. Let's compare this to the coefficient from a simple linear regression model using age shown in @tbl-lemurs-age-model-slr.

```{r}
#| label: tbl-lemurs-age-model-slr
#| tbl-cap: "Simple linear regression model using age to explain variability in the weight of young lemurs" 
#| echo: false
...
The values of the estimated model coefficients $\hat{\beta}_j$ depend both on the relationship between $x_j$ and $y$ along with the units of $x_j$. Therefore, we cannot use the values of the coefficients to determine which predictor is "most important" in explaining variability in the response variable.

For example, let's refit the model in @tbl-lemurs-model using `age` recorded in years instead of months.

```{r}
#| label: tbl-lemurs-model-age-years
#| tbl-cap: "Model using age recorded in years" 
lemurs <- lemurs |>
  mutate(age_in_years = age / 12)

lemurs_model_age <- lm(weight ~ age_in_years + sex + 
                     taxon + litter_size,
                   data = lemurs)


age_years_coeff <- tidy(lemurs_model_age) |> 
  filter(term == "age_in_years") |>
  pull(estimate)

tidy(lemurs_model_age) |>
  kable(digits = 3)
```

The new variable `age_in_years` is computed as `age/ 12`. Thus the coefficient for `age_in_years` in @tbl-lemurs-model-age-years, $\hat{\beta}{\text{age\_in\_years}} =\hat{\beta}_{\text{age}} * 12$ . The new coefficient `r age_years_coeff` is much larger, because it is the expected change in weight over 12 months instead of the expected weight over one month. It does not mean the effect of age is stronger when recorded in years versus months.

Thus, it would be misleading to use values of the model coefficients to determine which variable is the most important. In @sec-standardize-predictors, we introduce a model using standardized predictors that can be used to make claims about variable importance.
:::
...
As the data scientists, we can determine which level we would like to be the baseline level if neither of these approaches align well with the data. To do so, use functions, such as `fct_relevel()` in R, to impose an ordering with the desired baseline as the first level.
:::
...
- `litter_size = 0`: There were 0 lemurs in the litter size in which the lemurs born. (Note that this is not possible in the data set).
...
In the previous section, we determined that it is not possible for the litter size to be 0, because the litter size includes the observed lemur. Therefore, we can use centering to create a new variable `litter_size_cent` by shifting `litter_size` by 1, so that `litter_size_cent = 0` means `litter_size = 1` . Though it is not always the case, the newly centered variable `litter_size_cent` has a new meaningful interpretation. It is the number of siblings in the litter. Putting this together, `litter_size_cent` = 0 means the lemur was born into a litter with no siblings.
...
```{r}
#| label: fig-centering
#| fig-cap: "Original versus centered values of litter size"
#| fig-subcap: 
...
```{r}
#| label: tbl-centering
#| tbl-cap: "Summary statistics for litter size and number of siblings"
#| echo: false
...
From @fig-centering and @tbl-centering, we see that the only statistics that have changed are those about location (e.g., $Q1$, $Q3$, min, max) and those related to the center (mean and median). The shape of the distribution and statistics regarding the spread (standard deviation and IQR) have remained the same after centering. By centering the variable we have essentially moved the distribution along the number line by some constant amount.
...
#| fig-subcap: 
#| - "weight versus litter size"
#| - "weight versus number of siblings"
#| echo: false
#| layout-ncol: 2
...
#| tbl-cap: "Summary statistics for age and mean-centered age"
...
#| tbl-cap: Model of weight using centered quantitative predictors
...
#| tbl-cap: Model coefficients for original model (@tbl-lemurs-model) and model with centered quantitative predictors (@tbl-lemurs-model-centered)
#| echo: false
...
### Standardized quantitative predictors {#sec-standardize-predictors}

Thus far, we have interpreted the coefficients to understand the relationship between the response and each predictor variable. Sometimes, however, the primary objective of the analysis may be less focused on interpretation and more focused on identifying which predictors are most important in understanding variability in the response variable. In @sec-mlr-interpret-quantitative, we showed why conclusions about variable importance can't be made when the predictors have different units. Thus, if our goal is identifying which predictors are most important, we can fit a model using standardized values of the quantitative predictors.
...
where $\bar{X}$ is the mean of $X$ and $s_{X}$ is the standard deviation of $X$. In addition to shifting the center of the new distribution to be equal to 0, the standardized variable is rescaled such the standard deviation (and variance) of the standardized variable is 1. This is similar to computing a $z$-score from the normal distribution.
...
#| tbl-cap: Model coefficients for original model (@tbl-lemurs-model),  model with centered quantitiatve predictors (@tbl-lemurs-model-centered), and model with standardized quantitative predictors (@tbl-lemurs-model-std)
#| echo: false
...
#| tbl-cap: Model that includes interaction between age and taxon 
#| echo: false
...
Another type of interaction effect is between two categorical predictors. Let's consider the interaction between `taxon` and `sex` for this data. We might include this interaction effect if we want to explore whether the relationship between sex and weight differs by taxon.[^07-mlr-12]

[^07-mlr-12]: The interaction between sex and taxon would also tell us if the relationship between taxon and weight differs by sex. We have decided to interpret the interaction effect in the other direction given it is more practically meaningful to understand differences between sexes within each taxon versus the other way around.

```{r}
#| label: fig-lemurs-taxon-sex-int
#| fig-cap: "Relationship between weight and sex, faceted by taxon"
#| echo: false
...
#| tbl-cap: Model including interaction between sex and taxon
#| echo: false
...
#| fig-cap: "Relationship between weight versus litter size by age"
#| echo: false
...
Because we are specifically interested in interpreting how the effect of litter size changes as age changes, we have created bins to mark off age ranges and looked at the scatterplot of litter size and weight for each bin.
...
#| tbl-cap: Model including interaction between age and litter size
#| echo: false
...
The `tidy` function structures the model output into a data frame, and `kable()` is used to display the results as a neatly formatted table.

### Prediction

We use `predict()` to compute predicted values from multiple linear regression models. We begin by making a tibble that contains the values of the new observation. The variable names in the tibble of new observations must match the column names in the original data exactly, including letter case. Additionally, the observed values of the categorical predictors are put in the tibble, not indicator variables. As with variable names, the values of the categorical predictors must be input exactly as they show in the data set (including case).
