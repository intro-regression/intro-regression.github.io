...
The new variable `age_in_years` is computed as `age/ 12`. Thus the coefficient for `age_in_years` in @tbl-lemurs-model-age, $\hat{\beta}{\text{age_in_years}} =\hat{\beta}_{\text{age}} * 12$ . The new coefficient `r age_years_coeff` is much larger, because it is the expected change in weight over 12 months instead of the expected weight over one month. It does not mean the effect of age is stronger when recorded in years versus months.
