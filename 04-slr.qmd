...
$$Y = \beta_0 + \beta_1 X + \epsilon \hspace{8mm} \epsilon \sim N(0, \sigma^2_{\epsilon})$$ {#eq-slr}
...
$$y_i = \beta_0 + \beta_1 x_i + \epsilon_i \hspace{7mm} \epsilon_i \sim N(0, \sigma_{\epsilon}^2)$$ {#eq-slr-complete}
...
$$\text{audience\_score} = \beta_0 + \beta_1~\text{critics\_score} + \epsilon, \hspace{5mm}\epsilon  \sim N(0, \sigma^2_{\epsilon})$$ {#eq-movies-stat-model}
...
::: {.yourturn latex=""}
The RMSE for the movie scores model is `r round(rmse,3)`. The values of audience score in the data range from `r min(movie_scores$audience_score)` to `r max(movie_scores$audience_score)`. What is your evaluation of the model fit based on RMSE? Explain your response.[^04-slr-11]
:::

[^04-slr-11]: **Example answer:** An error of 12.452 is about a 17% error based on the range of the audience scores, `r min(movie_scores$audience_score)` to `r max(movie_scores$audience_score)`. This error could seem relatively large, depending on how the model will be used to inform decisions in practice.
