...
$$SSR = \sum\limits_{i=1}^{n}[y_i - \hat{y}_i]^2 = [y_i - (\beta_0 + \beta_1 x_i)]^2 = [y_i - \beta_0 - \beta_1 x_i]^2$$ {#eq-ssr}
...
\end{aligned}$$ {#eq-par-deriv-estimators}
...
From @eq-est-beta0, we know the $\hat{\beta}_0$ that satisfies the first equality in @eq-par-deriv-estimators is

$$\hat{\beta}_0 = \bar{y} - \hat{\beta}_1\bar{x}$$ {#eq-beta0}

The formula for $\hat{\beta}_0$ contains $\hat{\beta}_1$, so now let's find the value of $\hat{\beta}_1$ that satisfies the second equality in @eq-par-deriv-estimators. The mathematical steps are below in @eq-beta1-pt1.
...
(\text{Given }\bar{x} = \frac{1}{n}\sum_{i=1}^n x_i)&\Rightarrow \sum\limits_{i=1}^{n}x_iy_i  = n\bar{y}\bar{x} - \hat{\beta}_1n\bar{x}^2 + \hat{\beta}_1\sum\limits_{i=1}^{n}x_i^2\\[10pt]
...
\end{aligned}$$ {#eq-beta1-pt1}

We will use the following rules to write @eq-beta1-pt1 in a form that is more recognizable:

$$\sum_{i=1}^n x_iy_i - n\bar{y}\bar{x} = \sum_{i=1}^n(x_i - \bar{x})(y_i - \bar{y}) = (n-1)\text{Cov}(x,y)$$ {#eq-cov}

$$\sum_{i=1}^n x_i^2 - n\bar{x}^2 = \sum_{i=1}^n(x_i - \bar{x})^2 = (n-1)s_x^2$$ {#eq-var-x}
...
Plugging in the formula for $\text{Cov}(x,y)$ into @eq-est-beta1-pt2, we have

$$\hat{\beta}_1 = \frac{\text{Cov}(x,y)}{s_x^2} = r\frac{s_ys_x}{s_x^2} = r\frac{s_y}{s_x}$$

We have found values of $\hat{\beta}_0$ and $\hat{\beta}_1$ that satisfy @eq-par-deriv-estimators. Now we need to confirm that we have found the minimum value (rather than a maximum value or saddle point). To do, we use the second partial derivative. If it is positive, then we know we have found a minimum.
...
\end{aligned}$$ {#eq-ssr-second-deriv}
...
$$y_i = \beta_0 + \beta_1 x_{i1} + \dots + \beta_p x_{ip} + \epsilon \hspace{8mm} \epsilon \sim N(0, \sigma^2)$$ {#eq-basic-model}

The regression model in @eq-basic-model can be represented using vectors and matrices.

$$\mathbf{y} = \mathbf{X}\boldsymbol{\beta} + \boldsymbol{\epsilon} \hspace{8mm} \boldsymbol{\epsilon} \sim N(\mathbf{0}, \sigma^2\mathbf{I})$$ {#eq-linear-matrix}
...
\epsilon_n
\end{bmatrix}
}_{\boldsymbol{\epsilon}}$$ {#eq-linear-matrix-expand}
...
As before the error terms are normally distributed, centered at $\mathbf{0}$, a column vector of 0's, with a variance $\sigma^2\mathbf{I}$, where $\mathbf{I}$ is the identity matrix.

::: {.math_rules latex=""}
The variance of the error terms $\boldsymbol{\epsilon}$ is
...
This is the matrix notation showing that the error terms are independent and have the same variance $\sigma^2$.
:::
...
$$\boldsymbol{\epsilon} = \mathbf{y} - \mathbf{X}\boldsymbol{\beta}$$ {#eq-errors-matrix}

As with simple linear regression in @sec-least-sq-math, the least-squares estimator of the vector of coefficients, $\hat{\boldsymbol{\beta}}$, is the vector that minimizes the sum of squares in @eq-sum-sq-matrix.

$$\sum\limits_{i=1}^{n} \epsilon_{i}^2 = \boldsymbol{\epsilon}^\mathsf{T}\boldsymbol{\epsilon} = (\mathbf{y} - \mathbf{X}\boldsymbol{\beta})^\mathsf{T}(\mathbf{y} - \mathbf{X}\boldsymbol{\beta})$$ {#eq-sum-sq-matrix}
...
\end{aligned}$$ {#eq-ssr-matrix-expand}
...
$$SSR = \mathbf{y}^\mathsf{T}\mathbf{y} - 2 \boldsymbol{\beta}^\mathsf{T}\mathbf{X}^\mathsf{T}\mathbf{y} + \boldsymbol{\beta}^\mathsf{T}\mathbf{X}^\mathsf{T}\mathbf{X}\boldsymbol{\beta}$$ {#eq-ssr-matrix-expand-2}
...
This is also denoted as $\frac{\partial f}{\partial \mathbf{x}}$.
...
$$\nabla_\mathbf{x} \hspace{1mm} \mathbf{x}^\mathsf{T}\mathbf{A}\mathbf{x} = (\mathbf{A}\mathbf{x} + \mathbf{A}^\mathsf{T} \mathbf{x}) = (\mathbf{A} + \mathbf{A}^\mathsf{T})\mathbf{x}$$ If $\mathbf{A}$ is symmetric, then $(\mathbf{A} + \mathbf{A}^\mathsf{T})\mathbf{x} = 2\mathbf{A}\mathbf{x}$
:::
...
$$-2\mathbf{X}^\mathsf{T}\mathbf{y} + 2\mathbf{X}^\mathsf{T}\mathbf{X}\hat{\boldsymbol{\beta}} = \mathbf{0}$$ {#eq-ssr-deriv-2}
...
This is also denoted as $\frac{\partial^2f}{\partial \mathbf{x}^2}$
...
$$h_{ii} =  \frac{1}{n} + \frac{(x_i - \bar{x})^2}{\sum_{j=1}^{n}(x_j-\bar{x})^2}$$ {#eq-leverage-slr}
...
$$h_{ii} = \mathbf{x}_i^\mathsf{T}(\mathbf{X}^\mathsf{T}\mathbf{X})^{-1}\mathbf{x}_i$$ {#eq-leverage-mlr}
...
The sum of the leverages for all observations is $p+1$, where $p$ is the number of predictors in the model. Thus, $\text{rank}(\mathbf{H}) = \sum_{i=1}^n h_{ii} = p+1$. The average value of leverage is $\bar{h} = \frac{p+1}{n}$. Observations with leverage greater than $2\bar{h}$ are considered to have high leverage. <!--# am i using "high" or "large"-->

Using $\mathbf{H}$, we can write the residuals as

$$\mathbf{e} = \mathbf{y} - \hat{\mathbf{y}} = \mathbf{y} - \mathbf{Hy} = (\mathbf{I} - \mathbf{H})\mathbf{y}$$ {#eq-residuals-hat}

@eq-residuals-hat shows one feature of observations with high leverage. Observations with high leverage tend to have small residuals. In other words, the model tends to pull towards observations with high leverage.

## Assumptions of linear regression {#sec-assumptions-matrix}

In @sec-slr-foundation, we introduced four assumptions of linear regression. Given the matrix form of the linear regression on model in @eq-linear-matrix, $\mathbf{y} = \mathbf{X}\boldsymbol{\beta} + \boldsymbol{\epsilon}$ such that $\boldsymbol{\epsilon} \sim N(\mathbf{0}, \sigma^2\mathbf{I})$, these assumptions are the following:
...
3.  The variance of $\mathbf{y}$ given $\mathbf{X}$ is $\sigma^2\mathbf{I}$.
...
$$\mathbf{y}|\mathbf{X} \sim N(\mathbf{X}\boldsymbol{\beta}, \sigma^2\mathbf{I})$$ {#eq-dist-y-matrix}

In @sec-mlr-matrix, we showed Assumption 4 from the distribution of the error terms. Here we will show Assumptions 1 - 3.

<!-- # could I move the linear combination property down to the next section where I use it?-->
...
The distribution of the error terms $\boldsymbol{\epsilon}$ is normal, and the vector of responses $\mathbf{y}$ is a linear combination of the error terms. More specifically, $\mathbf{y}$ is computed as the error terms, shifted by $\mathbf{X}\boldsymbol{\beta}$. Thus, from the math property above, we know that $\mathbf{y}$ is normally distributed.
...
Lastly, we show Assumption 3 that $Var(\mathbf{y}) = \sigma^2\mathbf{I}$ in linear regression. Similar to the expected value, we can get the variance from the mathematical property used to show Assumption 1. Here, we show Assumption 3 directly from the properties of variance.
...
$$\hat{\boldsymbol{\beta}} \sim N\Big(\boldsymbol{\beta}, \sigma^2_{\epsilon}(\mathbf{X}^\mathsf{T},\mathbf{X})^{-1}\Big)$$ {#eq-dist-beta-matrix}
...
&= \boldsymbol{\beta}
\end{aligned}$$ {#eq-beta-hat-expected-matrix}
...
\end{aligned}$$ {#eq-var-beta-matrix}
...
\end{aligned}$$ {#eq-rewrite-beta-matrix}

From @eq-rewrite-beta-matrix, we see that $\hat{\boldsymbol{\beta}}$ is a linear combination of the error terms $\boldsymbol{\epsilon}$. Because $\boldsymbol{\epsilon}$ is normally distributed, the linear combination is also normally distributed (see @sec-assumptions-matrix). Thus, the distribution of the coefficients $\hat{\boldsymbol{\beta}}$ is normal.

<!--# add section on properties of estimators - unbiased, linear, gauss markov-->

## Prediction intervals {#sec-predict-intervals-matrix}

In @sec-mlr-theory-predict, we introduced intervals to quantify uncertainty in predictions from multiple linear regression models. The general form of the intervals, based on @eq-mlr-pred-int-general is

$$\hat{y}_0 \pm t^* \times SE$$ {#eq-pred-interval-appendix}

where $t^* \sim t_{n-p-1}$.

In this formula, $\hat{y}_0$ is the predicted value for the new observation, $t^*$ is the critical value and $SE$ is the standard error for the prediction.

Given the design matrix, $\mathbf{X}$, we define the row in the design matrix for the new observation $\mathbf{x}_0^\mathsf{T}$ as $\mathbf{x}_0^\mathsf{T} = [1, \mathbf{x}_0]$, where $\mathbf{x}_0$ are the values of the predictors for the new observation. Then $\hat{y}_0 = \mathbf{x}_0^\mathsf{T}\hat{\boldsymbol{\beta}}$ where, $\hat{\boldsymbol{\beta}}$ is the vector of estimated coefficients (see @sec-mlr-estimation-matrix).

The standard error $SE$ in @eq-pred-interval-appendix is defined based on whether we are quantifying uncertainty in the estimated mean response or prediction for an individual observation. The standard error for the estimated mean response is

$$SE_{\hat{\mu}} =\sqrt{\hat{\sigma}^2(\mathbf{x}_0^\mathsf{T}(\mathbf{X}^\mathsf{T}\mathbf{X})^{-1}\mathbf{x}_0)}$$ {#eq-mean-predict-se-matrix}

The standard error for the predicted value of an individual observation is

$$SE_{\hat{y}} =\sqrt{\hat{\sigma}^2(1 +\mathbf{x}_0^\mathsf{T}(\mathbf{X}^\mathsf{T}\mathbf{X})^{-1}\mathbf{x}_0)}$$ {#eq-indiv-predict-se-matrix}

\
Similar to what we observed in the case of simple linear regression in @sec-slr-theory-predict, the formula for SE associated with the prediction for an individual observation (@eq-indiv-predict-se-matrix) has the additional $\sigma^2_{\epsilon}$ to account for the additional variability in the distribution of the response for given combination of predictors.
...
$$L(\boldsymbol{\beta}, \sigma^2_{\epsilon} | \mathbf{X}, \mathbf{y}) = \frac{1}{(2\pi)^{n/2}\sigma^n}\exp\Big\{-\frac{1}{2\sigma^2}(\mathbf{y} - \mathbf{X}\boldsymbol{\beta})^\mathsf{T}(\mathbf{y} - \mathbf{X}\boldsymbol{\beta})\Big\}$$ {#eq-likelihood-matrix}
...
$$\begin{aligned}\log L(\boldsymbol{\beta}, \sigma^2_\epsilon &| \mathbf{X}, \mathbf{y}) \\ & = -\frac{n}{2}\log(2\pi) - n \log(\sigma) - \frac{1}{2\sigma^2_{\epsilon}}(\mathbf{y} - \mathbf{X}\boldsymbol{\beta})^\mathsf{T}(\mathbf{y} - \mathbf{X}\boldsymbol{\beta})\end{aligned}$$ {#eq-log-likelihood-matrix}
...
$$\log(Y) = \beta_0 + \beta_1X_1 + \dots + \beta_pX_p + \epsilon$$ {#eq-model-logy}
...
$$Y = \beta_0 + \beta_1X_1 + \dots + \beta_j\log(X_j) + \dots + \beta_pX_p + \epsilon$$ {#eq-model-logx}

Now, because the predictor $X_j$ has been transformed to the logarithmic scale, we write interpretations in terms of a multiplicative change in $X_j$. More specifically, given a constant $C$, $\log(CX_j) = \log(C) + \log(X_j)$. Let $Y|CX_j$ be the expected value of $Y$ when we plug in $CX_j$ and $Y|X_j$ be the expected value of $Y$ when we plug in $X_j$. Assuming all other predictors held constant, we have
...
$$\log(Y) = \beta_0  + \beta_1X_1 + \dots + \beta_j\log(X_j) + \dots + \beta_pX_p$$ {#eq-model-log-xy}
...
\end{aligned}$$ {#eq-log-xy-math}

Therefore, given the model in @eq-model-log-xy with a transformed response variable and transformed predictor, when $X_j$ is multiplied by a constant $C$, $Y$ is expected to multiply by $C^{\beta_j}$, holding all other predictors constant.
