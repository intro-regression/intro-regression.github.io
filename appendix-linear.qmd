...
$$\sum\limits_{i=1}^{n} \epsilon_{i}^2 = \boldsymbol{\epsilon}^\mathsf{T}\boldsymbol{\epsilon} = (\mathbf{y} - \mathbf{X}\boldsymbol{\beta})^\mathsf{T}(\mathbf{y} - \mathbf{X}\boldsymbol{\beta})$$
 {#eq-sum-sq-matrix}
...
$$SE_{\hat{\mu}} =\sqrt{\hat{\sigma}^2_\epsilon(\mathbf{x}_0^\mathsf{T}(\mathbf{X}^\mathsf{T}\mathbf{X})^{-1}\mathbf{x}_0)}$$
 {#eq-mean-predict-se-matrix}

The standard error for the predicted value of an individual observation is

$$SE_{\hat{y}} =\sqrt{\hat{\sigma}^2_\epsilon(1 +\mathbf{x}_0^\mathsf{T}(\mathbf{X}^\mathsf{T}\mathbf{X})^{-1}\mathbf{x}_0)}$$
 {#eq-indiv-predict-se-matrix}
