...
$$SE_{\hat{\mu}} =\sqrt{\hat{\sigma}^2_\epsilon(\mathbf{x}_0^\mathsf{T}(\mathbf{X}^\mathsf{T}\mathbf{X})^{-1}\mathbf{x}_0)}$$ {#eq-mean-predict-se-matrix}

The standard error for the predicted value of an individual observation is

$$SE_{\hat{y}} =\sqrt{\hat{\sigma}^2_\epsilon(1 +\mathbf{x}_0^\mathsf{T}(\mathbf{X}^\mathsf{T}\mathbf{X})^{-1}\mathbf{x}_0)}$$ {#eq-indiv-predict-se-matrix}
...
$$L(\boldsymbol{\beta}, \sigma^2_{\epsilon} | \mathbf{X}, \mathbf{y}) = \frac{1}{(2\pi)^{n/2}\sigma^n_{\epsilon}}\exp\Big\{-\frac{1}{2\sigma^2_{\epsilon}}(\mathbf{y} - \mathbf{X}\boldsymbol{\beta})^\mathsf{T}(\mathbf{y} - \mathbf{X}\boldsymbol{\beta})\Big\}$$ {#eq-likelihood-matrix}
