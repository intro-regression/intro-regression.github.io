...
\end{aligned}$$ {#eq-par-deriv-estimates}
...
In matrix notation, the error terms can be written as $$\boldsymbol{\epsilon} = \mathbf{y} - \mathbf{X}\boldsymbol{\beta}$$ {#eq-errors-matrix}

As with simple linear regression in @sec-least-sq-math, the least-squares estimator of the vector of coefficients, $\hat{\boldsymbol{\beta}}$, is the vector that minimizes the sum of squares in @eq-sum-sq-matrix. $$\sum\limits_{i=1}^{n} \epsilon_{i}^2 = \boldsymbol{\epsilon}^\mathsf{T}\boldsymbol{\epsilon} = (\mathbf{y} - \mathbf{X}\boldsymbol{\beta})^\mathsf{T}(\mathbf{y} - \mathbf{X}\boldsymbol{\beta})$$ {#eq-sum-sq-matrix}
...
The steps to find this $\hat{\boldsymbol{\beta}}$ are below. $$\begin{aligned}
...
Lastly, we show Assumption 3 that $Var(\mathbf{y}) = \sigma^2_{\epsilon}\mathbf{I}$ in linear regression. Similar to the expected value, we can get the variance from the mathematical property used to show Assumption 1. Here, we show Assumption 3 directly from the properties of variance. $$\begin{aligned}
