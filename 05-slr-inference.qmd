...
Lets take a look at the intervals for the mean response for cities with expediture of \$150 and the interval for a individual city with expendutre of \$150.
...
#### Bootstrap intervals for the mean response

- The code for simulation-based prediction intervals are an extension to the code for bootstrapped intervals.

- We compute the slope and intercept from each bootstrap sample, then we need to `pivot` the data so that there is a separate column for the slope and intercept.

- Then we use mutate to compute the predicted values.

- Similar to bootstrap confidence intervals, we use `quantile()` to compute the lower and upper bounds.

- The full pipeline is below
