...
$$\text{playgrounds} = \beta_0 + \beta_1~\text{per\_capita\_expend} + \epsilon, \hspace{5mm} \epsilon \sim N(0, \sigma^2_{\epsilon})$$ {#eq-parks-stat-model}
...
$$Y = \beta_0 + \beta_1 X + \epsilon \hspace{8mm} \epsilon \sim N(0, \sigma^2_{\epsilon})$$ {#eq-slr-model-ch5}
...
where $x^*$ is the inputted value of the predictor, $\bar{x}$ is the mean value of the predictor across all observations and, $\sigma_{\epsilon}$ is the regression standard error. We plug one of these formulas in for $SE$ in @eq-slr-pred-int-general depending on the type of prediction we're doing.
