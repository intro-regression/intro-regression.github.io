...
In @sec-bootstrap-ci, we introduced confidence intervals for quantifying uncertaininty

- in \[SEC\] we introduced intervals to quantify uncertainty. As with the inference for model coefficients, there are theory-based approaches that provide equations for computing prediction intervals.

- The advantage of these methods is that they are very quick, since they do not rely on simulating data.

- The primary disadvantage is that they require that certain assumptions are met. We discuss this more in \[ChAP REf\].

- As introduced in \[CI- SEC\], intervals for prediction take the same format as all other intervals estimate +/- critical value \* SE.

- The estimate is the predicted value $\hat{y}$ that is obtained by plugging in the value of the predictor.

- We compute the critical value in the same way as discussed in \[SEC\].

- Thus, lets spend some time on the standard error.

- There are two types of intervals for predictions - (1) the interval around the estimated mean response at a given value of the predictor and (2) the interval for the predicted value for an individual observation.

- Your turn. which interval do you expect to be wider - interval for the mean response or interval for the prediction of a individual response? why?

- Based on theoretical results, the equation for the estimated mean response is \[INSERT EQUATION\].

- The equation for the prediction for an individual response is \[INSERT EQUATION\].

- Note that hte standard error for the interval for an individual respose has the additional +1. This is to account for the additional variabiltiy in the distribution of the respose around the regression line.

Lets take a look at the intervals for the mean response for cities with expediture of \$150 and the interval for a individual city with expendutre of \$150. Note that the interval for the mean response in @tbl-prediction-intervals is very similar to the prediction interval produced by bootstrapping in \[REF for bootstrap interval\]

```{r}
#| label: tbl-prediction-intervals
#| tbl-cap: "Intervals for the mean response and individual response when `per_capita_expend = 150`" 


new_city_expend <- tibble(per_capita_expend = 150)

mean_predict <- as.data.frame(predict(parks_model, new_city_expend, interval = "confidence", level = 0.95))

individual_predict <- as.data.frame(predict(parks_model, new_city_expend, interval = "prediction", level = 0.95))
