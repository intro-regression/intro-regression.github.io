...
- Recall from \[SEC\] that we can use the model to compute predictions for different values of the predictor.

- As we've discussed in this chapter, because we are working with sample data, there is uncertainty in the estimated coefficients.

- Because we're using the estimated coefficients to compute predictions, then there is naturally uncertainty in the model predictions as well.

- Similar to how we share intervals around the coefficient estimates, we can compute intervals around the predictions.

- We will use the bootstrapped samples along with the coefficients produced by each bootstrapped sample to compute the predictions.

- Similar to bootstrapped intervals, for a given value of the predictor, the $C\%$ interval is made from the middle $C\%$ of the distribution.

- Let's use the bootstrap data from \[SEC\] and look at the predicted number of playgrounds per 10,000 residents when the spending per resident is \$150.

Table X shows the predictions from the first five bootstrap samples.
...
The distribution of the predictions with the bounds for the 95% interval shown below.
...
The bounds for the 95% interval of predictions when `spend = 150` is `r round(boot_pred_lb, 3)` to `r round(boot_pred_ub, 3)`. The interpretation is as follows:
...
- Note that this interval is for the predicted mean among all cities with expenditure equal to \$150. In other words, we are predicting the value on the line , but recall from \[SEC\] that there is a distribution of the response for each value of the predictor. We need to account for this extra variability when computing the predictions for individual observations. WE'll take more about this in \[SEC - prediction theory section\].
