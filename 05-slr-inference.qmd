...
Use `predict()` introduced in \[REF - chap 4?\]. code is shown below.

- Make a tibble with the new value.

- Argument `interval =` takes value `confidence` for the interval for the mean response and `prediction` for the interval for an individual response.

- `level` is the is a value 0 to 1 that specifies the confidence level.
