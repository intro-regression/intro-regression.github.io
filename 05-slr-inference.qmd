...
<!--# add something about prediction intervals-->
...
One type of outdoor space the authors study in the report is playgrounds. The report describes playgrounds as outdoor spaces that "bring children and adults together" [@publicland, pp. 13], and in one example the playground was a place that was important for distributing "fresh food and prepared meals to those in need, particularly school-aged children" [@publicland, pp. 9] during the global COVID-19 pandemic.
...
The visualizations and summary statistics for univariate exploratory data analysis are in @fig-parks-univariate-eda and @tbl-parks-univariate-eda. The bivariate EDA is in @fig-parks-bivariate-eda.
...
#| fig-cap: "Univariate distributions for the number of playgrounds per 10,000 residents a city's per capita spending"
#| fig-subcap: 
...
#| tbl-cap: "Summary statistics for number of playgrounds per 10,000 residents a city's per capita spending"
#| echo: false
...
The distribution of `playgrounds`, the number of playgrounds per 10,000 residents (the response variable), is unimodal and right-skewed. The center of the distribution is the median of about `r median(parks$playgrounds)` playgrounds per 10,000 residents, and the spread of the middle 50% of the distribution (the IQR) is `r IQR(parks$playgrounds)`. There appear to be two potential outlying cities with more than 6 playgrounds per 10,000 residents, indicating high playground access relative to the other cities in the data set.

The distribution of `per_capita_expend`, a city's expenditure per resident (the predictor variable), is also unimodal and right-skewed. The center of the distribution is around `r median(parks$per_capita_expend)` dollars per resident, and the middle 50% of the distribution has a spread of about `r IQR(parks$per_capita_expend)` dollars per resident. Similar to the response variable, there are some potential outliers. There are `r parks |> filter(per_capita_expend > 300) |> nrow()` cities that invest more than 300 dollars per resident.
...
#| fig-cap: "Bivariate exploratory data analysis of the relationship between the number of playgrounds per 10,000 residents a city's per capita spending" 
...
From @fig-parks-bivariate-eda there appears to be a positive relationship between a city's per capita expenditure and the number of playgrounds per 10,000 residents. The correlation is `r round(corr_spend_playground, 3)`, indicating the relationship between playground access and city expenditure is not strong. This is partially influenced by the outlying observations that have relatively low values of per capita expenditure but high numbers of playgrounds per 10,000 residents.
...
$$\text{playgrounds} = \beta_0 + \beta_1~\text{per\_capita\_expend} + \epsilon, \hspace{5mm} \epsilon \sim N(0, \sigma^2)$$ {#eq-parks-stat-model}

The output of the fitted regression model is @tbl-parks-model.

```{r}
#| label: tbl-parks-model
#| tbl-cap: "Linear regression model using a city's per capita spending to explain variability in the number of playgrounds per 10,000 residents"
...
spend_slope <- parks_model_tidy |> filter (term == "per_capita_expend") |> select(estimate) |> pull()

spend_se <- parks_model_tidy |> filter (term == "per_capita_expend") |> select(std.error) |> pull()
```

$$\widehat{\text{playgrounds}} = 2.418 + 0.003 \times \text{per\_capita\_expend}$$ {#eq-parks-model}
...
[^05-slr-inference-2]: Slope: For each additional dollar a city spends per resident, there is expected to be 0.003 more playgrounds per 10,000 residents, on average.\
...
- **Hypothesis test:** Test of a specific claim about the population-level slope
- **Confidence interval**: A range of values that the population-level slope may reasonably take
...
$$Y = \beta_0 + \beta_1 X + \epsilon \hspace{8mm} \epsilon \sim N(0, \sigma^2)$$ {#eq-slr-model-ch5}
...
3.  The variance $Y|X$ is $\sigma^2$. This variance is equal for all values of $X$ and thus does not depend on $X$.
...
[^05-slr-inference-3]: We sample with replacement so that we get a new sample of $x_i, y_i$ pairs each time we bootstrap. In each bootstrap sample, some $x_i, y_i$ pairs from original data may appear multiple times and some may be omitted. If we sampled without replacement, the resulting bootstrap samples would always be exactly the same as the original sample data.
...
Using these four steps, let's construct the 95% confidence interval for the population slope $\beta_1$ of the relationship between `per_capita_spend` and `playgrounds`.
...
#| echo: false
#| tbl-cap: "First 10 rows from a single bootstrap sample. The replicate column identifies the bootstrap sample."
...
2.  Next, we fit a linear model of the form in @eq-parks-stat-model to each of the 1000 bootstrap samples. The estimated slopes and intercepts for the first three bootstrap samples are shown in @tbl-parks-boot-slopes.
    ...
    #| echo: false
    #| tbl-cap: "Estimated slope and intercept computed for three bootstrap samples"
...
3.  We are focused on inference for $\beta_1$, the slope of `per_capita_expend`, so we collect estimated slopes of `per_capita_expend` to make the bootstrap distribution. This is the approximation of the sampling distribution of $\hat{\beta}_1$. A histogram and summary statistics for this distribution are shown in @fig-parks-boot-dist and @tbl-parks-boot-dist, respectively.
    ...
    #| fig-cap: "Bootstrap distribution of the slope for per capita spending constructed from 1,000 bootstrap samples"
    ...
    #| tbl-cap: "Summary statistics for the bootstrap distribution of the slope for per capita spending"

    spend_dist |>
      summarise(Min = min(estimate), Q1 = quantile(estimate, 0.25), Median = median(estimate), Q3 = quantile(estimate, 0.75), Max = max(estimate), Mean = mean(estimate), Std.Dev. = sd(estimate)) |>
      kable(digits = 3)
    ```
...
#| tbl-cap: "95% confidence interval for the slope of per capita spending"
spend_dist |>
...
[^05-slr-inference-6]: The points at the $5^{th}$ and $95^{th}$ percentiles make the bounds for the 90% confidence interval. The points at the $1^{st}$ and $99^{th}$ percentiles mark the lower and upper bounds for a 98% confidence interval.
...
In **permutation sampling** the values of the response variable are randomly shuffled and paired with values of the predictor variable, thus generating a new sample of the same size as the original data. The process of randomly pairing the values of the response and the predictor variables simulates the null hypothesized condition that there is no linear relationship between the two variables.
...
#| tbl-cap: "First 10 rows of a permutation sample. The replicate column identifies the permutation sample."
set.seed(12345)
niter = 1000
...
#| tbl-cap: Estimated slopes computed from 10 permutation samples.
...
3.  Next, we collect the estimated slopes from the previous step to construct the simulated null distribution. We will use this distribution to assess the strength of the evidence from the original sample data against the null hypothesis.
    ...
    #| fig-cap: "Simulated null distribution to test the slope of per capita spending"
...
#| tbl-cap: "Summary statistics for the simulated null distribution to test the slope of per capita spending"
...
num_extreme_obs <- spend_null_dist |>
  filter(term== "per_capita_expend") |>
  filter(estimate >= spend_slope | estimate <= -spend_slope)|> 
  nrow()
...
#| fig-cap: "Simulated null distribution with p-value represented by the shaded area. The shaded area includes values -0.003 or less and values 0.003 or greater."
...
A commonly used threshold is $\alpha = 0.05$. If stronger evidence is required to reject the null hypothesis,<!--# Maybe explain why this might be the case--> then a lower threshold can be used to make a conclusion. If such strong evidence is not required (this may be the case in analyses with very small sample sizes), then higher a threshold can be used. It is general convention to use a threshold 0.1 or less, so any p-value greater than 0.1 is considered large enough to fail to reject the null hypothesis.
...
### Type I and Type II error {#sec-inf-error}
...
```{r}
#| label: tbl-errors
#| tbl-cap: "Type I and Type II error"
tab <- tibble(
  Decision = c(
...
A **Type II error** has occurred if the alternative hypothesis is actually true, but we fail to reject the null hypothesis, because the p-value is large. Computing the probability of making this type of error is less straightforward. It is computed as $1 - \text{Power}$. The **power** of a hypothesis test is the probability the null hypothesis is rejected given the alternative hypothesis is true, $P(\text{rejct }H_0 | H_a \text{ true})$. Power depends on the minimum effect size (magnitude of the slope) that would be meaningful for the analysis and the sample size. We often consider the power as we plan in the first step of the data science workflow (see @fig-data-science-workflow). The details for computing power are beyond the scope of this text, but we note there are many online calculators and R packages, such as **pwr** [@pwr], that can be used compute power based on the specifications of the analysis.
...
This illustrates the power of confidence intervals; they can not only be used to draw a conclusion about a claim (reject or fail to reject $H_0$), but they also give the range of values that the population slope may take. Thus, it is good practice to always report the confidence interval, because the confidence interval provides more detail about a population slope $\beta_1$ beyond the reject/fail to reject conclusion of the hypothesis test.
...
## Prediction intervals {#sec-slr-bootstrap-pred-intervals}

In @sec-slr-predict , we use the linear regression model to compute predicted values of the response variable for a given value of the predictor. Because the regression model is fit using sample data, there is variability in the estimated model coefficients (slope and intercept) used to compute the predicted value. Given this uncertainty in the model coefficients, there is naturally uncertainty in the predicted values as well. Thus, we will quantify the uncertainty in the predictions utilizing similar methods introduced in previous sections to quantify uncertainty in model coefficients.

In @sec-bootstrap-ci, we introduced bootstrap sampling to construct the sampling distribution of the slope, $\beta_1$. Similarly, we can use the bootstrap samples and the model estimated from each bootstrap sample to generate a distribution of predictions, $\hat{y}$ for a given value of the predictor, $x$. As with the bootstrap confidence interval for the slope, mark the bounds for the middle $C\%$ of the distribution as the interval for our prediction.

Given a particular value of the predictor, call it $x*$, the $C\%$ interval for the predicted values when $x = x*$ are as follows:

1.  Generate $niter$ bootstrap samples. For each bootstrap sample:
    1.  Fit the model.
    2.  Use the estimated slope and intercept to compute the predicted response $\hat{y}$ when $x = x*$.
2.  Use the $niter$ values of $\hat{y}$ to construct the distribution of predicted values when $x = x*$.
3.  Mark the bounds for the middle $C\%$ of the distribution to construct the $C\%$ interval for the predicted value when $x = x*$.

To illustrate this, let's use the bootstrap samples from @sec-construct-bootstrap-ci to identify a plausible range of values for the predicted number of playgrounds per 10,000 residents when a city's per capita expenditure is is \$150. Table @tbl-bootstrap-pred-150 shows the estimated slope, intercept, and prediction for the first five bootstrap samples.

```{r}
#| label: tbl-bootstrap-pred-150
#| tbl-cap: "Estimated slope, intercept, and predicted number of playgrounds when per capita spending is 150 USD, computed from five bootstrap samples"

set.seed(12345)

niter = 1000
boot_prediction <- parks |>
  specify(playgrounds ~ per_capita_expend) |>
  generate(reps = niter, type = "bootstrap") |>
  fit() |>
  pivot_wider(
  ...
  mutate(prediction = intercept + per_capita_expend * 150) |>
ungroup()
...
Recall from @sec-construct-bootstrap-ci, we generated 1000 bootstrap samples. The distribution of the predictions from the 1000 bootstrap samples when `per_capita_spend = 150` is in @fig-bootstrap-pred-150. The points on the distribution span the middle 95% of the distribution are marked on the visualization.

```{r}
#| label: fig-bootstrap-pred-150
#| fig-cap: "Bootstrap distribution and bounds for the 95% confidence interval for the predicted number of playgrounds when per capita spending is equal to 150 USD"

boot_pred_lb <- quantile(boot_prediction$prediction, 0.025)
boot_pred_ub <- quantile(boot_prediction$prediction, 0.975)

ggplot(data = boot_prediction, aes(x = prediction)) + 
...
The bounds for the 95% interval for the predicted number of playgrounds per 10,000 residents when `spend = 150` is `r round(boot_pred_lb, 3)` to `r round(boot_pred_ub, 3)`. Now that we have the interval, we can interpret it in the context of the analysis.

> We are 95% confident there are between `r round(boot_pred_lb, 3)` and `r round(boot_pred_ub, 3)` playgrounds per 10,000 residents in a city with expenditure of \$150.

Recall from @sec-slr-specify that the output of the linear regression model is the estimated mean value of the response given a value of the predictor. Therefore, we have quantified the uncertainty in the estimated [*mean number of playgrounds per 10,000 residents*]{.underline} among all cities with per capita expenditure of \$150. If we wish to compute an interval around the predicted value for an individual city, we need to account for some additional uncertainty. We discuss this more in @sec-slr-theory-predict.
...
Thus far we have approached inference using simulation-based methods (bootstrapping and permutation) to generate sampling distributions and null distributions. When certain conditions are met, however, we can use theoretical results about the sampling distribution to understand the variability in $\hat{\beta}_1$. In this section, we present that theory, then use it to conduct statistical inference for the population slope. Notice as we go through this section that the inferential procedures and conclusions are very similar as before. The primary difference is in how we understand the sampling variability in $\hat{\beta}_1$ and obtain the null distribution.

### Central Limit Theorem {#sec-slr-clt}

<!--# maybe add something in the appendix that the CLT is not needed if we have normal errors-->

The **Central Limit Theorem (CLT)** is a foundational theorem in statistics about the distribution of a statistic and the associated mathematical properties of that distribution. <!--# find a source for a good statement of CLT-->For the purposes of this text, we will focus on what the Central Limit Theorem says about the distribution of an estimated slope $\hat{\beta}_1$, but note that this theorem applies to statistics other than the slope. We will also focus on the results of the theorem and less so on derivations or advanced mathematical details of the Central Limit Theorem.<!--# Texts such as [insert references] or the appendix may be of interest to those seeking more detail about the theorem.-->
...
$$SE_{\hat{\beta}_1} = \hat{\sigma}_{\epsilon}\sqrt{\frac{1}{(n-1)s_X^2}}$$ {#eq-slr-se}
...
### Estimating $\sigma$ {#sec-estimate-reg-std-err}

As discussed in @sec-slr-specify, there are three parameters that need to be estimated for simple linear regression $\beta_0$, $\beta_1$, and $\sigma$. @sec-slr-estimation introduced least squares regression, a method for deriving $\hat{\beta}_0$ and $\hat{\beta}_1$, the estimates for intercept and slope, respectively. Now, we turn to the estimation of $\sigma_\epsilon$, also known as the **regression standard error*.***
...
$$\hat{\sigma}_{\epsilon} = \sqrt{\frac{\sum_{i = 1}^ne_i^2}{n - 2}} = \sqrt{\frac{\sum_{i = 1}^n(y_i - \hat{y}_i)^2}{n - 2}}$$ {#eq-slr-reg-se}
...
Recall that the standard deviation is the average distance between each observation and the mean of the distribution. Similarly, the regression standard error can be thought of as the average distance the observed value of the response is from the regression line. The regression standard error $\hat{\sigma}_{\epsilon}$ is used to quantify the sampling variability in the estimated slope $\hat{\beta}_1$ (@eq-slr-se), so we will use this value as we conduct inference on the slope.

### Hypothesis tests for the slope {#sec-slr-inf-clt-hypothesis-test}
...
$$T = \frac{\hat{\beta}_1 - 0}{SE_{\hat{\beta}_1}}$$ {#eq-test-stat}
...
::: {.yourturn latex=""}
Consider the magnitude of the test statistic, $|T|$. Do you think test statistics with small magnitude provide evidence in support of or against the null hypothesis? What about test statistics with large magnitude?[^05-slr-inference-12]
:::
...
\end{aligned} $$ {#eq-pvalue-two-sided}
...
where $\beta_1$ is the true slope between `per_capita_expend` and `playgrounds`. From @tbl-parks-model, we know the observed slope $\hat{\beta}_{1}$ is `r round(spend_slope, 4)` and the estimated standard error $SE_{\hat{\beta}_{1}}$ is `r round(spend_se, 4)`. The test statistic is
...
Given there are $n$ = `r n` observations, the test statistic follows a $t$ distribution with $97 - 2 =$ `r df` degrees of freedom. The p-value, is $Pr(t < - |2.063| \text{ or } t > |2.063|) =$ `r round(pval, 3)`. The area under the curve of the $t$ distribution used to compute the p-value is shown in @fig-compute-pvalue. The code to compute the p-value from the $t$ distribution is in @sec-theory-inf-r; however, we typically obtain the p-value from the software's model output in practice.

```{r}
#| label: fig-compute-pvalue
#| fig-cap: "Shaded area used to compute the p-value from a t-distribution with 95 degrees of freedom"

# Parameters
df_val <- df  # degrees of freedom for the t-distribution

# Create data for the full curve
x_vals <- seq(-5, 5, length.out = 1000)
t_data <- data.frame(
  x = x_vals,
  y = dt(x_vals, df = df_val)
)

# Data for the shaded tails
left_tail <- t_data %>% filter(x <= -2.063)
right_tail <- t_data %>% filter(x >= 2.063)
...
ggplot(t_data, aes(x = x, y = y)) +
  ...
  geom_area(data = left_tail, aes(x = x, y = y), fill = "darkcyan") +
  geom_area(data = right_tail, aes(x = x, y = y), fill = "darkcyan") +
  ...
  theme_bw() +
  annotate("text", x = -2.75, y = y_max * 1.05, label = "-2.063", 
           color = "gray30", size = 5, fontface = "bold") +
  annotate("text", x = 2.75, y = y_max * 1.05, label = "2.063", 
           color = "gray30", size = 5, fontface = "bold") 
```

::: {.yourturn latex=""}
A colleague makes the following statement:

*The p-value `r round(pval,3)` means there is a `r round(pval,3) * 100` % chance the null hypothesis is true.* Is this interpretation of the p-value correct? If not, write the correct interpretation.[^05-slr-inference-13]
:::

[^05-slr-inference-13]: This is incorrect and a common misinterpretation of the p-value. A correct interpretation is *given there is no relationship between a city's per capita spending and number of playgrounds per 10,000 residents, the probability of observing a slope of `r round(spend_slope,3)` or more extreme is `r round(pval, 3)`*.
...
### Confidence intervals {#sec-slr-theory-ci}
...
<!--# add the general formula for a confidence interval-->

The equation for a $C\%$ confidence interval for $\beta_1$ is

$$\hat{\beta}_1 \pm t^* \times SE_{\hat{\beta}_1}$$ {#eq-conf-int-beta}
...
### Prediction intervals {#sec-slr-theory-predict}

In @sec-slr-bootstrap-pred-intervals, we introduced simulation-based methods for constructing intervals around the predicted mean response given a value of the predictor. As with inference for model coefficients, we can quantify the uncertainty in model predictions used equations based on mathematical theory. The primary advantages of the intervals computed from mathematical equations are that they are more exact than the simulation-based intervals and the computational time needed to compute them is negligible. The primary disadvantage is that these intervals are only valid when certain assumptions about the data hold. We discuss these assumptions in @sec-ch-slr-conditions.

$$\hat{y} \pm t^* \times SE$$ {#eq-pred-int-general}

where $t^* \sim t_{n-2}$ .

@eq-pred-int-general is the general equation for quantifying uncertainty in predictions, such that $\hat{y}$ is the predicted value, $t^*$ is the critical value and $SE$ is the standard error for the prediction.

The predicted value $\hat{y}$ is the value output by the model given a particular value of the predictor variable. The critical value $t^*$ is determined by the confidence level as introduced in @sec-slr-theory-ci. It is the value on the $t$ distribution with $n-2$ degrees of freedom, such that $C\%$ of the area under the curve is between $-t^*$ and $t^*$.
...
::: {.yourturn latex="1"}
Which type of prediction do you expect to have a wider interval:

- Prediction for the mean value of the response
- Prediction for an individual observation ?

Briefly explain your choice.
:::

@eq-slr-pred-mean-se and @eq-slr-pred-indiv-se are the equations to compute the standard error for the mean response and an individual observation, respectively.

$$SE_{\hat{\mu}} = \hat{\sigma}_{\epsilon}\sqrt{\frac{1}{n} + \frac{(x^* - \bar{x})}{\sum_{i=1}^n(x_i - \bar{x})^2}}$$ {#eq-slr-pred-mean-se}

$$SE_{\hat{y}} = \hat{\sigma}_{\epsilon}\sqrt{1 + \frac{1}{n} + \frac{(x^* - \bar{x})}{\sum_{i=1}^n(x_i - \bar{x})^2}}$$ {#eq-slr-pred-indiv-se}

where $x^*$ is the inputted value of the predictor, $\bar{x}$ is the mean value of the predictor across all observations and, $\sigma$ is the regression standard error. We plug one of these formulas in for $SE$ in @eq-slr-pred-int-general depending on the type of prediction we're doing.

The details of the equations are beyond the scope of this text, but let's take a moment to compare the two intervals. The equations look very similar with the exception of the additional 1 in @eq-slr-pred-indiv-se. This is accounts for the fact that when quantifying uncertainty in the prediction for an individual observation, we not only take into account the variability about the regression line but also the variability in the distribution of the response for a given value of the predictor. Thus predictions for an individual observation result in wider intervals than intervals for the mean response.

```{r}
#| label: predicted-playgrounds-150
#| 
new_city_expend <- tibble(per_capita_expend = 150)
pred_150 <- predict(parks_model, new_city_expend)
```

Let's take a look at the intervals around predictions given `per_capita_expend = 150`. <!--# do I have the model equation anywhere earlier?--> The predicted number of playgrounds per 10,000 residents when per capita spending is 150 is `r pred_150`. @tbl-prediction-intervals shows the standard error and 95% interval for predicting the mean number of playgrounds per 10,000 residents among all cities with per capita spend of \$150 and the 95% interval for the predicted number of playgrounds for an individual city with per capita spend of \$150.

```{r}
#| label: tbl-prediction-intervals
#| tbl-cap: "Standard errors and 95% intervals for the estimated mean number of playgrounds per 10,000 and predicted number of playgrounds per 10,000 residents for an individual city  when per capita spending is 150 USD"

new_city_expend <- tibble(per_capita_expend = 150)

#interval for mean response

mean_predict <- predict(parks_model, new_city_expend, interval = "confidence", level = 0.95, se.fit = TRUE)
...
indiv_predict <- predict(parks_model, new_city_expend, interval = "prediction", level = 0.95, se.fit = TRUE)
...
The interval for the mean response in @tbl-prediction-intervals is similar to the interval produced by bootstrapping in @sec-slr-bootstrap-pred-intervals.

The interpretations for the intervals in @tbl-prediction-intervals are as follows:

> We are 95% confident that the mean number of playgrounds per 10,000 residents in cities that have per capita expenditure of \\\$150 is between `r round(mean_predict_fit[2], 3)` to `r round(mean_predict_fit[3], 3)`.
>
> We are 95% confident the number of playgrounds per 10,000 residents in an individual city with per capita expenditure of \\\$150 is between `r round(indiv_predict_fit[2], 3)` to `r round(indiv_predict_fit[3], 3)`.

## Inference in R

### Bootstrap confidence interval for $\beta_1$ {#sec-slr-bootstrap-r}
...
### Permutation tests for $\beta_1$ {#sec-slr-permutation-r}
...
1.  Set a seed to make the results reproducible.
2.  Define the number of permutation samples (iterations). Permutation sampling can be computing intensive when using large data sets and a large number of iterations. We recommend using a small number of iterations ( 10 - 100) when testing code, then increasing the iterations once the code is finalized.
...
Finally, we compute the the p-value using `get_p_value()`.
...
# compute p-value 
get_p_value(null_dist, estimated_slope, direction = "two-sided") |>
  filter(term == "per_capita_expend")
```

This p-value is close but not the same as the p-value we computed in @sec-calc-pvalue. This is due to the way `get_p_value()` computes the p-value. In @sec-calc-pvalue, we computed the p-value by counting all estimated slopes that were less than -`r spend_slope` or greater than `r spend_slope`. In contrast, `get_p_value()` computes the p-value by counting the number of observations greater than `r spend_slope` and multiplying by two. It makes use of the fact that the theoretical null distribution is symmetric (see @sec-slr-inf-clt-hypothesis-test), even if the simulated null distribution is not exactly symmetric.

We can use either approach to compute the p-value. In this case, the p-value was so close to the standard cut off of 0.05, that the different approaches led to different inferential conclusions. This is an example of why it is not good practice to solely rely on a p-value and is important to take into account the magnitude of the estimated slope and confidence interval when making conclusions in practice.
...
### Theory-based inference for $\beta_1$ {#sec-theory-inf-r}
...
The code for computing prediction intervals largely builds upon code we've seen in previous sections.

#### Simulation-based intervals for prediction

The code for computing bootstrapped prediction intervals is below. We use the code from @sec-slr-bootstrap-r to generate the bootstrap samples and fit the model for each sample. Because the resulting output is in long format, we pivot the data to wide format using `pivot_wider()` in the **tidyr** package [@tidyr] (part of **tidyverse**). This results in a data frame such that the bootstrapped intercepts and bootstrapped slopes are in separate columns.

We then use `mutate()` from **dplyr** [@dplyr] to compute the predictions from each bootstrap sample. The bounds of the prediction interval are computed using `quantile()` as with the bounds of the bootstrap confidence intervals in @sec-slr-bootstrap-r.
...
boot_prediction |>
  summarise(boot_pred_lb = quantile(prediction, 0.025),
              boot_pred_ub = quantile(prediction, 0.975))
```

#### Theory-based intervals

The `predict()` function introduced in @sec-slr-predict-R is used to compute the intervals for preditcted mean response or an individual observation. The argument `interval =` specifies the interval type, and the argument `level =` specifies the confidence level. The default level is 95%.

Start by making a tibble of the value of the predictor that you wish to use to make predictions. Below is the code to specify that the predictor `per_capita_expend` is equal to 150.

```{r}
new_city <- tibble(per_capita_expend = 150)
```

We can specify multiple values in the tibble to produce the predictions and respective intervals for multiple values of the predictor.

The code to produce the 95% confidence interval for the predicted mean number of playgrounds per 10,000 residents for all cities with per capita spending of \$150 is below.

```{r}
#| echo: true

predict(parks_model, new_city_expend, interval = "confidence", level = 0.95)
```

The code to produce the 95% confidence interval for the predicted number of playgrounds per 10,000 residents for a single city with per capita spending of \$150 is below.

```{r}
#| echo: true
predict(parks_model, new_city_expend, interval = "prediction", level = 0.95)

```

In the output for each interval, `fit` is the predicted value, $\hat{y}$, `lwr` is the interval's lower bound, and `upr` is the interval's upper bound.

## Summary

In this chapter we introduced two approaches for conducting statistical inference to draw conclusions about a population slope, simulation-based methods and theory-based methods. The standard error, test statistic, p-value and confidence interval we calculated using the mathematical models from the Central Limit Theorem align with what is seen from the output produced by statistical software in @tbl-parks-model. Modern statistical software will produce these values for you, so in practice you will not typically derive these values "manually" as we did in this chapter. As the data scientist your role will be to interpret the output and use it to draw conclusions. It's still valuable, however, to have an understanding of where these values come from in order to interpret and apply them accurately. As more software has embedded artificial intelligence features, understanding how the values are computed also helps us check if the software's output makes sense given the data, analysis objective, and methods.
...
<!--# how would you simulate the prediction interval for an individual observation-->
