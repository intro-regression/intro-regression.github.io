...
$$SE_{\hat{\mu}} = \hat{\sigma}_{\epsilon}\sqrt{\frac{1}{n} + \frac{(x^* - \bar{x})}{\sum_{i=1}^n(x_i - \bar{x})^2}}$$
 {#eq-slr-pred-mean-se}

$$SE_{\hat{y}} = \hat{\sigma}_{\epsilon}\sqrt{1 + \frac{1}{n} + \frac{(x^* - \bar{x})}{\sum_{i=1}^n(x_i - \bar{x})^2}}$$
 {#eq-slr-pred-indiv-se}
...
names(mean_predict_all) <- c("y.hat", "Lower bound", "Upper bound", "Std. Error")

# interval for individual response - START HERE , Remove eval - fALSE

mean_predict <- mean_predict |>
  select(se.fit, fit) |>
  as.data.frame()

#individual_predict <- as.data.frame(predict(parks_model, new_city_expend, interval = "prediction", level = 0.95, se.fit = TRUE))

#bind_rows(mean_predict, individual_predict)  |>
  mutate(type = c("Mean response", "Individual response"), .before = 1) |>
  select(fit, lwr, upr) |>
  kable(digits = 3, col.names = c("Type", "Lower bound", "Upper bound"))
```

Note that the interval for the mean response in @tbl-prediction-intervals is very similar to the prediction interval produced by bootstrapping in \[REF for bootstrap interval\]

The interpretations for the intervals in @tbl-prediction-intervals are as follows:

> We are 95% confident that the mean number of playgrounds per 10,000 residents in cities that have per capita expenditure of \\\$150 is between `r round(mean_predict$lwr, 3)` to `r round(mean_predict$upr, 3)`.
>
> We are 95% confident the number of playgrounds per 10,000 residents in an individual city with per capita expenditure of \\\$150 is between `r round(individual_predict$lwr, 3)` to `r round(individual_predict$upr, 3)`.
