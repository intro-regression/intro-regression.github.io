...
- `country`: Cuisine's country / region of origin (`Brazilian`, `Filipino`, `French`, `Italian`, `Other`). This is a simplified version of the `country` variable in the `cuisines` data frame.

- `author`: User who posted the recipe

- `chef_john`: Indicator of whether the recipe was posted by American Chef John Mitzewich (known as Chef John), derived from the variable `author`. (`1`: Posted by Chef John, `0`: Not posted by Chef John)
...
This book is about regression analysis, using statistical models to describe the relationship between a response variable and one or more predictor variables and to predict values of the response variable. This chapter focuses exploratory data analysis, an important step of the workflow that happens before modeling as shown in @fig-data-science-workflow. **Exploratory data analysis** **(EDA)** is the process of examining the data to better understand the observations, distributions of variables, relationships between variables, and other interesting features of the data. It helps us identify **outliers,** observations that are far from the majority of the data, along with instances of missing data, and potential errors in the data<!--# such as errors in how the data were recorded or how the data set was loaded into the statistical software-->. Lastly, it is where we gain initial insights about potential relationships between variables that help inform the decisions we'll make as we do the regression analysis.
...
> "Today, exploratory and confirmatory \[methods\] can — and should — proceed side by side." [@tukey1977exploratory, p. vii]
...
#| tbl-cap: "Glimpse of recipes data frame"
...
#| tbl-cap: "Glimpse of data with correct formats for Chef John and month variables"
...
#| fig-cap: "Barc chart for the distribution of the month recipes were posted"
...
#| fig-cap: "Visualizations of the distribution of season recipes were posted"
#| fig-subcap: 
#|   - "Bar chart"
#|   - "Pie chart"
#|   - "Waffle chart"
...
[^03-eda-2]: In general, the number of recipes posted is approximately equal across the winter, summer, and fall seasons. The smallest proportion of recipes are posted during the spring season.
...
#| fig-cap: "Histogram for the distribution of average ratings of recipes"
...
@fig-univar-eda-rating is the histogram of `avg_rating`. We now clearly see that a vast majority of the recipes have average ratings between 4 and 5. There are very few recipes with ratings less than 3, and at least one outlier with a an average rating around 1.
...
#| fig-cap: "Example histogram bin widths"
#| fig-subcap: 
#|   - Default
#|   - Small
#|   - Large
...
@fig-histogram-binwidth shows examples of the default bin width, very small bin widths, and very large bin widths. Small bin widths add more detail the histogram. This can make the histogram more challenging to read without adding much useful information for the analysis. In contrast, large bin widths can aggregate the data so much that we lose key information about the distribution.
...
#| fig-cap: "Density plot and boxplot of average ratings"
#| fig-subcap:
#|   - "Density plot"
#|   - "Boxplot"
#| layout-ncol: 2
...
#| fig-cap: Examples of distribution skewness
#| fig-subcap: 
#|   - "Left-skewed"
#|   - "Right-skewed"
#|   - "Symmetric"
...
#| fig-cap: Examples of distribution modality
#| fig-subcap: 
#|   - "Unimodal"
#|   - "Bimodal"
#|   - "Multimodal"
...
In addition to the skewness, the shape of the distribution includes the **modality**, the number of peaks. @fig-shape-mode shows examples of distributions with three different modalities: unimodal, bimodal, and multimodal. @fig-shape-mode-1 is an example of a **unimodal** distribution, a distribution with a single peak. The distribution of annual incomes for adults in the United States is an example of a unimodal distribution.

@fig-shape-mode-2 is an example of a **bimodal** distribution, a distribution with two peaks. Bimodal distributions generally indicate there are at least two subgroups represented in the data that we may want to take into account in the analysis. An example of a bimodal distribution is a city's average daily temperatures across a year. If the city has four distinct seasons, there will typically be a peak around the mean temperature in the fall and winter (low temperatures) and a peak around the mean temperature for spring and summer (high temperatures).

@fig-shape-mode-3 is an example of a **multimodal** distribution, a distribution with three or more peaks. Similar to bimodal distributions, multimodal distributions generally indicate there are multiple subgroups in the data we want to take into account in the analysis. An example of a multimodal distribution is the distribution of heights for school-aged children across a large age range, about 5 - 18 years old. There will be a peak around the mean height for elementary school children (around ages 5 - 9), a peak around the mean height for middle school children (around ages 10 - 14), and a peak around the mean height for high school children (around ages 14 - 18).
...
#| tbl-cap: "Summary statistics for average rating"
...
#| fig-cap: "Distribution of recipe cooking time (in minutes)"
...
#| tbl-cap: "Summary statistics for cooking time"
...
#| fig-cap: "Original versus truncated distribution of cooking time" 
#| fig-subcap: 
#|   - "All observations"
...
#| fig-cap: "Scatterplot of the relationship between number of calories and grams of protein"
...
@fig-bivar-eda-protein-calories is the scatterplot with `calories` on the x-axis and `protein` on the y-axis. From the visualization, we can see the shape and direction of the relationship, along with outliers. We also get an indication of the strength of the relationship, which we will quantify using summary statistics.
...
#| fig-cap: Example directions of bivariate relationships
#| fig-subcap: 
#|   - "Positive"
#|   - "Negative"
#|   - "No direction"
...
From @fig-bivar-eda-protein-calories, the relationship between `calories` and `protein` is positive and linear. In general, recipes with more calories per serving tend to also have more grams of protein per serving.

#### Describe the relationship: Strength {.unnumbered}

The **strength** of the relationship between two variables is a measure of how closely the observations follow the overall pattern or shape. Points that are tightly clustered together indicate a stronger relationship than points that are more dispersed. @fig-bivariate-strength shows examples of linear relationships with different strengths.

```{r}
#| label: fig-bivariate-strength
#| fig-cap: Example strengths of bivariate relationships
#| fig-subcap: 
#|   - "Strong"
#|   - "Moderate"
#|   - "Weak"
...
The direction of the linear relationship between two variables is indicated by the sign of the correlation. The strength is indicated by the magnitude of the correlation, $|r|$.
...
#| fig-cap: Example relationships with low correlation 
#| fig-subcap: 
#| - "|r| = 0.12"
#| - "|r| = -0.12"
#| layout-ncol: 2
...
When looking at the relationship between two variables, outliers are observations that fall outside the general trend of the data. Outliers may have out-sized influence when building a regression model. Therefore, it is important to identify outliers in the EDA, so we can take them into account when evaluating the results from the regression analysis. We discuss the impact of outliers on regression modeling in @sec-outliers-impact.
...
#| fig-cap: "Relationship between preparation time and average rating" 
...
#| fig-cap: "Relationship between prepration time and average rating for recipes with preparation time 45 minutes or less"
...
#| fig-cap: "Relationship between whether recipe was posted by Chef John and average rating"
#| fig-subcap: 
#|   - Boxplot
#|   - Ridgeline plot
#|   - Violin plot
...
#| tbl-cap: "Summary statistics for averae rating based on whether recipe was posted by Chef John"
...
#| fig-cap: "Relationship between cuisine's country/region of origin and average rating"
...
#| fig-cap: "Visualizations of the relationship between whether a recipe was posted by Chef John and the season"
#| fig-subcap: 
#|   - "Grouped bar chart"
#|   - "Segmented bar plot"
#|   - "Mosaic plot"
...
#| fig-cap: "Segmented bar chart of cuisine's country/region of origin and season"
...
#| fig-cap: "Multivariate EDA of the relationship between average rating and whether a recipe was posted by Chef John, faceted by season"  
...
#| tbl-cap: "`skim()` output for average rating and season"
#| echo: true
...
We introduced waffle charts in @sec-univar-eda-cat to explore the univariate distribution of a categorical variable. Waffle charts are produced using `geom_waffle()` in the **waffle** package [@waffle]. The functions in this package are developed to work within the **ggplot2** framework, so we can use all the **ggplot** functions from @sec-customize-plots to customize the charts.
...
In @sec-bivar-eda-quant-cat, we introduced ridgeline plots to explore the relationship between a quantitative and categorical variable. We use `geom_density_ridges()` in the **ggridges** package [@ggridges] to create the plots. Similar to the **waffle** package, the functions in **ggridges** are developed to work within the **ggplot2** framework, so we can utilize the functions from @sec-customize-plots to customize the plots.
