...
$$Var_{\hat{\beta}_j} = j^{th} \text{diagonal element of }\hspace{2mm} \hat{\sigma}^2_{\epsilon}(\mathbf{X}^\mathsf{T}\mathbf{X})^{-1}$$
 {#eq-mlr-se-beta}
...
- the equations utilize matrix notation, so we reserve the notation for the appendix. note that the comparison is the same as with simple linearregression if we are doing intervals for individual predictions or the mean preidiction.

- link to notes for equations: <https://sta221-sp26.github.io/hw/hw-03.html#exercise-9>

- show the table with the SE and intervals

- your turn - whcih interval is wider, has larger standard error. is this what you expect?

- interpret the intervals.

- which interval is like the obotstrap interval? is this what you expect?
...
- Explain what the value of the standardized residual means in the context of the data.[^08-mlr-inference-11]
:::

[^08-mlr-inference-11]: The residual means that the observed weight for this lemur is `r round(max_resid, 3)` grams greater than the weight predicted by the model. The standardized residual means that the residual for this observation is `r round(max_std_resid,3)`standard errors above the mean of the distribution of residuals.
