...
::: {.yourturn latex="1"}
The 95% confidence in @fig-bootstrap-lemur-pred is `r round(boot_pred_lb, 3)` to `r round(boot_pred_ub, 3)`. Interpret this interval in the context of the data.[^08-mlr-inference-6]
:::

[^08-mlr-inference-6]: We are 95% confident that the mean weight among 10-month-old male lemurs from the VRUB taxon who were born into litters with 4 lemurs is `r round(boot_pred_lb, 3)` to `r round(boot_pred_ub, 3)` grams.
...
::: {.yourturn latex=""}
The test statistic is 28.181. Do you think this provides evidence in support of or against the null hypothesis?[^08-mlr-inference-7]
:::

[^08-mlr-inference-7]: This is a large test statistic, so it appears to provide evidence against the null hypothesis. We will complete the last two steps of hypothesis testing to see the conclusion.
...
- Which confidence level will you choose to get the most precise interval (interval with smallest margin of error)?[^08-mlr-inference-9]
:::

[^08-mlr-inference-9]: The confidence level 99% produces the most accurate interval. The confidence level 90% produces the most precise interval.
...
- Which interval corresponds to the interval constructed by bootstrapping in @sec-mlr-bootstrap-pred-intervals ?[^08-mlr-inference-10]
:::

[^08-mlr-inference-10]: We use the prediction interval for an individual observation. We are 95% confident Jackson's weight is between `r round(indiv_predict_fit[2], 3)` to `r round(indiv_predict_fit[3], 3)` grams.
...
::: {.yourturn latex=""}
Suppose we decide to fit a model using `taxon`, `age`, `sex`, and `litter_size` to predict the body length of young lemurs (instead of the weight)? How does the leverage for each observation compare in this model to the leverage in the model we've examined using these variables to predict weight?[^08-mlr-inference-11]
:::

[^08-mlr-inference-11]: The leverage of each observation will be the same for the two models. Leverage only depends on the predictor variables. It does not depend on the response variable.

An observation has *high leverage* if its combination of predictors is far away from the typical combination in the data. We use a threshold of $\frac{2(p+1)}{n}$, where $p+1$ is the number of terms in the model and $n$ is the sample size, to determine whether an observation has high leverage.[^08-mlr-inference-12]

[^08-mlr-inference-12]: Note: We may choose to use the higher threshold of $\frac{3(p+1)}{n}$if the sample size is large.
...
- Explain what the value of the standardized residual means in the context of the data.[^08-mlr-inference-13]
:::

[^08-mlr-inference-13]: The residual means that the observed weight for this lemur is `r round(max_resid, 3)` grams greater than the weight predicted by the model. The standardized residual means that the residual for this observation is `r round(max_std_resid,3)` standard errors above the mean of the distribution of residuals.
