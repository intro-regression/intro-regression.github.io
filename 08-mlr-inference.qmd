...
We produce the new permutation samples by shuffling the values of the response variable `weight` without shuffling any of the other columns. By shuffling the values of `weight`, we achieve the goal of creating new samples generated under the null condition of no linear relationship between `weight` and `age`. We see this in @fig-mlr-permute-weight that shoes scatterplots of `weight` versus `age` for the original data (top left corner) and the first weight permutation samples.
...
Though we have acheived the goal of creating samples such that the relationship between `age` and `weight` achieve the null conduction, by permuting `weight`, there is also no longer a relationship between `weight` and the other predictor variables. We see this in @tbl-mlr-inf-permute-weight that shows the values from the original data and values from the first five permutations for an individual lemur. In the table, we see the values for the predictors `age`, `sex`, `taxon`, and `litter_size` are the same for each permutation. Thus, we have preserved any relationship between `age` and the other predictors. The values of `weight` are randomly shuffled, and there is no linear relationship (or any relationship) between `weight` and any of the predictor variables in the permutation samples.
...
Recall that the question of interest was whether there is evidence of a linear relationship between age and weight after accounting for the other predictors. Because the permutation strategy also impacted the relationship between the response variable, weight, and the other predictors, the results from our test, doesn't answer this question exactly. However, studies have shown that in practice, the results from this permutation strategy can still be used to answer our question of interest. \[ANDERSON AND LEG\] found that this permutation strategy has the same Type I error rate and power (see \[SEC REF\]) as permutation methods that preserve the relationships between the response and other predictor variables in most cases. The power of this approach was reduced when the data had extreme outliers, so we point readers to the references in the next section for details on alternative permutation methods. <!--# need to rewrite this. We are only going to discuss permutation of the response variable (put Manly citations - 1997 book and the updated edition). Anderson and Legendre 1999 paper said that this method in most cases, this method showed the same Type 1 error rate and power as other methods in most cases. The power was destablized when there was an extreme outlier, so we will point the reader to alternative permutation sampling methods at the end of this section.-->

Keep in mind, however, that due to the permutation strategy we used, this is [**not**]{.underline} the relationship between these two variables after taking into account the other predictors.
