...
- `litter_size`: Total number of lemurs in the litter the lemur was born into
...
The objective of statistical inference is to use the sample data to draw conclusions about the population of interest. In the context of linear regression, this means using the sample data to draw conclusions about the relationship between the response variable and the predictors. This is done through one of two inferential procedures:
...
Can we answer each question using a confidence interval, hypothesis test, or both?[^08-mlr-inference-1]
:::

[^08-mlr-inference-1]: Question 1: This can be answered by a hypothesis test and confidence interval. Question 2: This question can be answered by a confidence interval.
...
In @sec-hypothesis-test-overview, we explained how hypothesis tests are conducted assuming the null hypothesis is true, and the aim is to evaluate the strength of the evidence against the null hypothesis. Therefore, we need to obtain the sampling distribution of $\hat{\beta}_{age}$ under the assumption that there is no linear relationship between age and weight, after accounting for the other predictors in the model. This sampling distribution is the **null distribution**, and we construct it using permutation sampling.

Permutation sampling is the process of shuffling one or more columns of data, to get many new combinations of the data, i.e., "new" samples. In simple linear regression, we only had to consider the relationship between the response and predictor variable as we permuted the data. In multiple linear regression, however, because there are multiple predictor variables, we need to consider both the relationship between the response variable and predictor of interest but also the relationship between the predictor variables. In this analysis, this means we need to consider both the relationship between `age` and `weight` but also the relationship between `age` and the other predictors `sex`, `taxon`, and `litter_size`. Therefore, we will discuss two different approaches to permutation sampling that build upon inference for simple linear regression in @sec-slr-testing-sim: permute the response variable and permute the predictor variable of interest. We will discuss the implications of each sampling strategy and provide resources to readers interested in more advanced methods.

![Relationships between variables in original lemurs data](images/08-lemur-scattermatrix-original.png){#fig-lemurs-matrix fig-align="center"}

```{r}
#| label: make-lemurs-matrix
#| eval: false

lemurs_sample_young |>
  ggpairs(columns = 4:8, progress = FALSE, 
          upper = "blank", 
          lower = list(combo = "box_no_facet", discrete = "crosstable")
  )
```

@fig-lemurs-matrix is a scatterplot matrix that shows the relationship between each pair of variables in the `lemurs` data frame. For simplicity, we will only focus on two variable pairs as we introduce the two permutation sampling strategies. We'll consider how the relationship between `age` and `weight` changes (or not) with different permutation strategies. The relationship between these two variables is in the scatterplot outlined by the solid blue line. As we observed in @sec-mlr-eda, there is a strong, positive relationship between these two variables.

We'll also consider changes in the relationship between `age` and `taxon`. The relationship between these two variables is shown in the side-by-side boxplots outlined by the dashed pink line. The distribution of `age` largely overlaps between the three taxa; however, VRUB has a slightly higher median compared to ERUF and PCOQ.

#### Permute the response variable {.unnumbered}

Let's start by using the sample permutation strategy we used in @sec-slr-testing-sim, permuting the response variable. We create new samples by randomly shuffling the values of `weight` only without shuffling any of the other columns. By shuffling the values of `weight`, we create new samples generated under the null condition of no linear relationship between `weight` and `age`. Note, however, that with this permutation strategy, there is also no longer a relationship between `weight` and any of the other predictor variables. Therefore, we can use the permutation test to conclude whether there is evidence of a linear relationship between `age` and `weight`, not taking into account the relationship between `weight` and the other predictors.

```{r}
#| label: tbl-mlr-inf-permute-weight
#| tbl-cap: "First five permutations for an individual lemur after permuting `weight`"
#| echo: false
...
permute_samp_weight <- lemurs_sample_young |> 
  specify(weight ~ age + sex + taxon + litter_size) |> 
  hypothesize(null = "independence") |>
  generate(reps = niter, type = "permute")

permute_samp_lemur1 <- permute_samp_weight |> 
  group_by(replicate) |> 
  slice(1) |>
  ungroup() |> 
  slice(1:5)
...
@tbl-mlr-inf-permute-weight shows the values from the original data and values from the first five permutations for an individual lemur. In the table, we see the values for the predictors `age`, `sex`, `taxon`, and `litter_size` are the same for each permutation. The values of `weight` are randomly shuffled, and there is no linear relationship (or any relationship) between `weight` and any of the predictor variables in the permutation samples.

![Relationship between variables in one permutation sample with weight imputed](images/08-lemur-scattermatrix-weight.png){#fig-lemurs-matrix-weight fig-align="center"}

The relationship between variable pairs for a single permutation sample after permuting `weight` is shown in @fig-lemurs-matrix-weight. We can compare these relationships to those from the original data in @fig-lemurs-matrix to see how the data changed as a result of this permutation strategy. As we expect under the null condition, there now appears to be no relationship between `age` and `weight` (scatterplot outlined by the solid blue line). Because we permuted the response variable, the relationship between predictors was unchanged, so we see the same relationship between `age` and `taxon` as we did in the original data (side-by-side boxplots outlined in dashed pink line).

```{r}
#| label: make-fig-lemurs-matrix-weight
#| eval: false 

permute_samp_weight |> 
  ungroup() |>
  filter(replicate == 10) |>
  select(sex, taxon, litter_size, age, weight) |>
...
```{r}
#| label: calc-permute-pval-weight
#|echo: false

pval <- permute_samp_weight |> 
  fit() |> 
  filter(term == "age") |>
...
![Infer warning about reporting p-value of 0.](images/08-infer-pval-warning.png){alt="Infer warning about reporting p-value of 0." fig-alt="Warning from infer output that says \"Please be cautious in reporting a p-value of 0. This result is an approximation based on the number of reps chosen in the generate step. See get_p_value for more information.\"" fig-align="center"}
...
#### Permute single predictor variable  {.unnumbered}

The second permutation strategy is to permute the values for the predictor of interest, `age` in this case. Similar to the previous strategy, this approach helps us achieve the null condition that there is no linear relationship between `age` and `weight`. Because we are only shuffling the values of `age`, it also preserves the relationship between `weight` and the other predictor variables. Therefore, we this strategy will help us construct a null distribution to test the relationship `weight` and `age` after adjusting for the other predictors in the model (exactly what we want to see!).

The primary limitation to this approach is that by permuting `age`, the permutation samples will not account for any relationship between `age` and the other predictors. Recall from @sec-mlr-interpret-quantitative that we assume the predictors are independent. If there is truly no relationship between the predictors, then this is the prefered permutation strategy. In practice, however, there is often some correlation between the predictor variables. If the predictor of interest is strongly correlated with one or more other predictors, then there is *multicollinearity* and we should avoid this permutation strategy. We discuss multicollinearity in @sec-mlr-multicollinearity.

For now, let's conduct the permutation test by permuting `age`.
...
permute_samp <- lemurs_sample_young |> 
  specify(weight ~ age + sex + taxon + litter_size) |> 
  hypothesize(null = "independence") |>
  generate(reps = niter, type = "permute", variables = c(age))
...
@tbl-mlr-inf-permute shows the values from the original data and values from the first five permutations for an individual lemur. In the table, we see the values for `weight`, `sex`, `taxon`, and `litter_size` are the same for each permutation. The values of `age` are randomly shuffled, and there is no linear relationship (or any relationship) between `age` and any of the other variables in the permuted samples. In contrast to permuting the response variable (@tbl-mlr-inf-permute-weight), the relationship between weight and the other predictors, `sex`, `taxon`, and `litter_size` is preserved.

![Relationships between variables in one permutation sample with age imputed](images/08-lemur-scattermatrix-age.png){#fig-lemurs-matrix-age fig-align="center"}

The relationship between variable pairs for a single permutation sample after permtuing `age` is shown in @fig-lemurs-matrix-age. We can compare these relationships to those from the original data in @fig-lemurs-matrix to see how the data changed as a result of this permutation strategy. As we expect under the null condition, there now appears to be no relationship between `age` and `weight` (scatterplot outlined by the solid blue line). Because we permuted `age`, the relationship between `age` and the other predictors changed. We now see no difference in the median age across taxon (see side-by-side boxplots outlined in dashed pink line), compared to a higher median age for VRUB lemurs in the original data.

```{r}
#| label: make-fig-lemurs-matrix-age
#| eval: false
...
As before, we use the permutation samples to construct the null distribution.
...
@fig-mlr-inf-permute-null-dist shows the null distribution with the value of $\hat{\beta}_{age}$ observed in the original data marked with the red dotted line. Similar to @fig-mlr-inf-permute-null-dist-weight, the null distribution is centered around 0. The coefficient estimated from the observed data is very far away from the center of the distribution, so the data appears to provide strong evidence against the null hypothesis. More specifically, the p-value is approximately `r pval_age`. As before, we reject the null hypothesis and conclude there is evidence of a linear relationship between age and weight of young lemurs after adjusting for sex, taxon, and litter size.

::: analysis_in_practice
Something about limitations and here is where we can go to learn more.
:::

We used different strategies of permutation sampling to construct the null distribution for the hypothesis test to evaluate a claim about the relationship between age and weight for young lemurs. Because we conducted a two-sided hypothesis test, we can use the bootstrap confidence interval from @sec-mlr-bootstrap-ci to evaluate this claim as well. The $C\%$ confidence interval corresponds to a test with a decision-making threshold of $\alpha = (1-\frac{C}{100})$.

The bootstrap confidence interval is advantageous, because bootstrap sampling does not have the same limitations as permutation sampling in multiple linear regression, and the confidence interval also gives information about the size of the effect of `age`. See @sec-slr-inf-relationship-ci-test for more detail about the connection between hypothesis test and confidence intervals. <!--# the percent / dividing C by 100 needs to be smoothed out-->
...
- Is this interval consistent with the conclusion that there is evidence of a linear relationship between age and weight, after accounting for sex, taxon, and number of lemurs in the litter? Explain. [^08-mlr-inference-4]
:::

[^08-mlr-inference-4]: It corresponds to a hypothesis test with $\alpha = 0.05$. Yes, the interval is consistent, because the null hypothesized value 0 is not in the interval.

### Prediction intervals {#sec-mlr-bootstrap-pred-intervals}

In @sec-slr-bootstrap-pred-intervals, we introduced a simulation-based approach for quantifying the uncertainty in predictions from a simple linear regression model. We use the same approach to quantify the uncertainty in predictions from multiple linear regression models. The primary difference is now we will quantify the uncertainty for the predicted value given a combination of values for the predictors in the model.

In @sec-mlr-predict, we found that the predicted weight for 10-month old male lemurs in the VRUB taxon who were born into a litter of 4 lemurs is 2282.931 grams. Given the uncertainty in the estimated coefficients used to compute this prediction, we expect the actual mean weight is not exactly but is close to this value. Thus, we can construct an interval to more specifically quantify this uncertainty and produce a plausible range of values we expect the actual mean weight to take.

As in @sec-slr-bootstrap-pred-intervals, we use bootstrap sampling to construct a distribution of predict values computed from the 1,000 bootstrap samples. We then use the distribution to compute the bounds for the interval. @tbl-bootstrap-lemur-pred shows the estimated coefficients and predictions from the first five bootstrap samples.

```{r}
#| label: tbl-bootstrap-lemur-pred
#| tbl-cap: "Estimated coefficienst and predictions in first five bootstrap samples"

set.seed(12345)

niter = 1000

boot_lemur_prediction <- lemurs_sample_young |> 
  specify(weight ~ age + sex + taxon + litter_size) |> 
  generate(reps = niter, type = "bootstrap") |>
  fit() |>
  pivot_wider(
  ...
  mutate(prediction = intercept + age * 10 + sexM * 1 + taxonPCOQ * 0 + taxonVRUB * 1 + litter_size * 4) |>
  ungroup()

boot_lemur_prediction |> 
  slice(1:5) |>
  kable(digits = 3)
```

The distribution of the predicted weights from all 1,000 bootstrap samples with bounds for the 95% interval is @fig-bootstrap-lemur-pred. Recall from @sec-slr-bootstrap-pred-intervals that that this is the confidence interval for the predicted mean weight among all lemurs that have the specified combination of predictors. We discuss the prediction interval for an individual observation in @sec-mlr-theory-predict.

```{r}
#| label: fig-bootstrap-lemur-pred
#| fig-cap: "Bootstrap distribution of predictions with bounds for the 95% interval"

boot_pred_lb <- quantile(boot_lemur_prediction$prediction, 0.025)
boot_pred_ub <- quantile(boot_lemur_prediction$prediction, 0.975)

ggplot(data = boot_lemur_prediction, aes(x = prediction)) + 
  geom_histogram(fill = "darkcyan", color = 'white') + 
  geom_vline(xintercept = boot_pred_lb, linetype = 2 , color = "red") + 
  geom_vline(xintercept = boot_pred_ub, linetype = 2, color = "red") +
...
::: {.yourturn latex="1"}
The 95% confidence in @fig-bootstrap-lemur-pred is `r round(boot_pred_lb, 3)` to `r round(boot_pred_ub, 3)`. Interpret this interval in the context of the data.[^08-mlr-inference-5]
:::

[^08-mlr-inference-5]: We are 95% confident that the mean weight among 10-month-old male lemurs from the VRUB taxon who were born into litters with 4 lemurs is `r round(boot_pred_lb, 3)` to `r round(boot_pred_ub, 3)` grams.

## Theory-based inference {#sec-mlr-theory-inf}

The other approach for inference is using theory-based methods based on mathematical models and the Central Limit Theorem. Theory-based methods have formulas to quantify the sampling variability in the estimated coefficient and use distributions to compute exact confidence intervals and p-values. Because these methods rely on formulas rather than resampling methods, they are very computationally efficient. Results from theory-based inferential methods are displayed in the model output from most statistical software. For example, you can see the inferential results we'll discuss in the model output in @tbl-lemur-model-inf.
...
When doing multiple linear regression, we assume a model structure of the form in @eq-mlr-stat-model.

$$Y = \beta_0 + \beta_1X_1 + \beta_2X_2 + \dots + \beta_pX_p + \epsilon \hspace{8mm} \epsilon \sim N(0, \sigma^2)$$ {#eq-mlr-stat-model}

The equation is written in terms of the distribution of $Y$ given a combination of predictors $X_1, X_2, \ldots, X_p$ in @eq-mlr-dist-y.

$$Y|X \sim N(\beta_0 + \beta_1X_1 + \beta_2X_2 + \dots + \beta_pX_p, \sigma^2)$$ {#eq-mlr-dist-y}

By @eq-mlr-dist-y, the distribution of the response variable $Y$ given a combination of predictors $X_1, X_2, \ldots, X_p$ is normally distributed, centered at $\beta_0 + \beta_1X_1 + \beta_2X_2 + \dots + \beta_pX_p$ with variance $\sigma^2$. Based on this distribution, the following assumptions are made when we conduct multiple linear regression:
...
3.  The variance of the distribution of $Y$ given $X_1, X_2, \ldots, X_p$ is $\sigma^2$. This variance is equal for all combination of predictors $X_1, X_2, \ldots, X_p$ and does not depend on the predictors.
...
In @sec-ch-mlr, we showed how to compute $\hat{\beta}_0, \hat{\beta}_1, \hat{\beta}_2, \ldots, \hat{\beta}_p$, the estimated model coefficients. The remaining parameter $\sigma$, called the regression standard error, is the variability of the observations about the regression line. The distance between the observed values and the regression line (the predicted value of $Y$ for a given combination of predictors) is the residual. @eq-mlr-sigma shows how we use the residuals to estimate the regression standard error.

$$\hat{\sigma}_{\epsilon} = \sqrt{\frac{\sum_{i=1}^n e_i^2}{n - p -1 }} = \sqrt{\frac{\sum_{i=1}^n (y_i - \hat{y}_i)^2}{n - p -1 }}$$ {#eq-mlr-sigma}
...
$$Var_{\hat{\beta}_j} = j^{th} \text{diagonal element of }\hspace{2mm} \hat{\sigma}^2(\mathbf{X}^\mathsf{T}\mathbf{X})^{-1}$$ {#eq-mlr-se-beta}
...
$$\hat{\beta}_j \sim N(\beta_j, SE_{\hat{\beta}_j}^2) $$ {#eq-mlr-beta-dist}
...
### Confidence intervals {#sec-mlr-theory-ci}
...
$$\hat{\beta}_j \pm t^*_{n-p-1} \times SE_{\hat{\beta}_j}$$ {#eq-mlr-beta-ci}
...
- Which confidence level will you choose to get the most accurate interval (interval most likely to contain the true parameter)?

- Which confidence level will you choose to get the most precise interval (interval with smallest margin of error)?[^08-mlr-inference-8]
:::
...
### Prediction intervals {#sec-mlr-theory-predict}

In @sec-slr-theory-predict, we introduced a theory-based approach to construct intervals to quantify the uncertainty in predictions. Intervals produced from mathematical equations are more exact and have much lower computational time compared to intervals computed using simulation-based methods. As with simple linear regression, however, we do require stricter adherence to the model conditions (@sec-mlr-conditions-diagnostics).

Similar to interval for simple linear regression, @eq-mlr-pred-int-general is the general form for the intervals used to quantify uncertainty in predictions.

$$\hat{y} \pm t^* \times SE$$ {#eq-mlr-pred-int-general}

where $t^* \sim t_{n-p-1}$.

In this formula, $\hat{y}$ is the predicted value, $t^*$ is the critical value and $SE$ is the standard error for the prediction.

The predicted value $\hat{y}$ is the value output by the model given a particular value of the predictor variable. The critical value $t^*$ is determined by the confidence level as introduced in @sec-mlr-theory-ci. It is the value on the $t$ distribution with $n-p-1$ degrees of freedom, such that $C\%$ of the area under the curve is between $-t^*$ and $t^*$.

Recall that the model outputs the predicted mean value of the response variable given a value of the predictor variable. We use this same model output if we wish to compute the predicted value for an individual observation. The standard error used to quantify the uncertainty in the predictions $(SE)$ , however, differ depending on the type of prediction we're making.

The formulas for standard error utilize the matrix notation for regression, so we present those in @sec-predict-intervals-matrix. @tbl-lemur-prediction-intervals shows the standard errors and intervals for the two types of predictions for a 10-month old male lemur from the VRUB taxon who was born in a litter of 4 lemurs.

```{r}
#| label: tbl-lemur-prediction-intervals
#| tbl-cap:  "Standard errors and 95% intervals for the predicted mean response and individual response"
...
mean_predict <- predict(lemurs_model, new_lemur, interval = "confidence", level = 0.95, se.fit = TRUE)

mean_predict_fit <- mean_predict$fit
mean_predict_se <- mean_predict$se.fit

mean_predict_all <- bind_cols(mean_predict_fit, mean_predict_se)

names(mean_predict_all) <- c("Predicted", "Lower bound", "Upper bound", "Std. Error")

# interval for individual response 

indiv_predict <- predict(lemurs_model, new_lemur, interval = "prediction", level = 0.95, se.fit = TRUE)

indiv_predict_fit <- indiv_predict$fit
indiv_predict_se <- sqrt(indiv_predict$se.fit^2 + indiv_predict$residual.scale^2)

indiv_predict_all <- bind_cols(indiv_predict_fit, indiv_predict_se)

names(indiv_predict_all) <- c("Predicted", "Lower bound", "Upper bound", "Std. Error")

bind_rows(mean_predict_all, indiv_predict_all)  |>
  select(Predicted, `Std. Error`, `Lower bound`, `Upper bound`) |>
  mutate(Type = c("Mean response", "Individual response"), .before = 1) |>
  kable(digits = 3)

```

::: {.yourturn latex="1"}
Use the intervals in @tbl-lemur-prediction-intervals.

- Jackson is a 10-month old, male lemur from the VRUB taxon, who was born in a litter of 4 lemurs. We are 95% confident his weight is between what range of values?

- Which interval corresponds to the interval constructed by bootstrapping in @sec-mlr-bootstrap-pred-intervals ?[^08-mlr-inference-9]
:::

[^08-mlr-inference-9]: We use the prediction interval for an individual observation. We are 95% confident Jackson's weight is between `r round(indiv_predict_fit[2], 3)` to `r round(indiv_predict_fit[3], 3)` grams.

    The interval that corresponds with the interval constructed by bootstrapping is the interval for the estimated mean response, `r round(mean_predict_fit[2], 3)` to `r round(mean_predict_fit[3], 3)`

## Model conditions and diagnostics {#sec-mlr-conditions-diagnostics}

The simulation- and theory-based inference methods rely on a set of assumptions about the relationship between the response and predictor variables in the population ([@sec-mlr-inf-foundation]). We are unable to check whether these assumptions truly hold in the population, so we use the sample data to evaluate whether they hold in the data using model conditions. We also use model diagnostics to assess whether there are outliers in the data that are having an out-sized influence on the model results. These model conditions and diagnostics are the same as the ones for simple linear regression (@sec-ch-slr-conditions) with some additional considerations for the multiple predictor variables. Here we will show the conditions and diagnostics applied to the lemur model. See @sec-ch-slr-conditions for a detailed introduction to these topics.
...
In the context of multiple linear regression, the linearity condition means that the relationship between the response and the predictor variables can be adequately described using the linear regression model. When this is the case, the residuals $y_i - \hat{y}_i$ capture random variability that stems from the fact that there are factors other than the ones in the model that explain variability in weight and the natural random differences in weight we'd expect in a group of lemurs. To examine this, we use a plot of the residuals versus predicted values. Ideally, the residuals are randomly scattered, indicating the systematic relationship between the response and predictor variables has been captured by the model. The linearity condition is important for simulation-based and theory-based inference, so any violations in linearity should be addressed before conducting inference.

```{r}
#| label: fig-mlr-resid-plot
#| fig-cap: Residuals versus predicted for the lemurs model
#| echo: false
#| fig-align: center
...
#| fig-cap: Residuals versus each predictor in the lemurs model
#| fig-subcap: 
...
The next condition in LINE is normality. This condition is based on the assumption that the error terms are normally distributed, $\epsilon_i \sim N(0, \sigma^2)$. We we use a plot of the residuals to evaluate this claim.
...
The normality assumption is important when computing intervals for predictions of individual observations. Because these intervals take into account where an observation may lie in the distribution of the response variable about the regression line, it relies on the assumption that the shape is approximately normal, unimodal and symmetric.

#### Equal variance {.unnumbered}

The final model condition in LINE is the equal variance condition (also called *constant variance*or *homoskedasticity*). This condition comes from the assumption about the distribution of the error terms, $\epsilon_i \sim N(0, \sigma^2)$, that the variance in this distribution, $\sigma^2$, is the same for all observations regardless of the values of the predictors. Thus, the variability of the residuals should be the same for all predicted values (combinations of predictors).
...
@fig-mlr-equal-var shows a plot of the residuals versus predicted values, similar to @fig-mlr-resid-plot. As we move along the $x$-axis, we are looking to see if the vertical spread is approximately equal for all observations. To help evaluate this, we have added horizontal lines encompassing the majority of the data. Note as we move along the $x$-axis, the vertical spread of the residuals is largely captured within these horizontal lines. Thus, the equal variance condition is [**satisfied**]{.underline}. As with other conditions, we can ignore the high outlier when assessing the condition. Additionally, we are looking for noticeable violations in the conditions rather than perfect adherence when making the determination.
...
#| fig-cap: Cook's distance for each observation in the lemur analysis
#| echo: false
...
::: {.yourturn latex=""}
Suppose we decide to fit a model using `taxon`, `age`, `sex`, and `litter_size` to predict the body length of young lemurs (instead of the weight)? How does the leverage for each observation compare in this model to the leverage in the model we've examined using these variables to predict weight?[^08-mlr-inference-10]
:::

[^08-mlr-inference-10]: The leverage of each observation will be the same for the two models. Leverage only depends on the predictor variables. It does not depend on the response variable.

An observation has *high leverage* if its combination of predictors is far away from the typical combination in the data. We use a threshold of $\frac{2(p+1)}{n}$, where $p+1$ is the number of terms in the model and $n$ is the sample size, to determine whether an observation has high leverage.[^08-mlr-inference-11]

[^08-mlr-inference-11]: Note: We may choose to use the higher threshold of $\frac{3(p+1)}{n}$if the sample size is large.

```{r}
#| label: fig-mlr-leverage
#| fig-cap: Leverage for observations in the lemurs analysis
#| echo: false
...
#| fig-cap: Standardized residuals versus predicted values for the lemur model 
#| echo: false
...
- Explain what the value of the standardized residual means in the context of the data.[^08-mlr-inference-12]
:::

[^08-mlr-inference-12]: The residual means that the observed weight for this lemur is `r round(max_resid, 3)` grams greater than the weight predicted by the model. The standardized residual means that the residual for this observation is `r round(max_std_resid,3)` standard errors above the mean of the distribution of residuals.
...
Ideally the predictors in a multiple linear regression model would be completely independent of one another, so knowing something about one predictor would not provide information about the other predictors. In practice, however, there is often some correlation between a subset of the predictors. This is why some data scientists prefer to interpret coefficients using the language "after adjusting for all other predictors" rather than "holding all else constant" (see @sec-mlr-interpret). This is typically not an issue for the model, but issues may arise if two or more predictors are too highly correlated. High correlation among predictors is called **multicollinearity**. More specifically, multicollinearity means there is near-linear dependencies between the predictors. More details of the mathematics behind multicollinearity are in @sec-multicollinearity-matrix.

If two or more predictors are exactly correlated, then we are unable to compute estimates for some of the model coefficients. This will often show up as `NA` in the model output where the estimated coefficient should be, so software provides a clear signal when this is the case. The more challenging issue, then, is when predictors are strongly correlated, i.e., when there is multicollinearity. Though we are still able to obtain estimates for the model coefficients, there are other concerns with the associated inferential statistics.

When multicollinearity is present, the standard errors for correlated predictors $(SE_{\hat{\beta}_j})$ are very large (see @sec-multicollinearity-matrix for mathematical details). This is because when there is multicollinearity, different combinations of the coefficient estimates produce equally good model fits. Therefore, it becomes less clear what combination of coefficient estimates result in the "best fit" model. This ultimately leads to unreliable inferential conclusions, because the confidence intervals and hypothesis tests in @sec-mlr-theory-inf rely on $SE_{\hat{\beta}_j}$. Because the standard errors are inflated, we may conclude there is no statistically significant relationship between the response and a predictor, when, in fact, there is (i.e., make a Type 1 error).

We detect multicollinearity using the **variance inflation factor (VIF)**. This is a measure of how much $SE_{\hat{\beta}_j}$ for a predictor is inflated due to its correlation with other predictors. @eq-vif is the VIF for the $j^{th}$ predictor.

$$VIF_j = \frac{1}{1 - R^2_j}$$ {#eq-vif}
...
Because we expect at least some correlation between the predictors, a commonly used threshold for identifying concerning collinearity VIF \> 10. @tbl-lemur-vif are the VIF for the predictors in @tbl-lemur-model-inf. Because all the VIFs are less than 10, there are no concerns with multicollinearity in this model. Thus, we can proceed with the analysis.
...
Next, the code to construct the null distribution and compute the p-value for the permutation test is below. Note that the argument `variables = age` is used in `generate()` to specify that `age` is the only column that is permuted (shuffled). The default for `generate()` is to permute all columns.
...
\[ADD SOMETHING HERE ABOUT PERMUTING RESPONSE VS PREDICTOR\]
...
The variance inflation factors can be computed using `vif()` in the **rms** R package [@rms].
...
In this chapter, we built upon inference for simple linear regression introduced @sec-ch-slr-inf and showed how inference can be used to draw conclusions about model coefficients in multiple linear regression models. We showed how to conduct simulation-based inference using bootstrapping for confidence intervals and permutation tests for hypothesis testing. Then, we showed how to compute the values shown in the regression output by conducting inference using mathematical models based on the Central Limit Theorem. Lastly, we showed how to check model conditions and diagnostics for multiple linear regression, building off of the introduction to these topics in @sec-ch-slr-conditions.
