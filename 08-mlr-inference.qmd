...
#| fig-cap: "Relationship betwteen `age` and `weight` in original data and 8 randomly selected permutation samples"
set.seed(12345) 
...
samp_replicates <- sample(1:1000, size = 8, replace = FALSE)


permute_samp_graph <- permute_samp_weight |> 
  filter(replicate %in% samp_replicates) |>
...
lemurs_graph_data <- bind_rows(permute_samp_graph, lemurs_samp_orig) |>
  mutate(replicate = factor(replicate, levels = c("Original", samp_replicates)))
...
We produce the new permutation samples by shuffling the values of the response variable `weight` without shuffling any of the other columns. By shuffling the values of `weight`, we create new samples generated under the null condition of no linear relationship between `weight` and `age` (see @fig-mlr-permute-weight). This is our goal; however, there is also no longer a relationship between `weight` and any of the other predictor variables. Therefore, we can use the permutation test to conclude whether there is evidence of a linear relationship between `age` and `weight`, not taking into account the relationship between `weight` and the other predictors.
