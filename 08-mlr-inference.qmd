...
lemurs_sample_young <- read_csv("data/lemurs-sample-lt24mo.csv") |>
...
#| tbl-cap: Model of weight using various characteristics
#| echo: false
...
#| tbl-cap: Five observations from two bootstrap samples
#| echo: false 
...
#| tbl-cap: Coefficients produced from 10 bootstrap samples
#| echo: false
...
#| fig-cap: Bootstrap sampling distribution for age with bounds of 95% confidence interval
#| echo: false
...
permute_samp_graph <- permute_samp_weight |> 
  filter(replicate <= 8) |>
  select(age, weight, replicate) |>
  mutate(replicate = as.character(replicate))

lemurs_samp_orig <- lemurs_sample_young |>
  select(age, weight) |>
  mutate(replicate = "Original") 

lemurs_graph_data <- bind_rows(permute_samp_graph, lemurs_samp_orig) |>
  mutate(replicate = factor(replicate, levels = c("Original", "1", "2", "3", "4", "5", "6", "7", "8")))

ggplot(data = lemurs_graph_data, aes(x = age, y= weight)) +
  geom_point(alpha = 0.5) +
  facet_wrap(~ replicate) +
  theme_bw()
```

We produce the new permutation samples by shuffling the values of the response variable `weight` without shuffling any of the other columns. By shuffling the values of `weight`, we achieve the goal of creating new samples generated under the null condition of no linear relationship between weight and age. We see this in @fig-mlr-permute-weight that shows scatterplots of `weight` versus `age` for the original data (top left corner) and the first eight permutation samples.

```{r}
#| label: tbl-mlr-inf-permute-weight
#| tbl-cap: "Data for an individual lemur in the orignal data and five permutation samples produced by permuting weight"
#| echo: false
...
Though we have achieved the goal of creating samples such that the relationship between age and weight achieve the null conduct, by permuting the response variable `weight`, there is also no longer a relationship between weight and the other predictor variables. This is illustrated in @tbl-mlr-inf-permute-weight that shows the values from the original data and values from the first five permutations for an individual lemur. In the table, we see the values for the predictors `age`, `sex`, `taxon`, and `litter_size` are the same for each permutation. Thus, we have preserved any relationship between `age` and the other predictors. The values of `weight` are randomly shuffled, and there is no linear relationship (or any relationship) between `weight` and any of the predictor variables in the permutation samples. We'll come back to this as we interpret the conclusion from the test.
...
#| fig-cap: Null distribution for the coefficient of age produced from 1000 permutation samples that were generated by permuting weight
#| echo: false
...
Based on the simulation, the p-value is `r pval`. This means in 1000 permutation samples drawn under the assumption the null hypothesis is true, we did not compute an estimated coefficient for `age` equal to `r round(age_coef, 3)` or more extreme. We compare this p-value to a decision-making threshold $\alpha$ to draw the conclusion (see @sec-slr-inf-conclusion). The p-value of `r pval` is less than $\alpha$ for any chosen threshold, so we *reject the null hypothesis.* The data provide strong evidence in favor of the alternative hypothesis that there is a linear relationship between age and weight.

::: {.analysis_in_practice latex=""}
Though the p-value from the simulation-based hypothesis test is equal to 0, the true p-value is very small but not exactly 0. R, and other analysis software, will produce a warning like the one below to highlight this.

![Infer warning about reporting p-value of 0.](images/08-infer-pval-warning.png){fig-alt="Warning from infer output that says \"Please be cautious in reporting a p-value of 0. This result is an approximation based on the number of reps chosen in the generate step. See get_p_value for more information.\"" fig-align="center"}
...
Recall that the question of interest was whether there is evidence of a linear relationship between age and weight after accounting for the other predictors. Because the permutation strategy also impacted the relationship between the response variable `weight` and the other predictors, the results from our test, doesn't answer this question exactly. However, studies have shown that in practice, the results from this permutation strategy can still be used to answer our question of interest. @anderson1999empirical found that in most cases this permutation strategy has the same Type I error rate and power (see @sec-inf-error) as permutation methods that preserve the relationships between the response and other predictor variables. The power of this approach can be reduced when the data had extreme outliers, so we point readers to the references at the end of this section for details on alternative permutation test methods.

#### Connection to bootstrap confidence intervals {.unnumbered}
...
#### Further reading on permutation tests {.unnumbered}

There have been many approaches proposed to conduct simulation-based inference for multiple linear regression models. Some, like the method presented in this text, permute the original data, while others permute the residuals produced from a model on the original data. For readers interested in an in-depth introduction to these methods, we recommend @manly2018randomization. We also recommend @anderson1999empirical and @hardin2024exchangeability for readers interested in comparing the statistical properties of these various permutation test approaches.
...
#| tbl-cap: Estimated coefficients and predictions from five bootstrap samples
...
#| fig-cap: Bootstrap distribution of predictions with bounds of the 95% interval
...
#| layout-ncol: 2
#| tbl-cap: Comparison of standard error for age in simple and multiple linear regression models
#| echo: false
...
#| fig-cap: Residuals versus predicted values for model n @tbl-lemur-model-inf
#| echo: false
#| fig-align: center
...
#| fig-cap: Residuals versus each predictor variable for model in @tbl-lemur-model-inf
#| fig-subcap: 
...
#| fig-cap: Standardized residuals versus predicted values for the model in @tbl-lemur-model-inf
#| echo: false
...
```{r}
#| label: tbl-lemur-vif
#| tbl-cap: VIF for predictors in @tbl-lemur-model-inf
...
Next, the code to perform permutation sampling and use the $\hat{\beta}_{age}$ from these permuted samples to construct the null distribution and compute the p-value. Note that in `generate()`, the default is permute the response variable.
