...
Despite the growth of self-driving cars, there are still complex moral questions [e.g., @Maxmen2018SelfDrivingCarEthics; @Vemoori2024EthicalDilemmasForbes] and uncertainty about the general readiness and experience individuals have driving with this new technology. David Li, co-founder of Hesai, a remote-sensing technology company, said that the adoption of self-driving cars is not just a question about technology development, but "it is a social question, it is a regulatory question, it is a political question" [@White2025HesaiSensors]. Therefore, it is important to understand how people, some of who may already own or will one day purchase a self-driving car, view this technological phenomenon.
...
$$\text{odds(Y = 1)} = \frac{Pr(Y = 1)}{Pr(Y = 0)} = \frac{Pr(Y = 1)}{1 - Pr(Y = 1)}$$ {#eq-odds}
...
gss24_ai |>
  ...
  kbl(,
      escape = FALSE,
      align = c("l", "c", "c"), 
      col.names = c("Tech Easy", "No (0)", "Yes (1)")) |>
  add_header_above(c(" " = 1, "AI Drive Comfort" = 2)) |>
  collapse_rows(
    columns = 1,
    valign = "middle"
  )
...
::: {.yourturn latex=""}
> Compute the odds ratio of being comfortable with driverless cars for `tech_easy = Agree` versus `tech_easy = Disagree`. Interpret this value in the context of the data.[^11-logistic-4]
:::
...
$$\widehat{\text{aidrive\_comfort}} = \hat{\beta}_0 + \hat{\beta}_1 \times \text{age}$$ {#eq-aidrive-age-linear}
...
$$\hat{\pi} = \hat{\beta}_0 + \hat{\beta}_1 \times \text{age}$$ {#eq-aidrive-age-prob}
...
$$\log\Big(\frac{\pi}{1 - \pi}\Big) = \beta_0 + \beta_1x_1 + \dots + \beta_px_p$$ {#eq-logistic-model}

where $\pi = Pr(Y = 1)$ and $\log\big(\frac{\pi}{1 - \pi}\big)$ is the **logit transformation**, also called the "log odds". The log odds can take any value $-\infty$ to $\infty$, so it does not have the boundary constraints of the probability and odds. The log odds output from @eq-logistic-model can be transformed back to the odds and probabilities, and we will use the probabilities to assign each observation to a predicted class, `aidrive_comfort = 0` or `aidrive_comfort = 1`. We'll discuss this more in @sec-ch-logistic-prediction.
...
\end{aligned}$$ {#eq-logistic-comfort-tech}
...
\end{aligned}$$ {#eq-logistic-comfort-tech-exp}
...
> *The **predicted odds** a respondent who agrees technology makes life easier is comfortable with driverless cars 2.012 (* $e^{0.699}$*) **times** the predicted odds of a respondent who is neutral about whether technology makes life easier.*[^11-logistic-6]

[^11-logistic-6]: Recall from @sec-transform-logy that interpretations of a multiplicative change can be written in terms of a percent change. Thus, this interpretation may equivalently be written as *"The predicted odds a respondent who agrees technology makes life easier are `r (round(exp(0.699), 3) - 1) * 100`% greater than the predicted odds of a respondent who is neutral about whether technology makes life easier."*
...
The coefficient for `Agree` in this model is 0.678. It is close but not equal to the coefficient for `Agree` in the previous model, because now the coefficient is computed after adjusting for `age`. The interpretation is similar as before, taking into account the fact that age is also in the model.
...
\end{aligned}$$ {#eq-logistic-comfort-tech-age-exp}

where $\hat{\pi}$ is the predicted probability a respondent is comfortable with driverless cars.

Based on @eq-logistic-comfort-tech-age-exp, the interpretation of the `Agree` in terms of the odds is the following:

> *The **predictedodds** a respondent who agrees technology makes life easier is comfortable with driverless cars are 1.970 (* $e^{0.678}$*) **times** the predicted odds of a respondent who is neutral about whether technology makes life easier, holding age constant.*
...
- Interpret the coefficient of `Disagree` in terms of the odds of being comfortable with driverless cars.[^11-logistic-7]
:::

[^11-logistic-7]: The log odds a respondent who disagrees technology makes life easier is comfortable with driverless cars are expected to be 0.499 less than the log odds of a respondent who is neutral about whether technology makes life easier, holding age constant.
...
& = -0.015 
\end{aligned}$$ {#eq-age-interpret-log-odds}
...
> *For each additional year increase in age, the **predicted odds** a respondent is comfortable with driverless cars change by a **multiplicative factor** of `r round(exp(-0.015), 3)` (* $e^{-0.015}$*), holding opinions about technology constant.*
...
\end{aligned}$$ {#eq-age-interpret-odds}
...
- How are the odds of being comfortable with driverless cars expected to change when age increases by 5 years?[^11-logistic-8]
:::

[^11-logistic-8]: When age increases by 5, the log odds are expected to decrease by `r round(abs(-0.015 * 5), 3)` ($-0.015 \times 5$ ). The expected odds change by a multiplicative factor of `r round(exp(-0.015 * 5), 3)` ( $e^{-0.015 \times 5}$).
...
\end{aligned}$$ {#eq-logistic-hypotheses}
...
The general process for theory-based inference for a single coefficient in logistic regression is similar as in linear regression. The primary difference is in the distribution of the estimated coefficient $\hat{\beta}_j$.
...
The test statistic, in @eq-logistic-test-stat, is called the Wald test statistic [@wald1943tests], <!--# make sure this is the correct reference-->and it is based on the approximation of the mean and standard error as $n$ is large.

$$z = \frac{\hat{\beta}_j - 0}{SE_{\hat{\beta}_j}}$$ {#eq-logistic-test-stat}
...
The test statistic[^11-logistic-9] is computed as

[^11-logistic-9]: Note the difference between the result here and the model output is because the model output is computed using exact values of $\hat{\beta}_j$ and $SE_{\hat{\beta}_j}$.
...
$$\hat{\beta}_j \pm z^* \times SE_{\hat{\beta}_j}$$ {#eq-logistic-theory-ci}
...
This means we can use a confidence interval to evaluate a claim about a coefficient and get a range of values the population coefficient may take. The following is the relationship between the confidence interval and hypothesis test:
...
- Using $\alpha = 0.05$, do the data provide evidence that the odds are different among those who disagree technology makes life easier compared to those who are neutral (the baseline), after adjusting for age? Explain.
- Show how the 95% confidence interval for `tech_easyDisagree` is computed. Is it consistent with your conclusion in the previous question?[^11-logistic-11]
:::
...
$$\text{empirical logit} = \log\Big(\frac{\pi}{1 - \pi}\Big) = \log\Big(\frac{0.545}{1 - 0.545}\Big) = 0.180$$ {#eq-emp-logit}
...
The code for simulation-based in logistic regression is the same as the code for linear regression introduced in @sec-slr-R. Behind the scenes, the functions in the **infer** R package use the response variable to determine whether to fit a linear or logistic regression model.
...
The code below produces the empirical logit for being comfortable with driverless cars for each level of `tech_easy`, the opinion on whether technology makes life easier.
...
The code to compute the empirical logit based on a quantitative predictor is similar, with the additional step of splitting the quantitative predictor into bins. There are many ways to do this in R; here we will show the functions used for the tables in @sec-logistic-conditions.
...
The **Stat2Data** R package [@Stat2Data] has built-in functions for making an empirical logit plot utilizing the base R plotting functions (instead of **ggplot2**).
...
We can include the argument `out = TRUE` to save the underlying data used to make the plot. Additionally, the argument `showplot = FALSE` will suppress the plot, if we are only interested in the underlying data.
