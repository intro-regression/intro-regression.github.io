...
> Measure ranging 0 to 1 that describes the song's positiveness. Songs with high valence (values close to 1) sound more positive, and songs with lower valence (values close to 0). sound more negative. For example, a song that sounds happy and cheerful will have high valence, and a song that sounds sad or angry will have a low valence.
...
#| fig-cap: "Bivariate exploratory data analysis for Spotify data"
#| fig-subcap: 
#|   - "valence versus playlist_genre"
#|   - "valence versus danceability"
#|   - "valence versus. energy"
#|   - "valence versus loudness"
#|   - "valence versus mode"
#|   - "valence versus acousticness"
#|   - "valence versus instrumentalness"
#|   - "valence versus  liveness"
#|   - "valence versus tempo"
#|   - "valence versus duration"
#| layout-nrow: 4
...
$$\text{Adjusted }R^2 = 1 - \frac{SSR / (n - p - 1)}{SST / (n - 1)}$$ {#eq-adj-rsq}
...
#| tbl-cap: "$R^2$ and Adjusted $R^2$ for two candidate models"
#| echo: false
#| 
...
$$RMSE = \sqrt{\frac{\sum_{i=1}^n(y_i - \hat{y}_i)^2}{n}} = \sqrt{\frac{SSR}{n}}$$ {#eq-rmse}
...
#| tbl-cap: RMSE for two candidate models 
...
: Guidelines for comparing two models, $M_1$ and $M_2$, using AIC and BIC
...
#| tbl-cap: "First step of forward selection (`valence ~ 1` is the intercept-only model)"
...
#| tbl-cap: "Second step of forward selection (`valence ~ 1` is the intercept-only model)"
...
#| tbl-cap: Model selected by forward selection using AIC as selection criteria
...
#| tbl-cap: "First step of backward selection (`-var` indicates which variable is removed)"
...
#| tbl-cap: "Second step of backward selection (`-var` indicates which variable is removed)"
...
#| tbl-cap: Model selected by backward selection using AIC as selection criteria
#| echo: false
...
In this example, the same model was chosen based on forward and backward selection; however, forward and backward selection may not always agree in practice. When different models are chosen by these two methods, we can evaluate the chosen models using the model performance statistics introduced in @sec-model-compare-stats . If our analysis objective is building a model to produce predictions on future observations, the model performance statistics on the selected model may not be the best evaluation on how a model performs on new data. In the next section, we introduce strategies for splitting our data to get reliable evaluations of how models may perform on new data.
...
#| fig-cap: Illustration of strategies for splitting data into training and testing sets (Light blue = Training set, Dark blue = Testing set)
#| fig-subcap: 
...
include_graphics(c("images/10-train-test-random.png", "images/10-train-test-stratified.png","images/10-train-test-time.png"))
...
#| tbl-cap: Model selected by forward selection using AIC as the selection criteria on the training set
#| echo: false
...
#| tbl-cap: Performance on training and testing sets for model in  @tbl-spotify-model-train 
#| echo: false
...
Below is the outline of a model building workflow using cross-validation.
...
- Use a smaller number of folds if there are few observations in the training set. Only one fold is part of the assessment set used to evaluate the model performance at each iteration, so if $k$ is large, there will be very few observations in the assessment set.
...
- Large values of $k$ result in more variability in the estimated model performance statistic, for example RMSE. This means that each iteration may produce very different estimates of the model performance, resulting in an unreliable evaluation of the model performance.
...
#| tbl-cap: Model coefficients produced from 5-fold cross validation 
...
#| tbl-cap: Model performance on the assessment sets in 5-fold cross validation 
#| echo: false
...
2.  Specify the model in the form of a multiple linear regression model (see @eq-mlr-stat-model).
...
Here we introduced the basics of cross-validation, but there are many variations of cross validation, including Leave One Out Cross Validation (LOOCV) and Monte Carlo cross validation. There are also extensions of the training and testing sets, such as a validation set that can be used alongside the training set in the model building process. We point readers to Chapters 10 and 11 of @kuhn2022tidy and Chapter 3 of @kuhn2019feature as starting points for more advanced discussion of these topics.
