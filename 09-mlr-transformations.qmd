...
The data set `ncaa-basketball-DI-2023-2024.csv` contains data from the 2023 - 2024 academic year. We will use the following variables that have been derived from variables in the EADA.
...
#| fig-cap: Distribution of the response variable, expenditure in millions
...
#| tbl-cap: "Summary statistics for enrollment and expenditure" 
...
#| fig-subcap: 
#|   - "expenditure versus enrollment"
#|   - "expenditure versus region"
#|   - "expenditure versus type"
...
From the bivariate exploratory data analysis in @fig-ncaa-bivariate-eda-1, there appears to be a strong positive relationship between the enrollment and expenditure. This aligns with what we might expect; colleges with more students are likely to spend more on basketball programs. We note that this trend does seem to level off for colleges with very high enrollment, because their expenditure is lower than what we would expect if expenditure increased as a constant rate for values of enrollment.
...
#| fig-subcap: 
#|    - "expenditure versus enrollment by type"
#|    - "expenditure versus enrollment by region"
#| layout-ncol: 2
...
#| tbl-cap: Model of NCAA basketball expenditure with 95% confidence intervals for the coefficients
...
#| fig-cap: Residual plots from the original NCAA basketball expenditure model in @tbl-ncaa-model-orig
#| fig-subcap: 
...
There is a "fan" shape in @fig-ncaa-orig-resid, so the equal variance condition is not met. Because the equal variance condition is not met, the results of theory-based inference output in @tbl-ncaa-model-orig cannot reliably be used to draw conclusions about the relationships between the response and predictor variables. For example, the inferential results suggest the interaction between enrollment and institution type is not useful in this model, despite what we observed in @fig-ncaa-eda-multivariate. Before making a final conclusion about the interaction term and any other term in the model, let's first address the issue with the equal variance condition.
...
#| fig-cap: "Relationship between expenditure and enrollment in four candidate models"
#| layout-ncol: 2
#| fig-subcap: 
#|   - y versus x
#|   - log(y) versus x
...
[^09-mlr-transformations-3]: From the figure, it appears @fig-ncaa-model-compare-4 with a log transformation on both `enrollment_th` and `expenditure_m` would be best represented by a linear model, because the shape is most linear.
...
The coefficients of a regression model may still be interpretable if the equal variance condition is not met (see @sec-slr-conditions-recap). Thus, if we aim to interpret the model coefficients without theory-based inference, then we can use the original model and not transform the response variable.

It is good practice to report inferential results alongside interpretations, so we can conduct simulation-based inference ([@sec-mlr-sim-inf]) if we do not wish to transform the response variable.
:::
...
$$\log(y_i) = \beta_0 + \beta_1x_{i1} + \beta_2x_{i2}+ \dots + \beta_px_{ip} + \epsilon_i \hspace{8mm} \epsilon_i \sim N(0, \sigma^2)$$ {#eq-log-y}

::: {.math_rules latex=""}
**log = ln**

In statistics, "$\log$" refers to the natural log (typically denoted "$\ln$" in mathematics).
:::

As shown in @eq-log-y, the log transformation is applied to the individual values of $Y$, and the transformed values are used in the model. Therefore, when we use a log (or any) transformation on the response variable, we apply the assumptions from @sec-slr-foundation to the relationship between the predictors and the transformed values of $Y$. Using @eq-log-y, this means we make assumptions about the relationship between the predictors and $\log(y_i)$. The full analysis, including evaluating conditions and diagnostics, conducting inference, and computing predictions are done in terms of $\log(y)$. Interpretations, however, are in terms of the original variable $y$. Although we can write interpretations in terms of $\log(y)$ (we'll see an example of this shortly), it is not as intuitive for readers to understand interpretations on a log-scale as on the natural scale.
...
#| fig-cap: "Univariate and bivariate EDA for original and log-transformed expenditure"
#| fig-subcap: 
#| - "Distribution of expenditur"
#| - "Distribution of log(expenditure)"
#| - "expenditure versus enrollment"
#| - "log(expenditure) versus enrollment"
#| layout-ncol: 2
...
Now we fit the main effects model using `log(expenditure_m)`, the log-transformed expenditure, as the response variable. The output for the new model is in @tbl-ncaa-model-logy and the equation is in @eq-ncaa-model-logy.

```{r}
#| label: tbl-ncaa-model-logy
#| tbl-cap: Model with log(expenditure) as the response variable 
#| echo: false
...
\end{aligned}$$ {#eq-ncaa-model-logy}
...
#| fig-cap: "Residuals versus predicted values for original model in @tbl-ncaa-model-orign and model with log(expenditure) in @tbl-ncaa-model-logy"
#| fig-subcap: 
#| - Original model 
#| - "Model with log(expenditure)"
#| layout-ncol: 2
#| echo: false
...
Comparing the plots in @fig-ncaa-logy-resid, the first observation is that the $y$-axis differs on the plots. This is the because the residuals are on the same scale and use the same units as the response variable. The residuals in @fig-ncaa-logy-resid-1 are in terms of the original variable `expenditure_m`, and the residuals in @fig-ncaa-logy-resid-2 are in terms of `log(expenditure_m)`. The same is true for the predicted values on the $x$-axis. The second observation is that the vertical spread of the residuals is much more equal in @fig-ncaa-logy-resid-2 with the log-transformed response variable. Given this, it appears applying a log transformation on the response variable has addressed some of the problems with the equal variance condition.
...
> *For each additional 1000 students at an NCAA Division I college, the expected total expenditure on basketball programs **is multiplied by a factor** of `r round(exp(0.056), 3)` (*$e^{0.056}$*), holding region and type constant.*
>
> *The expected total expenditure on basketball programs for NCAA Division I colleges in the Northeast region is `r round(exp(-0.311), 3)` (*$e^{-0.311}$*) **times** the expenditure at colleges in the North Central region, holding enrollment and type constant.*

::: {.analysis_in_practice latex=""}
Another common interpretation of the coefficients in terms of the original variable is in terms of percent change. Given the model coefficient $\hat{\beta}_j$, the multiplicative change of $e^{\hat{\beta}_j}$in the response is equivalent to a $(e^{\hat{\beta}_j} - 1) \times 100 \%$ change in the response variable. For example, we may rewrite the interpretation for the effect of enrollment as

> *For each additional 1000 students at an NCAA Division I college, the expected total expenditure on basketball programs* is expected to change by *`r (round(exp(0.056), 3) - 1) * 100` %, holding region and type constant.*
:::

::: {.yourturn latex=""}
Write the interpretation for `typePublic` in @tbl-ncaa-model-logy in terms of the change in expenditure. [^09-mlr-transformations-4]
:::
...
$$y_i = \beta_0 + \beta_1x_{i1} + \dots + \beta_j\log(x_{ij}) + \dots + \beta_px_{ip} + \epsilon_i  \hspace{5mm} \epsilon_i \sim N(0, \sigma^2)$$ {#eq-mlr-logx}
...
&+ \beta_4\text{West}_i + \beta_5\text{Public}_i + \epsilon_i \hspace{5mm} \epsilon_i \sim N(0, \sigma^2)
\end{aligned}$$ {#eq-ncaa-logx}
...
#| tbl-cap: "Model with log(enrollment)"
#| eval: true
...
> *When the enrollment doubles (is multiplied by 2), the expenditure on basketball programs at NCAA Division 1 institutions is expected to increase by `r round(log(2) * model_logx_enrollment, 3)` (`r round(model_logx_enrollment, 3)`* $\times$ *log(2)) million dollars, on average, holding type and region constant.*
...
#| fig-cap: "Residuals vs. fitted values for model with log(enrollment) in @tbl-ncaa-model-logx"
...
$$\log(y_i) = \beta_0 + \beta_1x_{i1} + \dots + \beta_j\log(x_{ij}) + \dots + \beta_px_{ip} + \epsilon_i  \hspace{5mm} \epsilon_i \sim N(0, \sigma^2)$$ {#eq-mlr-logx-logy}
...
&+ \beta_4\text{West}_i + \beta_5\text{Public}_i + \epsilon_i \hspace{5mm} \epsilon_i \sim N(0, \sigma^2)
\end{aligned}$$ {#eq-ncaa-logx-logy}

```{r}
#| label: tbl-ncaa-model-logx-logy
#| tbl-cap: Model with log(expenditure) and log(enrollment)
...
#| fig-cap: Residuals versus predicted values for model with log(enrollment) and log(expenditure) in @tbl-ncaa-model-logx-logy
...
#| tbl-cap: "$R^2$ values for the four candidate models"
...
#| fig-cap: log(expenditure) versus log(enrollment) by institution type
#| 
...
#| tbl-cap: Model including interaction effect between log(enrollment) and institution type with 95% confidence intervals for the coefficients
...
> *For each additional 10% increase (* $C = 1.1$*) in enrollment at **private** NCAA Division I **private** colleges, the expenditure on basketball programs is expected to multiply by `r round(1.1^(enrollment_private), 3)`* $(1.1^{0.524})$*, holding region constant.*
>
> *For each additional 10% increase (* $C = 1.1$*) in enrollment at **public** NCAA Division I colleges, the expenditure on basketball programs is expected to multiply by `r round(1.1^(enrollment_private + enrollment_public_int), 3)`* $(1.1^{0.524 + 0.255})$*, holding region constant.*
...
#| tbl-cap: Exponentiated coefficients for model using log-transformed response and predictor
#| echo: true
